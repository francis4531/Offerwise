...
def post_close_survey_submit():
    ...
    # ── FLYWHEEL: agent post-close trigger (v5.87.40) ──────────────────────
    # If the buyer reports closed AND the analysis was shared by an agent,
    # fire the agent post-close flywheel email. This converts the agent
    # flywheel from agent-self-report (rare — depends on agent logging in
    # to mark closed) to buyer-driven (frequent — fires whenever a buyer
    # completes their post-close survey). The match is on the buyer's
    # email + property address, since AgentShare doesn't carry an
    # analysis_id FK. Both conditions must hold to avoid false positives
    # on shared addresses.
    try:
        if survey.did_buy == 'yes_closed' and survey.user_id:
            from models import AgentShare, User
            buyer = User.query.get(survey.user_id)
            buyer_email = (buyer.email if buyer else '') or ''
            prop_addr = survey.property_address or ''
            if buyer_email and prop_addr:
                # Find AgentShare(s) where the buyer email and address match.
                # In normal flow there should be at most one match per (buyer, address)
                # tuple, but we iterate to handle edge cases (duplicate shares).
                shares = AgentShare.query.filter(
                    AgentShare.buyer_email == buyer_email,
                    AgentShare.property_address == prop_addr,
                ).all()
                # Only fire on shares that haven't already been marked closed,
                # to avoid duplicate emails if the buyer re-submits the survey.
                from flywheel_notifications import send_agent_postclose_email
                for share in shares:
                    if getattr(share, 'deal_closed_at', None) is not None:
                        continue
                    send_agent_postclose_email(
                        agent_share_id=share.id,
                        final_sale_price=survey.final_price,
                    )
                    logging.info(f"🎉 Agent post-close fired from buyer survey: share_id={share.id}")
    except Exception as e:
        logging.warning(f"Agent post-close flywheel from survey failed (non-fatal): {e}")
