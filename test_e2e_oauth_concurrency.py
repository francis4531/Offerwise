...
class TestAppleOAuthCallback(unittest.TestCase):
    ...
    def test_apple_callback_new_user_creates_account(self):
        ...
        mock_apple.authorize_access_token.return_value = fake_token

        with _patch.object(auth_routes, 'apple', mock_apple):
            r = self.client.get('/auth/apple/callback', follow_redirects=False)

        with self.app.app_context():
            user = self.User.query.filter_by(email=email).first()
            self.assertIsNotNone(user, 'Apple new user must be created')
            self.assertEqual(user.auth_provider, 'apple')
            self.assertEqual(user.apple_id, 'apple_id_new')
            self.assertEqual(user.name, 'Apple NewUser',
                'Name must be assembled from firstName + lastName')

    def test_apple_callback_subsequent_login_no_name_does_not_clobber(self):
        """Apple omits 'name' on subsequent logins. The callback must NOT
        clobber the existing user's name with an empty string."""
        email = _unique_email('apple_subsequent')

        # Pre-create user
        with self.app.app_context():
            user = self.User(
                email=email,
                name='Existing Apple Name',
                auth_provider='apple',
                apple_id='apple_id_subsequent',
                tier='free',
                analysis_credits=2,
            )
            self.db.session.add(user)
        ...
        # Apple omits 'name' on subsequent logins
        fake_token = {
            'userinfo': {
                'email': email,
                'sub': 'apple_id_subsequent',
                # No 'name' key
            }
        }
        ...
        with self.app.app_context():
            user = self.User.query.get(uid)
            self.assertEqual(user.name, 'Existing Apple Name',
                f'CRITICAL: Apple subsequent login clobbered name. '
                f'Got name={user.name!r}, expected "Existing Apple Name"')

    def test_apple_callback_existing_user_zero_credits_restored(self):
        """Apple login by an existing user with 0 credits, never paid,
        zero analyses → 1 credit restored."""
        email = _unique_email('apple_restore')
        ...
        with self.app.app_context():
            user = self.User(
                ...
                name='Restore Test',
                auth_provider='apple',
                apple_id='apple_id_restore',
                tier='free',
                analysis_credits=0,        # zero credits
        ...
        with self.app.app_context():
            ...
            self.assertEqual(user.analysis_credits, 1,
                'Apple login: 0-credits + never-paid + 0-analyses → '
                f'1 free credit restored. Got {user.analysis_credits}')

    def test_apple_callback_no_email_redirects(self):
        fake_token = {
            'userinfo': {
                'sub': 'apple_id_no_email',
                # No email
            }
        }
        ...
        self.assertNotEqual(r.status_code, 500,
            'Apple callback without email must redirect, not crash')
