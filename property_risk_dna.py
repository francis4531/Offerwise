...
class PropertyRiskDNAEncoder:
    ...
    def _encode_financial_risk(self, analysis: Dict, metadata: Dict) -> np.ndarray:
        ...
        vector[8] = property_price / buyer_budget if buyer_budget > 0 else 1.0
        vector[9] = (property_price + total_cost) / buyer_budget if buyer_budget > 0 else 1.0
        vector[10] = self._calculate_affordability_score(property_price, total_cost, buyer_budget)
        vector[11] = metadata.get('risk_tolerance') or 0.5
    ...
    def _calculate_composite_score(
        ...
        """
        Calculate composite risk score from DNA signature with alignment correction
        
        PATENTABLE: Two-stage methodology
        - Stage 1: Calculate independent DNA-based composite
        - Stage 2: Apply alignment correction to ensure consistency
        
        Args:
            dna_signature: 64-dimensional risk vector
            overall_risk_score: Overall risk from main scoring model (for alignment)
        
        Returns:
            Aligned composite score
        """
        ...
        weights = {
            ...
            'systems': 0.20,
            'transparency': 0.20,  # Trust matters!
            'temporal': 0.15,
            'financial': 0.15
        }
        
        # Stage 1: Calculate independent DNA composite
        dna_composite = (
            np.mean(structural) * weights['structural'] * 100 +
            np.mean(systems) * weights['systems'] * 100 +
            np.mean(transparency) * weights['transparency'] * 100 +
            np.mean(temporal) * weights['temporal'] * 100 +
            np.mean(financial) * weights['financial'] * 100
        )
        
        dna_composite = min(dna_composite, 100.0)
        
        # Stage 2: Apply alignment correction if overall_risk_score provided
        if overall_risk_score is not None:
            # Check alignment
            ...
            if gap > 15:
                ...
                # The correction factor increases with gap size
                # Uses weighted blend favoring overall_risk when gap is large
                
                # Calculate correction weight (0.0 to 0.7 based on gap)
                # Gap 15 = 0.0 weight, Gap 50+ = 0.7 weight
                correction_weight = min((gap - 15) / 50.0, 0.7)
                
                # Blend: More correction for larger gaps
                # This preserves DNA insights while ensuring logical consistency
                corrected_composite = (
                    dna_composite * (1 - correction_weight) +
                    overall_risk_score * correction_weight
                )
                
                logger.info(f"   🔧 Alignment correction applied:")
                logger.info(f"      DNA composite (raw): {dna_composite:.1f}")
                logger.info(f"      Overall risk score: {overall_risk_score:.1f}")
                logger.info(f"      Gap: {gap:.1f} points")
                logger.info(f"      Correction weight: {correction_weight:.2f}")
                logger.info(f"      Corrected composite: {corrected_composite:.1f}")
                
                return min(corrected_composite, 100.0)
            else:
                # Good alignment - use DNA composite as-is
                logger.info(f"   ✅ DNA composite aligned (gap: {gap:.1f} points)")
                return dna_composite
        else:
            # No overall_risk_score provided - use DNA composite only
            return dna_composite
