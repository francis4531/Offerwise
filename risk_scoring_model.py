        ...
        for cat_score in category_scores:
            if cat_score.score >= 75:  # CRITICAL threshold
                ...
                if valid_issue_text:
                    ...
                    # v5.59.26: Clean up issue text for natural sentence flow
                    # Remove trailing period since we'll add our own sentence structure
                    valid_issue_text = valid_issue_text.rstrip('.')
                    
                    if cat_score.estimated_cost_high > 0:
                        deal_breakers.append(
                            f"{category_name} exhibits {valid_issue_text}. "
                            f"Estimated repair costs range from ${cat_score.estimated_cost_low:,.0f} to ${cat_score.estimated_cost_high:,.0f}. "
