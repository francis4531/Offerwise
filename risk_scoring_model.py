...
class RiskScoringModel:
    ...
    def calculate_risk_score(
        ...
        for category in RiskCategory:
            ...
            total_cost_low += cat_score.estimated_cost_low
            total_cost_high += cat_score.estimated_cost_high
    ...
    def _score_category(
        ...
        for finding in findings:
            ...
            if finding.estimated_cost_low:
                total_cost_low += finding.estimated_cost_low
                total_cost_high += finding.estimated_cost_high or finding.estimated_cost_low
            
            # Track critical attributes
            if finding.severity in [Severity.CRITICAL, Severity.MAJOR]:
                key_issues.append(finding.description[:100])
    ...
    def _calculate_weighted_score(self, category_scores: List[CategoryRiskScore]) -> float:
        ...
        if has_critical and avg_score < 60:
            avg_score = max(avg_score, 60)  # Floor at 60 for critical issues
    ...
    def _identify_deal_breakers(
        ...
        for cat_score in category_scores:
            ...
            if cat_score.severity_breakdown.get('critical', 0) > 0:
                for issue in cat_score.key_issues:
                    deal_breakers.append(f"CRITICAL {cat_score.category.value}: {issue}")
            
            # Category-specific deal-breakers
            if cat_score.category == RiskCategory.FOUNDATION_STRUCTURE and cat_score.score > 70:
                deal_breakers.append(f"Foundation issues with risk score {cat_score.score:.0f}/100")
            
            if cat_score.affects_insurability:
                deal_breakers.append(f"Insurance concerns in {cat_score.category.value}")
        
        # Apply buyer profile deal-breakers
        if buyer_profile:
            for cat_score in category_scores:
                cat_name = cat_score.category.value
                if cat_name in buyer_profile.deal_breakers and cat_score.score > 40:
                    deal_breakers.append(f"Buyer deal-breaker: {cat_name} (score: {cat_score.score:.0f})")
