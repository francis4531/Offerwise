...
FUNNEL_STAGES = [
    ...
    # Retention / engagement (v5.88.43)
    "watch_optin",          # User opted into PropertyWatch on an analysis
]

AD_CHANNELS = ["google_ads", "reddit_ads", "zillow", "organic", "direct", "referral"]
...
def fetch_ga4_funnel(days: int = 30) -> dict | None:
    ...
    try:
        ...
        if 'SERVICE_DISABLED' in err_str or '403' in err_str or 'has not been used' in err_str:
            logger.warning(
                "GA4 Analytics Data API not enabled — visit "
...
def get_ad_performance_summary(db_session, models, days: int = 30) -> dict:
    ...
    # ── Seed stubs for all active/upcoming paid channels ──
    ACTIVE_CHANNELS = ("google_ads", "reddit_ads", "zillow", "nextdoor", "internachi")
    summary = {
        ch: {
