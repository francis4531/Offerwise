...
def api_gtm_generate_content():
    ...
    try:
        ...
        if post_data is None:
            # v5.89.221: hard data gate — we don't fabricate stats. No real sample, no post.
            return jsonify({"status": "insufficient_data",
                            "message": "Not enough real analysis data to make a data-backed post. Nothing generated by design."}), 200
...
def api_gtm_generate_batch():
    ...
    try:
        ...
        for i in range(days):
            ...
            if post_data is None:
                insufficient.append(target_date.isoformat())  # data gate: no fabrication
                continue
            post = GTMSubredditPost(
        ...
        _db.session.commit()
        return jsonify({"status": "ok", "generated": generated, "skipped": skipped, "insufficient_data": insufficient})
    except werkzeug.exceptions.HTTPException:
        raise
    except Exception as e:
        _db.session.rollback()
