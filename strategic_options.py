...
class StrategicOptionsGenerator:
    ...
    def generate_options(
        ...
        Args:
            ...
            market_context: Market intelligence context (Phase 2)
            
        Returns:
            List of 3 StrategicOption objects
        """
        self._market = market_context or {}
    ...
    def _generate_moderate_options(
        ...
        # Phase 2: Market-aware rationale enhancements
        market_line = self._market_rationale_line()
        leverage_line = self._leverage_tactic_line(property_price)
        ...
        if leverage_line:
            tactics_list.append(leverage_line)
        ...
        options.append(StrategicOption(
            ...
            rationale=(
                ...
                + (f" {market_line}" if market_line else "")
            ),
            risk_level="medium",
            probability_success="medium",
            next_steps=[
    ...
    # ── Phase 2: Market Context Helpers ───────────────────────────────────

    def _market_rationale_line(self) -> str:
        """Generate a market context sentence for strategy rationale."""
        m = self._market
        if not m.get('market_applied'):
            return ""
        
        temp = m.get('market_temperature', '')
        leverage = m.get('buyer_leverage', '')
        avm_pct = m.get('asking_vs_avm_pct', 0)
        
        parts = []
        if temp in ('cool', 'cold'):
            parts.append("market conditions are in your favor with inventory rising and homes taking longer to sell")
        elif temp == 'hot':
            parts.append("the market is competitive right now, so move decisively if you want this property")
        
        if avm_pct > 5:
            parts.append(f"this home is listed {avm_pct:.0f}% above its estimated market value")
        elif avm_pct < -3:
            parts.append(f"this home appears priced {abs(avm_pct):.0f}% below market value — a potential opportunity")
        
        if not parts:
            return ""
        
        return "Market data shows " + "; ".join(parts) + "."
    
    def _leverage_tactic_line(self, property_price: float) -> str:
        """Generate a market-based tactic line for the tactics list."""
        m = self._market
        if not m.get('market_applied'):
            return ""
        
        comp_median = m.get('comp_median_price', 0)
        if comp_median > 0 and property_price > 0:
            diff_pct = round((property_price - comp_median) / comp_median * 100, 1)
            if diff_pct > 3:
                return f"Leverage: comparable homes sold at ${comp_median:,.0f} median — asking price is {diff_pct:.0f}% above comps"
            elif diff_pct < -3:
                return f"Note: comparable homes sold at ${comp_median:,.0f} median — this is priced below market"
        return ""
