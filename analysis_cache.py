...
class AnalysisCache:
    ...
    def get(self, cache_key: str) -> Optional[Dict[Any, Any]]:
        ...
        if row:
            ...
            if datetime.now() - created_dt < timedelta(days=90):
                ...
                # Invalidate stale cache entries that have no market data
                # Only applies to real analyses (must have offer_strategy key)
                # These predate market intelligence and will show "Run fresh analysis"
                if 'offer_strategy' in result:
                    _mc = (result.get('offer_strategy') or {}).get('market_context') or {}
                    _mi = result.get('market_intelligence') or {}
                    if not _mc.get('market_applied') and not _mi.get('avm_price'):
                        conn.close()
                        logging.info(f"🔄 Cache STALE (no market data): {cache_key[:16]}... — forcing fresh analysis")
                        return None
    ...
    def delete_by_address(self, property_address: str) -> int:
        """
        Delete all cache entries for a given property address.
        Called when a user deletes an analysis so re-runs get fresh results.
        ...
        cursor = conn.cursor()
        cursor.execute(
            'DELETE FROM analysis_cache WHERE property_address = ?',
            (property_address,)
        )
        ...
        conn.close()
        if deleted:
            logging.info(f"🗑️ Cache invalidated: {deleted} entr{'y' if deleted==1 else 'ies'} for {property_address}")
        return deleted
