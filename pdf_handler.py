...
def extract_tds_field_state(text):
    """
    Parse vision/OCR-extracted TDS text into the structured field-state dict the
    reasoning TDS parser (reasoning/tds_parser.py) consumes.

    This is the bridge from the existing extraction layer (PDFHandler /
    extract_text_via_vision, which already emit [X]/[ ] checkbox markers via the
    TDS-specific vision prompt) to the reasoning pipeline. It does NOT do OCR — it
    reuses the text those extractors already produce. It reuses the same Section
    A/B/C vocabulary as is_tds_complete() so TDS knowledge lives in one place.

    Returns:
        (field_state: dict | None, confidence: float, notes: list)
      field_state is None when the text is not a recognizable/complete TDS
      (caller should then skip reasoning rather than fabricate field state).
      Shape matches reasoning.tds_parser.parse_tds_field_state input:
        {section_A_present, section_A_freetext, section_B_defects_checked,
         section_C_yes, section_D}
    """
    import re

    complete, score, missing = is_tds_complete(text or "")
    notes = []
    if not text:
        return None, 0.0, ["no_text"]
    if score < 0.4:
        # Not enough TDS signal to trust structured extraction; do not guess.
        return None, score, ["tds_signal_too_low"] + (missing or [])

    t = text  # keep original case for free-text; use lower copies for matching
    low = t.lower()

    # Segment into section regions so a label in one section can't match a box
    # in another (e.g. 'plumbing' in Section A 'Plumbing Fixtures' vs Section B
    # 'Plumbing'). Boundaries are the section headers; missing headers => empty.
    def _region(start_pat, end_pats):
        ms = re.search(start_pat, low)
        if not ms:
            return ""
        start = ms.end()
        end = len(low)
        for ep in end_pats:
            me = re.search(ep, low[start:])
            if me:
                end = min(end, start + me.start())
        return low[start:end]

    region_A = _region(r'section\s*a\b', [r'section\s*b\b'])
    region_B = _region(r'section\s*b\b', [r'section\s*c\b'])
    region_C = _region(r'section\s*c\b', [r'section\s*d\b'])
    region_D = _region(r'section\s*d\b', [r'\Z'])
    # Fallback: if headers weren't detected, use the whole text for each (best effort).
    region_A = region_A or low
    region_B = region_B or low
    region_C = region_C or low
    region_D = region_D or low

    def _checked_in(region, label):
        """True if the box immediately following `label` within `region` is [X]."""
        for m in re.finditer(re.escape(label.lower()), region):
            tail = region[m.end():m.end() + 18]
            mk = re.search(r'\[\s*([x ])\s*\]', tail)
            if mk:
                return mk.group(1) == 'x'
        return False

    # ── Section A presence (decision-relevant items only; reuse vocabulary) ──
    A_present = []
    a_items = {
        'smoke_detectors_present': 'smoke detector',
        'co_devices_present': 'carbon monoxide',
        'water_conserving_fixtures': 'water-conserving',
        'water_heater_gas': 'water heater',
        'central_heating': 'central heating',
        'central_air': 'central air',
    }
    for field, label in a_items.items():
        if _checked_in(region_A, label):
            A_present.append(field)

    # ── Section A free-text: roof type + age ────────────────────────────────
    A_freetext = {}
    mroof = re.search(r'roof[^\n]{0,40}?(tile|composition|shingle|shake|metal|tar|flat|clay|slate)', region_A)
    if mroof:
        A_freetext['roof_type'] = mroof.group(1).title()
    mage = re.search(r'(?:roof[^\n]{0,30}?age|age[^\n]{0,10}?)(\d{1,3})\s*(?:years|yrs|yr)?', region_A)
    if mage:
        try:
            A_freetext['roof_age'] = int(mage.group(1))
        except ValueError:
            pass

    # ── Section B defects: a checked category box = disclosed defect ─────────
    B_checked = []
    b_items = {
        'B_interior_walls': 'interior wall',
        'B_roof': 'roof',
        'B_foundation': 'foundation',
        'B_slab': 'slab',
        'B_plumbing_sewers_septics': 'plumbing',
        'B_electrical_systems': 'electrical',
        'B_windows': 'window',
    }
    # Section B is only meaningful in the "defects" region; require the box checked
    for field, label in b_items.items():
        if _checked_in(region_B, label):
            B_checked.append(field)

    # ── Section C 1-16: answered Yes -> concern. Look for "Yes [X]" per item ─
    C_yes = []
    c_items = {
        'C1_environmental_hazards': 'environmental hazard',
        'C3_easements_encroachments': 'easement',
        'C4_unpermitted_additions': 'room addition',
        'C5_work_not_to_code': 'permit',
        'C6_fill_soil': 'fill',
        'C7_settling_slippage': 'settling',
        'C8_flooding_drainage': 'drainage',
        'C9_major_damage_disaster': 'major damage',
        'C10_zoning_violations': 'zoning',
        'C12_ccrs_deed_restrictions': 'cc&r',
        'C13_hoa': 'homeowners',
        'C15_notices_citations': 'citation',
        'C16_lawsuits': 'lawsuit',
    }
    for field, label in c_items.items():
        # 'Yes [X]' near the label => seller is aware (a concern). We look for a
        # 'yes' followed by [x] within the label's window.
        for m in re.finditer(re.escape(label), region_C):
            ws = max(0, m.start() - 10); we = min(len(region_C), m.end() + 60)
            window = region_C[ws:we]
            myes = re.search(r'yes\s*\[x\]', window)
            if myes:
                C_yes.append(field)
                break
    ...
    D = {}
    # D phrasing is "certifies ... compliance [X]" and "water heater braced/anchored".
    mcompl = re.search(r'compl\w*\s*\[\s*x\s*\]', region_D) or re.search(r'\[\s*x\s*\]\s*compl', region_D)
    if mcompl or ('smoke detector' in region_D and 'compl' in region_D and '[x]' in region_D):
        D['D1_smoke_detector_compliance'] = 'certified'
    if re.search(r'(braced|anchored|strapped)', region_D):
        D['D2_water_heater_braced'] = 'certified'

    field_state = {
        'section_A_present': A_present,
        'section_A_freetext': A_freetext,
        'section_B_defects_checked': B_checked,
        'section_C_yes': C_yes,
        'section_D': D,
    }

    # Confidence reflects the underlying TDS completeness score; flag that
    # checkbox reading from OCR text is inherently approximate.
    notes.append(f"tds_completeness={score:.2f}")
    if not complete:
        notes.append("partial_tds_extraction")
    return field_state, score, notes
