...
class _UserDripEntry:
    ...
    v5.88.07: Now reads ACTUAL drip state from the User row instead of
    hardcoding step=0. The User table grew drip_step / drip_last_sent_at /
    drip_completed / email_unsubscribed / unsubscribe_token columns in the
    same release. Without these, the auto-firing scheduler couldn't track
    progression and would re-send step 1 forever.
    """
    def __init__(self, user):
        import secrets as _sec
        self._user = user  # Keep a reference for write-back
        self.email              = user.email
        self.source             = getattr(user, 'auth_provider', '') or ''
        # v5.88.49: Populate result_address/result_zip from the user's most
        # recent analyzed property so the step 6+ monthly nearby-listings
        # email can personalize to the right ZIP. Falls through to None
        # if the user has no properties (drip_email_nearby handles that
        # with a generic fallback email).
        latest_addr = None
        latest_zip = None
        try:
            from models import Property
            latest = (Property.query
                      .filter_by(user_id=user.id)
                      .order_by(Property.id.desc())
                      .first())
            if latest:
                latest_addr = latest.address
                # Best-effort ZIP extraction from the address string
                if latest_addr:
                    import re as _re
                    m = _re.search(r'(\d{5})(?:-\d{4})?$', latest_addr) \
                        or _re.search(r'\b(\d{5})\b', latest_addr)
                    if m:
                        latest_zip = m.group(1)
        except Exception as e:
            # Don't let a property-lookup hiccup break the drip pipeline
            logger.warning(f"_UserDripEntry: property lookup failed for user {user.id}: {e}")
        self.result_address     = latest_addr
        self.result_grade       = None
        self.result_exposure    = None
        self.result_score       = None
        self.result_zip         = latest_zip
        # v5.88.07: read from the real User row
        self.drip_step          = getattr(user, 'drip_step', 0) or 0
        self.drip_last_sent_at  = getattr(user, 'drip_last_sent_at', None)
        self.drip_completed     = bool(getattr(user, 'drip_completed', False))
        self.email_unsubscribed = bool(getattr(user, 'email_unsubscribed', False))
        # Use stored token if present, generate one if not
        token = getattr(user, 'unsubscribe_token', None)
        if not token:
            token = _sec.token_urlsafe(32)
        self.unsubscribe_token  = token
        # Match Waitlist's created_at field for the scheduler to compute
        # hours-since-signup
        self.created_at         = user.created_at

    # Make it look writable — only the explicit attrs above can be set
    # (used during construction). After construction, the scheduler should
    # NOT try to mutate this shim; it should write to the underlying user.


def _persist_user_drip_state(user, step, sent_at):
    """v5.88.07: Write drip progression back to the User row after a send."""
    from models import db
    user.drip_step = step
    user.drip_last_sent_at = sent_at
    if step >= MAX_DRIP_STEP:
        user.drip_completed = True
    ...
    try:
        ...
        db.session.rollback()
        logger.error(f"_persist_user_drip_state commit failed for user {user.id}: {e}")
...
def send_user_drip_step(user, step: int) -> bool:
    ...
    v5.88.07: Now persists drip_step + drip_last_sent_at on the User row
    after a successful send so the auto-firing scheduler can progress to
    the next step.

    v5.88.49: Steps 6..MAX_DRIP_STEP now route through drip_email_nearby
    (monthly listings personalized to the user's analyzed-property ZIP).
    Previously this returned False with an 'Invalid drip step N' warning,
    which silently blackholed monthly emails for all users who completed
    steps 1-5. This brought the system into parity with the waitlist
    drip's send_drip_email() which has handled step 6+ since v5.55.
    ...
    # v5.88.49: Determine template to use — same branching as send_drip_email.
    if step in DRIP_TEMPLATES:
        # Steps 1-5: fixed nurture templates
        template_fn = DRIP_TEMPLATES[step]
        subject, html = template_fn(entry)
        ...
        # Steps 6+: monthly nearby-listings email. drip_email_nearby
        # handles the no-ZIP / no-listings fallback paths internally,
        # so we always get a valid (subject, html) pair back.
        subject, html = drip_email_nearby(entry)
        if subject is None:
            # Belt-and-suspenders: drip_email_nearby's documented fallback
            # always returns a subject, but if a future code path returns
            # None we skip silently rather than crash.
            logger.info(f"User drip step {step} skipped (nearby returned None): {user.email}")
            return False
        email_type = 'user_drip_monthly'
    else:
        # Beyond MAX_DRIP_STEP (~1 year). Don't send and don't advance.
        logger.info(f"User drip step {step} exceeds MAX_DRIP_STEP ({MAX_DRIP_STEP}) for {user.email}")
        return False
    ...
    logger.info(f"User drip step {step} {'sent' if success else 'FAILED'}: {user.email}" +
                (" [Monthly listings]" if step > 5 else ""))

    # v5.88.07: persist progression so the auto-scheduler can advance
    if success:
        _persist_user_drip_state(user, step=step, sent_at=datetime.now(timezone.utc))
...
def run_user_drip_scheduler(db_session, batch_size=50):
    ...
    Same time-window logic per step, same batch size, same skip rules.

    Excludes:
      - Users with email_unsubscribed=True
      - Users with drip_completed=True
      - Internal Francis accounts (founder)
      - Likely-bot emails (test/, mailinator, etc.)
      - Users who haven't waited the minimum hours since their last drip

    Returns: dict with counts (sent, skipped, errors, checked).
    """
    ...
    for user in candidates:
        try:
            # Skip exclusions and bots
            if user.email in EXCLUDE:
                stats['skipped'] += 1
                continue
            ...
            hours_since_signup = (now - created).total_seconds() / 3600
            current_step = user.drip_step or 0
            next_step = current_step + 1
            ...
            # For step 1, also require at least 5 minutes since signup
            # so we don't email mid-session
            if next_step == 1 and hours_since_signup < 0.083:
                stats['skipped'] += 1
                continue

            # Don't double-send: enforce min gap since last drip.
            # v5.88.49: matches run_drip_scheduler — 7 days for monthly
            # listing emails (steps 6+), 12h for nurture (steps 1-5).
            # Previously this was 12h for all steps, which would have
            # allowed monthly emails to fire more often than intended
            # the moment step 6+ started working.
            if user.drip_last_sent_at:
                last_sent = user.drip_last_sent_at
                if last_sent.tzinfo is None:
                    last_sent = last_sent.replace(tzinfo=timezone.utc)
                hours_since_last = (now - last_sent).total_seconds() / 3600
                min_gap = 168 if next_step > 5 else 12
            ...
            # Send. send_user_drip_step persists state on success.
            success = send_user_drip_step(user, next_step)
            if success:
                stats['sent'] += 1
            else:
                stats['errors'] += 1
            ...
            logger.error(f"User drip scheduler error for user {user.email}: {e}")
            stats['errors'] += 1
