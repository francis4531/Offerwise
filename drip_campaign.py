...
def drip_email_1(entry):
    ...
    content = f'''
        ...
        {_button("Get Your Free Analysis &rarr;", f"{BASE_URL}/login?signup")}
    '''
...
def drip_email_2(entry):
    """Day 2: Education — what California sellers skip on the TDS."""
    unsub_url = get_unsubscribe_url(entry.unsubscribe_token)

    content = f'''
        <h2 style="margin:0 0 16px 0;color:#f8fafc;font-size:24px;font-weight:700;">
            What California sellers are legally required to disclose (and what they skip)
        </h2>
        <p style="margin:0 0 20px 0;color:#cbd5e1;font-size:16px;line-height:1.6;">
            California's Transfer Disclosure Statement (TDS) has 11 sections and over 100 questions.
        ...
        {_button("Check a Disclosure &rarr;", f"{BASE_URL}/truth-check")}
    '''

    subject = "The 3 TDS sections California sellers skip most often"
    return subject, _wrap(content, "California sellers skip these 3 sections more than any others.", unsub_url)
...
def drip_email_3(entry):
    ...
    content = f'''
        ...
        {_button("View Sample Report &rarr;", f"{BASE_URL}/sample-analysis")}
    '''
...
def drip_email_4(entry):
    """Day 9: Urgency — your free credit expires in 7 days."""
    ...
    content = f'''
        <h2 style="margin:0 0 16px 0;color:#f8fafc;font-size:24px;font-weight:700;">
            Your free analysis credit expires in 7 days
        </h2>
        <p style="margin:0 0 20px 0;color:#cbd5e1;font-size:16px;line-height:1.6;">
            When you signed up, you received one free analysis credit.
            It expires in 7 days.{address_ref}
        </p>
        <p style="margin:0 0 20px 0;color:#cbd5e1;font-size:16px;line-height:1.6;">
            Upload a seller's disclosure and inspection report, and you'll get:
        </p>

        <table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="margin:20px 0;">
            <tr>
                <td style="padding:8px 0;color:#cbd5e1;font-size:15px;">
                    &#x2705; <strong style="color:#f8fafc;">OfferScore&trade;</strong> — property quality rating (0-100)
                </td>
            </tr>
            <tr>
                <td style="padding:8px 0;color:#cbd5e1;font-size:15px;">
                    &#x2705; <strong style="color:#f8fafc;">Property Risk DNA&trade;</strong> — 64 risk dimensions analyzed
                </td>
            </tr>
            <tr>
                <td style="padding:8px 0;color:#cbd5e1;font-size:15px;">
                    &#x2705; <strong style="color:#f8fafc;">Seller Transparency Report&trade;</strong> — contradictions exposed
                </td>
            </tr>
            <tr>
                <td style="padding:8px 0;color:#cbd5e1;font-size:15px;">
                    &#x2705; <strong style="color:#f8fafc;">Recommended Offer Price</strong> — data-backed, not a guess
                </td>
            </tr>
            <tr>
                <td style="padding:8px 0;color:#cbd5e1;font-size:15px;">
                    &#x2705; <strong style="color:#f8fafc;">Negotiation Toolkit</strong> — talking points for your agent
                </td>
            </tr>
        </table>

        {_button("Use Your Free Credit &rarr;", f"{BASE_URL}/login?signup", "#10b981")}

        <p style="margin:20px 0 0 0;color:#94a3b8;font-size:14px;text-align:center;">
            No credit card required. Credit expires {(datetime.now(timezone.utc) + timedelta(days=7)).strftime('%B %d, %Y')}.
        </p>
    '''

    subject = "Your free analysis credit expires in 7 days"
    return subject, _wrap(content, "Your free OfferWise credit expires soon.", unsub_url)


def drip_email_5(entry):
    """Day 14: Last chance — credit expires tomorrow."""
    unsub_url = get_unsubscribe_url(entry.unsubscribe_token)

    content = f'''
        <h2 style="margin:0 0 16px 0;color:#f8fafc;font-size:24px;font-weight:700;">
            Last chance: your free credit expires tomorrow
        </h2>
        <p style="margin:0 0 20px 0;color:#cbd5e1;font-size:16px;line-height:1.6;">
            You have one free analysis credit that expires tomorrow.
            After that, analyses start at $9.99 each.
        </p>
        <p style="margin:0 0 20px 0;color:#cbd5e1;font-size:16px;line-height:1.6;">
            If you're actively looking at a property, now's the time. Upload the seller's disclosure
            and inspection report — you'll have your recommended offer price in under 60 seconds.
        </p>

        {_button("Use It Before It's Gone &rarr;", f"{BASE_URL}/login?signup", "#ef4444")}

        <p style="margin:24px 0 0 0;color:#94a3b8;font-size:14px;text-align:center;">
            Not actively house hunting right now? No worries. We'll keep you on the list
            and send occasional market insights. You can always
            <a href="{unsub_url}" style="color:#60a5fa;">unsubscribe</a>.
        </p>
    '''

    subject = "Last chance — your free credit expires tomorrow"
    return subject, _wrap(content, "Your free OfferWise analysis credit expires tomorrow.", unsub_url)
...
def drip_email_nearby(entry):
    ...
    if not zip_code:
        ...
        content = """
            ...
            %s
        """ % _button("Analyze a Property &rarr;", "%s/app" % BASE_URL)
...
<div style="max-width:600px;margin:0 auto;background:white;border-radius:12px;overflow:hidden;margin-top:20px;margin-bottom:20px;">
    ...
    <div style="padding:32px 36px;color:#1e293b;">
        ...
        {sections_html}
        <a href="https://www.getofferwise.ai/dashboard" style="display:block;text-align:center;padding:14px 24px;background:linear-gradient(135deg,#3b82f6,#2563eb);color:white;border-radius:8px;text-decoration:none;font-weight:700;font-size:14px;margin:24px auto 0;max-width:280px;">View Your Dashboard →</a>
    </div>
