...
class ArcGISCrawler(BaseIngestionJob):
    ...
    def _fetch_page(self, url: str, params: dict) -> Optional[dict]:
        ...
        """
        # v5.89.34: abort check at the start. See SocrataCrawler._fetch_page
        # for rationale.
...
class PortlandCodeViolationsCrawler(ArcGISCrawler):
    """Portland, OR Bureau of Development Services code enforcement violations.

    SCAFFOLD added v5.89.27 — best-faith candidate but unverified.

    Background:
      Portland (city + Multnomah County) publishes development services data
      via ArcGIS Hub. The Bureau of Development Services (BDS) is the code
      enforcement authority. PortlandMaps is the public GIS interface, but
      the underlying ArcGIS REST services are accessible directly.

      Pacific Northwest representation matters for the corpus: distinct
      construction patterns (old-growth timber framing, post-and-beam,
      basement waterproofing for high water table), seismic retrofit
      requirements, and the climate's effect on building envelopes
      (continuous moisture, moss, ice damming in winters). None of which
      shows up in NYC/Chicago/Seattle-WA (Seattle WA is climate-similar
      but Portland OR has distinct municipal code conventions).

    Candidate service URL (UNVERIFIED — verify before activating):
      SERVICE_URL: https://www.portlandmaps.com/arcgis/rest/services/
                   Public/BDS_Permitting/MapServer/<LAYER_ID>

      The layer ID is the unknown. Likely candidates by name pattern:
      Code Enforcement Cases, Code Compliance, Property Violations, or
      Active Cases. The verifier output's "sample fields" will identify
      which layer holds violation text vs which holds permit records.

    Alternative source if the above 404s:
      services.arcgis.com/quVN97tn06YNGj9s/arcgis/rest/services/
        (Multnomah County's ArcGIS Hub may host code data at the county
         level if city-level isn't exposed publicly)

    What text quality to expect:
      ArcGIS-published code data is typically structured (case_type +
      description) similar to Detroit/Columbus quality. Whether Portland's
      narrative is rich (Seattle-tier) or short-coded (Detroit-tier)
      depends on the field schema — verifier output will tell us.

    Activation checklist:
      [ ] Find correct LAYER_ID on portlandmaps.com BDS MapServer
      [ ] Confirm TEXT_FIELD matches the actual response shape (candidate
          field names: CASE_DESC, VIOLATION_DESC, CASE_TYPE, COMMENTS)
      [ ] Run verifier from /api/admin/ml-crawl-verify
      [ ] If row count > 1000 and quality acceptable: flip STATUS='active'
      [ ] Update CRAWLER_REGISTRY tuple label and Coverage Tracker row
          (Portland is currently row #25 with ✗ missing status)
    """
    SOURCE_NAME = 'portland_code_v1'
    # CANDIDATE — layer ID needs verification. The MapServer path itself is
    # standard for Portland's BDS-published GIS; only the trailing layer
    # number is unverified.
    SERVICE_URL = 'https://www.portlandmaps.com/arcgis/rest/services/Public/BDS_Permitting/MapServer/0'
    TEXT_FIELD = 'CASE_DESC'  # candidate field name
    STATE_CODE = 'OR'
    # Loose filter: drop empty/null text. We don't know the exact field name
    # for case-status or date until verification; per-row date filtering can
    # be applied post-fetch once schema is confirmed.
    RELEVANT_WHERE_CLAUSE = "CASE_DESC IS NOT NULL AND CASE_DESC <> ''"
    MAX_ROWS = 10000
    PAGE_SIZE = 1000
