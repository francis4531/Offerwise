...
class BaseIngestionJob:
    ...
    def __init__(self, config: Optional[dict] = None):
        ...
        # v5.89.34: cooperative-cancellation flag. abort_registry.request_abort()
        # sets this to True; _maybe_flush_progress checks it on every flush
        # interval (~5s) and raises CrawlAborted if set. Allows external code
        # to terminate a stuck crawl without process termination.
    ...
    def _maybe_flush_progress(self) -> None:
        ...
        v5.89.34: also checks the cooperative-cancellation flag and raises
        CrawlAborted if set. This is the primary abort check point —
        crawlers call this between page-fetches during pagination, which
        is the natural pause point where it's safe to abort cleanly.
        """
        # v5.89.34: abort check happens BEFORE the flush. If we're being
        # killed, no point spending a round-trip writing progress that's
        # about to be overridden to 'failed' anyway.
        if self._abort_requested:
            from ml_ingestion.abort_registry import CrawlAborted
            raise CrawlAborted(
                f'aborted by watcher (rows_added={self._rows_added}, '
