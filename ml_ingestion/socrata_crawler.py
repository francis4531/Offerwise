...
class SocrataCrawler(BaseIngestionJob):
    ...
    def _fetch_page(self, url: str, params: dict) -> Optional[list]:
        ...
        # v5.89.34: abort check at the start. Without this, the abort
        # signal could be delayed by up to ~94s (3 retries × ~30s timeout
        # + exponential backoff) before the crawler returns to its outer
        # _log() loop where the check fires.
        if self._abort_requested:
            from ml_ingestion.abort_registry import CrawlAborted
            raise CrawlAborted(f'aborted before fetch (rows_added={self._rows_added})')
        for attempt in range(self.MAX_RETRIES):
            try:
                ...
                if resp.status_code in (429, 503):
                    ...
                    time.sleep(delay)
                    # v5.89.34: re-check abort between retries
                    if self._abort_requested:
                        from ml_ingestion.abort_registry import CrawlAborted
                        raise CrawlAborted(f'aborted between retries (rows_added={self._rows_added})')
                    continue
...
class HoustonCodeEnforcementCrawler(SocrataCrawler):
    """Houston, TX code enforcement / 311 service requests.

    SCAFFOLD added v5.89.27 — high-confidence candidate but unverified.

    Background:
      Houston operates an open data portal at data.houstontx.gov. The portal
      is Socrata-powered (SODA 2.x). The most likely violation/code-enforcement
      dataset is the 311 Service Requests, filtered to code-related case
      types: nuisance, dangerous building, weed/grass, junk vehicle, illegal
      dumping.

      Texas representation matters for the corpus: clay soil foundation
      issues, hurricane retrofit, gulf coast humidity damage. Houston's
      data complements Austin's (also TX) which is short-coded — Houston
      may carry richer narrative if the 311 description field is populated
      by inspector text rather than caller text.

    Candidate dataset (UNVERIFIED — verify before activating):
      DOMAIN:     data.houstontx.gov
      DATASET_ID: 4cse-mzh4   ← best guess based on Houston catalog pattern
                              (311 Service Requests). Other live candidates:
                              7jb3-r9h7 (older 311 archive),
                              wxqa-44q4 (Code Enforcement Cases).

    What text quality to expect (best case):
      311-based crawlers typically have short categorical descriptions
      ("Dangerous Building", "Grass/Weeds Over 9 Inches"). Comparable to
      Austin/Detroit short-code quality, not narrative-rich like NYC HPD.

    What to expect on the verifier:
      - If 4cse-mzh4 returns 404: catalog-search data.houstontx.gov for
        "311 Service Requests" or "Code Enforcement" and update DATASET_ID
      - If it returns rows but text field is wrong: check field names via
        the verifier's "sample fields" output; likely candidates are
        `service_request_description`, `sr_type`, or `descript`
      - If 311 has been replaced by Salesforce Public Sector Solutions:
        Houston moved that direction in late 2024 per public RFP records.
        If so, the open data portal may still expose a Socrata mirror,
        or this becomes a scaffold-without-endpoint case.

    Activation checklist:
      [ ] Verify DATASET_ID at data.houstontx.gov
      [ ] Confirm TEXT_FIELD matches the actual response shape
      [ ] Run verifier from /api/admin/ml-crawl-verify and sanity-check sample
      [ ] If row count > 1000 and text quality is acceptable: flip STATUS='active'
      [ ] Update CRAWLER_REGISTRY tuple label to '· active' and update
          architecture page Coverage Tracker row for Houston (currently row
          #5 with ✗ missing status — flip to ✓ active or ⚠ scaffold)
    """
    SOURCE_NAME = 'houston_code_v1'
    DOMAIN = 'data.houstontx.gov'
    DATASET_ID = '4cse-mzh4'  # CANDIDATE — verify before activating
    TEXT_FIELD = 'service_request_description'  # candidate field name
    STATE_CODE = 'TX'
    # Filter to code-enforcement-relevant case types. The 311 dataset includes
    # all service request types (potholes, missed garbage, etc.) — we only
    # want building/property/code records. Field name `sr_type` is the most
    # common Socrata convention for service-request type categorization;
    # verify and adjust if the actual schema differs.
    RELEVANT_WHERE_CLAUSE = (
        "service_request_description IS NOT NULL "
        "AND (UPPER(sr_type) LIKE '%CODE%' "
        "  OR UPPER(sr_type) LIKE '%BUILDING%' "
        "  OR UPPER(sr_type) LIKE '%NUISANCE%' "
        "  OR UPPER(sr_type) LIKE '%DANGEROUS%')"
    )
    MAX_ROWS = 10000
    STATUS = 'scaffold'  # v5.89.27: pending operator verification
...
CRAWLER_REGISTRY = [
    ...
    # v5.89.27: new scaffolds — high-confidence candidate endpoints,
    # pending operator verification via /api/admin/ml-crawl-verify.
    ('houston',       'Houston Code Enforcement (TX · Socrata · candidate endpoint, unverified) · scaffold', HoustonCodeEnforcementCrawler),
    ('portland',      'Portland BDS Code Violations (OR · ArcGIS · candidate layer, unverified) · scaffold', PortlandCodeViolationsCrawler),
]
