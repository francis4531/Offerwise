...
class StateDisclosureContext:
    ...
    statute_citation: str = ''      # governing statute (UNVERIFIED — pointer only)
    as_is_common: bool = False      # market practice: AS-IS/optional form common here
...
_MODERATE = {
    ...
    'WV', 'VT', 'KS', 'AR', 'ND', 'AK', 'DC', 'MS',
}
_CAVEAT_EMPTOR = {'AL', 'WY'}
# NOTE (v5.89.120): MS RECLASSIFIED caveat_emptor → moderate, VERIFIED against
# primary source on 2026-06-01. Mississippi is NOT caveat emptor for residential
# disclosure: Miss. Code §§ 89-1-501 to 89-1-527 require the seller of a 1–4 unit
# residence to deliver a Property Condition Disclosure Statement (PCDS, MREC form,
# § 89-1-509) and to disclose known material defects in good faith (§ 89-1-511);
# late/post-offer delivery gives the buyer a 3-day (5 by mail) right to terminate
# (§ 89-1-503). Placed in MODERATE not COMPREHENSIVE because the statutory duty
# attaches when a licensed broker/salesperson is involved — a true FSBO may not
# trigger the form (though common-law duties on known defects still bar active
# concealment) — and §§ 89-1-501(2) plus the "no occupancy + no knowledge" path
# carve out exemptions. Sources: Miss. Code text (Justia/FindLaw) + MREC PCDS form.
# Statute amended 2024 (Laws 2024, ch. 423). NOTE: AL and WY remain caveat_emptor
# and were NOT independently re-verified in this pass — both are plausibly correct
# (AL caveat emptor for used residential; WY no mandatory form) but unconfirmed.

# Market-practice signal (NOT a legal classification): states where "AS-IS" sales
# or optional/agent-only disclosure forms are common. Sourced from eForms' "Buyer
# Beware" index (a form-vendor reference, cross-checked below), which conflates
# "no mandatory form" with "no disclosure duty" — so this is kept SEPARATE from the
# legal _CAVEAT_EMPTOR set above. Most of these states DO impose a disclosure duty
# for known material defects (e.g. FL via Johnson v. Davis) even where a form is
# optional. Use this only to soften "missing disclosure = red flag" framing, never
# to assert the seller had no legal duty.
#   UNVERIFIED — confirm each against the primary statute before legal reliance.
_AS_IS_COMMON = {'AL', 'AR', 'CO', 'FL', 'IN', 'MA', 'MO', 'MT', 'NH', 'NJ', 'VA', 'WV', 'WY'}

# State disclosure statute citations. Sourced from the eForms property-disclosure
# index (secondary source) and cross-checked against general legal knowledge.
#   UNVERIFIED for legal reliance — statutes are amended; confirm current text and
#   citation against the official state code before presenting as authoritative.
# Used to point users to the governing law, NOT to interpret it.
_STATE_STATUTES = {
    'AK': 'AS 34.70.010', 'AZ': 'Hill v. Jones, 725 P.2d 1115',
    'CA': 'Cal. Civ. Code § 1102 (+ Natural Hazard Disclosure)', 'CO': 'C.R.S. § 38-35.7',
    'CT': 'Conn. Gen. Stat. § 20-327b', 'DE': 'Del. Code tit. 6 § 2577',
    'FL': 'Johnson v. Davis, 480 So.2d 625', 'GA': 'O.C.G.A. § 44-1-16',
    'HI': 'Haw. Rev. Stat. § 508D-5', 'ID': 'Idaho Code § 55-2508',
    'IL': '765 ILCS 77/35 (+ radon 765 ILCS 77/20)', 'IN': 'Ind. Code § 32-21-5',
    'IA': 'Iowa Code § 558A.2', 'KS': 'Kan. Stat. § 58-30,106',
    'KY': 'Ky. Rev. Stat. § 324.360', 'LA': 'La. Rev. Stat. § 9:3198',
    'ME': 'Me. Rev. Stat. tit. 33 § 173', 'MD': 'Md. Code, Real Prop. § 10-702',
    'MA': 'Mass. Gen. Laws ch. 15 § 301 / 197A', 'MI': 'Mich. Comp. Laws § 565.957',
    'MN': 'Minn. Stat. § 513.55', 'MS': 'Miss. Code §§ 89-1-501 to 89-1-527 (PCDS; form § 89-1-509)',
    'MO': 'Mo. Rev. Stat. § 442.606 (Form DSC-8000)', 'MT': 'Mont. Code § 37-51-313',
    'NE': 'Neb. Rev. Stat. § 76-2,120', 'NV': 'Nev. Rev. Stat. § 113.130',
    'NH': 'N.H. Rev. Stat. § 477:4-d', 'NJ': 'N.J.S.A. § 46:3C-10',
    'NM': 'N.M. Stat. § 47-13-4', 'NY': 'N.Y. Real Prop. Law § 462',
    'NC': 'N.C. Gen. Stat. § 47E-4(b)', 'ND': 'N.D. Cent. Code § 43-4-44',
    'OH': 'Ohio Rev. Code § 5302.30', 'OK': 'Okla. Stat. tit. 60 § 833',
    'OR': 'Or. Rev. Stat. § 105.464', 'PA': '68 Pa. Cons. Stat. § 7304',
    'RI': 'R.I. Gen. Laws § 5-20.8-2', 'SC': 'S.C. Code § 27-50-40',
    'SD': 'S.D. Codified Laws § 43-4-38', 'TN': 'Tenn. Code § 66-5-202(1)',
    'TX': 'Tex. Prop. Code § 5.008', 'UT': 'Utah Admin. R162-2f-401a (agents)',
    'VT': 'Vt. Stat. tit. 27 § 2296(4)', 'VA': 'Va. Code § 55.1-703 (formerly § 55-519/520)',
    'WA': 'Wash. Rev. Code § 64.06.020/.030', 'DC': 'D.C. Code § 42-1302/1305',
    'WV': 'W. Va. (disclosure optional)', 'WI': 'Wis. Stat. § 709.01(2)',
    'WY': 'Wyo. Stat. § 33-28-303(c) (agents)',
    'AL': 'Ala. (disclosure optional; caveat emptor tradition)',
    'AR': 'Ark. (disclosure optional)',
}
...
def get_state_context(state_code: str) -> StateDisclosureContext:
    ...
    if state_code == 'FL':
        ...
        notes.append('Mississippi requires a Property Condition Disclosure Statement (PCDS) when a licensed real estate agent is involved in the sale. A for-sale-by-owner with no agent may not trigger the statutory form, but the seller still cannot conceal known material defects. If a disclosure arrives after your offer, you have a limited right to terminate (3 days in person, 5 by mail). A sparse or missing PCDS in an agent-assisted sale is a reason to ask questions, not the expected norm.')

    # Market-practice note: where AS-IS / optional disclosure is common, soften
    # the "missing disclosure = red flag" framing — but do NOT imply no legal duty.
    if state_code in _AS_IS_COMMON and state_code not in _CAVEAT_EMPTOR:
        notes.append(
            f'AS-IS sales and optional/limited disclosure forms are common in {name}. '
            f'A sparse disclosure here is not unusual and does not by itself indicate '
            f'concealment — though most states still require disclosure of known '
            f'material defects. Verify the seller\'s specific obligations.'
        )
    ...
    return StateDisclosureContext(
        ...
        statute_citation=_STATE_STATUTES.get(state_code, ''),
        as_is_common=(state_code in _AS_IS_COMMON),
