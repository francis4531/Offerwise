...
class CostValidator:
    ...
    def validate_total_costs(repair_costs: float, property_price: float) -> float:
        """Validate total repair costs are reasonable. Clamps rather than raises
        so analysis always completes — extreme values are flagged upstream."""
        if repair_costs < 0:
            raise ValidationError("Negative repair costs")

        # Repair costs exceeding property value = teardown territory.
        # Cap at property value and continue; caller should flag walk-away.
        if property_price > 0 and repair_costs > property_price:
            logging.warning(
                f"Repair costs ${repair_costs:,.0f} exceed property value "
                f"${property_price:,.0f} — capping at property value"
            )
            repair_costs = property_price
...
class OfferValidator:
    ...
    def validate_discount(discount: float, asking_price: float) -> float:
        ...
        if discount < 0:
            logging.warning(f"Negative discount {discount} corrected to 0")
            discount = 0.0
    ...
    def validate_offer(offer: float, asking_price: float, discount: float) -> float:
        ...
        # Only recalculate if discount is explicitly provided and offer is implausibly
        # far from asking_price - discount. If discount is 0 (not provided), trust the offer.
        if discount > 0 and abs(offer - expected_offer) > asking_price * 0.30:
            # Offer deviates >30% from what discount math implies — recalculate
...
class CategoryValidator:
    ...
    def validate_categories(category_scores: List[Dict]) -> List[Dict]:
        ...
        for cat_score in category_scores:
            ...
            # Clamp negative scores — they indicate a calculation error
            if score is not None and score < 0:
                logging.warning(f"Negative category score {score} for {cat_score.get('name','?')} clamped to 0")
                cat_score['score'] = 0
                score = 0
...
def validate_analysis_output(analysis: Dict) -> Dict:
    ...
    try:
        ...
        # Validate risk score — handle both top-level and nested structures
        ...
        # Also clamp nested risk_score dict (new analysis format)
        if 'risk_score' in validated and isinstance(validated['risk_score'], dict):
            rs = validated['risk_score']
            if 'overall_risk_score' in rs:
                rs['overall_risk_score'] = RiskScoreValidator.validate_risk_score(
