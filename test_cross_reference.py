...
# ============================================================
# GROUP 4: NO-DISCLOSURE / INSPECTION-ONLY (v5.89.121)
# ============================================================

class TestNoDisclosureProvided:
    """When only an inspection is uploaded, transparency must NOT be judged.

    Regression guard for the inspection-only path. Previously a missing
    disclosure was treated as bank-owned/foreclosure: transparency 25, risk 75,
    every finding marked 'undisclosed', and 'exercise extreme caution' — which is
    false and defamatory when the buyer simply did not upload a disclosure.
    """

    def test_inspection_only_transparency_not_applicable(self, cross_ref, parser):
        disclosure = parser.parse_seller_disclosure("")  # nothing provided
        inspection = parser.parse_inspection_report(
            "Active water penetration observed in basement. "
            "Possible foundation movement at unlevel floor. "
            "Furnace blower safety switch did not work."
        )
        report = cross_ref.cross_reference(disclosure, inspection)
        assert report.transparency_applicable is False
        # neutral sentinel: trips neither the <50 discount nor the <30 caution
        assert report.transparency_score == 50.0
        # findings are NOT framed as concealed when there's nothing to conceal against
        assert len(report.undisclosed_issues) == 0
        # risk still reflects the actual inspection findings
        assert report.risk_score > 0
        assert "exercise extreme caution" not in report.summary.lower()

    def test_bank_owned_still_flags_caution(self, cross_ref, parser):
        disclosure = parser.parse_seller_disclosure(
            "Property sold AS-IS. Bank-owned REO property. No disclosure provided."
        )
        inspection = parser.parse_inspection_report(
            "Active water penetration observed. Cracked roof covering."
        )
        report = cross_ref.cross_reference(disclosure, inspection)
        # bank-owned remains the affirmative-no-disclosure caution path
        assert report.transparency_applicable is True
        assert report.transparency_score == 25
