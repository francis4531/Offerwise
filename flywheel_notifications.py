...
def send_agent_postclose_email(
    ...
    try:
        ...
        # Record the close.
        # v5.87.40: wrapped in try/except so the email still goes out even if
        # the database hasn't run the migration that adds these columns yet
        # (e.g. someone runs the function manually before deploy completes).
        # The migration is in alembic/versions/b3e8a2f5c9d1_*; under normal
        # deploy order, hasattr checks always succeed.
        try:
            if hasattr(share, 'deal_closed_at'):
                share.deal_closed_at = datetime.utcnow()
            if final_sale_price and hasattr(share, 'final_sale_price'):
                share.final_sale_price = final_sale_price
            db.session.commit()
        except Exception as _persist_err:
            logger.warning(f"Could not persist agent share close fields (non-fatal): {_persist_err}")
            try:
                db.session.rollback()
            except Exception:
                pass
...
def _send_lead_to_contractor(lead, contractor, claim) -> bool:
    ...
    v5.87.40: Refactored to derive scope from ContractorLead's actual fields
    (repair_system, cost_estimate, issue_description, severity) rather than
    a non-existent JSON column. The original implementation would have
    raised AttributeError on every call — this function never actually ran
    in production.

    The structured scope card complements the inline claim email (which
    delivers buyer contact info) by giving the contractor an at-a-glance
    severity + cost frame for what they're being asked to bid on.
    """
    try:
        address = lead.property_address or 'Property'

        # Derive a single-system "scope row" from the lead's actual fields.
        # This is the single repair the lead was created for; multi-system
        # support can be re-added later if leads ever carry richer scope data.
        repair_label = (lead.repair_system or 'General work').replace('_', ' ').title()
        cost_str = (lead.cost_estimate or '').strip()
        # Default severity to "major" — leads created from buyer analyses are
        # almost always for major+ findings (cosmetic issues don't trigger the
        # repair-system → contractor-lead pipeline).
        severity = 'major'
        sev_color = '#ef4444' if severity == 'critical' else '#fbbf24'
        sev_label = severity.upper()

        scope_html = f"""
            <tr>
              <td style="padding:10px 12px;font-size:13px;font-weight:600;color:#e2e8f0;">{repair_label}</td>
              ...
              <td style="padding:10px 12px;font-size:13px;font-weight:700;color:#e879f9;text-align:right;">{cost_str or '—'}</td>
            </tr>"""

        # Optional issue detail row (when buyer described the issue)
        issue_html = ''
        if lead.issue_description and lead.issue_description.strip():
            issue_text = lead.issue_description.strip()[:240]
            issue_html = f"""
            <tr>
              <td colspan="3" style="padding:10px 12px;font-size:12px;color:#94a3b8;line-height:1.6;border-top:1px dashed rgba(255,255,255,0.05);">
                <strong style="color:#cbd5e1;">Buyer said:</strong> {issue_text}
              </td>
            </tr>"""

        # Lead quality score: starts at 80, +5 if cost range provided,
        # +5 if issue described, +5 if timing is asap or this_week.
        quality = 80
        if cost_str:
            quality += 5
        if lead.issue_description:
            quality += 5
        if lead.contact_timing in ('asap', 'this_week'):
            quality += 5
        quality = min(quality, 99)

        # Use lead.expires_at as the effective close date — it's the marketplace
        # window, not the deal close, but it's the relevant deadline for the contractor.
        close_date = lead.expires_at.strftime('%b %d') if getattr(lead, 'expires_at', None) else 'Soon'
        portal_url = f"https://www.getofferwise.ai/contractor-portal"

        subject = f"⚡ Lead matched: {repair_label} at {address[:35]}"
  ...
  <div style="background:linear-gradient(160deg,#160d1a,#111118);border:1px solid rgba(232,121,249,0.2);border-radius:16px;overflow:hidden;margin-bottom:16px;">
    ...
    <div style="padding:22px 24px;display:flex;justify-content:space-between;align-items:flex-start;gap:12px;">
      <div>
        ...
        <div style="font-size:12px;color:#64748b;">Buyer is active · Marketplace closes {close_date}</div>
      </div>
...
def _update_estimate_from_completion(completion) -> None:
    ...
    v5.87.40: Refactored to produce structured, predictor-usable output:

    - work_completed parsed into known repair categories (electrical / plumbing /
      hvac / roof / foundation / etc.) using a keyword-matching pass. Falls
      back to 'general' if the contractor wrote freeform text we can't categorize.
    - metro_name populated from RepairCostZone via 3-digit ZIP-prefix lookup.
      Without this, the predictor can't bucket completions by region.
    - cost_multiplier read from the same zone (defaults to 1.0 if zone missing).
    - total_low/total_high replaced with a calibrated band: ±15% if a single
      system, ±25% if multi-system (the wider variance reflects scope ambiguity).
    - breakdown_json carries per-system rows so the predictor can train on
      (category, severity, ZIP, region, actual_price) tuples — the shape it
      actually needs. Each row also records the contractor's allocated portion
      of the total price.

    The previous implementation wrote a single `something_cost_X` row with
    cost_multiplier=1.0 and metro_name='', which the predictor couldn't learn
    from. That was capturing data without lifting the model. This is the same
    write path producing structured ground truth.
    """
    try:
        if not (completion.won_job and completion.final_price and completion.zip_code):
            return

        # Parse work_completed into categorized systems.
        # work_completed is a freeform string from the contractor's submission;
        # the keyword map below covers the categories the predictor cares about.
        # Anything we can't categorize lands in 'general' — better than dropping
        # the row entirely.
        CATEGORY_KEYWORDS = {
            'electrical':  ['electrical', 'electric', 'wiring', 'panel', 'outlet', 'breaker'],
            'plumbing':    ['plumbing', 'pipe', 'leak', 'water heater', 'drain', 'sewer', 'faucet', 'toilet'],
            'hvac':        ['hvac', 'furnace', 'air condition', 'ac unit', 'heat pump', 'duct'],
            'roof':        ['roof', 'shingle', 'gutter', 'flashing', 'attic'],
            'foundation':  ['foundation', 'crack', 'settling', 'basement', 'footing', 'pier'],
            'environmental': ['mold', 'asbestos', 'lead', 'radon', 'termite', 'pest'],
            'structural':  ['structural', 'beam', 'load bearing', 'wall', 'support'],
            'exterior':    ['siding', 'window', 'door', 'paint', 'exterior'],
            'interior':    ['flooring', 'drywall', 'ceiling', 'tile', 'trim', 'kitchen', 'bathroom'],
        }

        raw_systems = [s.strip().lower() for s in (completion.work_completed or '').split(',') if s.strip()]
        categorized = []
        for raw in raw_systems:
            ...
            categorized.append({
                'raw_text': raw[:100],
                'category': matched or 'general',
            })

        if not categorized:
            categorized = [{'raw_text': '', 'category': 'general'}]

        n_systems = len(categorized)
        # Allocate the total price evenly across categorized systems.
        # In a future iteration, this could weight by category baseline cost,
        # but even allocation is sufficient ground truth for the predictor's
        # ZIP-level adjustments.
        per_system_price = round(completion.final_price / n_systems, 2)

        # Pull metro/region context from the ZIP-prefix table.
        from models import RepairCostZone, RepairCostLog, db
        zip_5 = completion.zip_code[:5]
        zip_prefix = zip_5[:3] if len(zip_5) >= 3 else ''
        zone = None
        if zip_prefix:
            try:
                ...
                zone = None
        metro_name = (zone.metro_name if zone else '') or ''
        cost_multiplier = float(zone.cost_multiplier) if zone and zone.cost_multiplier else 1.0
        region = (zone.region if zone else '') or ''
        state = (zone.state if zone else '') or ''

        # Calibrate the band: tight for single-system, wider for multi-system
        # to reflect scope ambiguity. The predictor uses these as p10/p90
        # bounds when training.
        band_pct = 0.15 if n_systems == 1 else 0.25
        total_low = round(completion.final_price * (1 - band_pct))
        total_high = round(completion.final_price * (1 + band_pct))

        # Build the structured breakdown the predictor actually needs:
        # one row per system, with category + ZIP + region + actual price.
        breakdown = [
            {
                'category': item['category'],
                'raw_text': item['raw_text'],
                'allocated_price': per_system_price,
                'zip': zip_5,
                'metro': metro_name,
                'region': region,
                'state': state,
        ...
        cats = ', '.join(sorted({b['category'] for b in breakdown}))
        logger.info(
            f"📊 Estimate engine updated from completion: ZIP={zip_5} metro={metro_name or '?'} "
            f"systems=[{cats}] price=${completion.final_price:,.0f} band=[${total_low:,}–${total_high:,}]"
        )
    except Exception as e:
