...
# v5.87.80: Cold outreach uses a personal sender identity, not noreply@.
# Cold outreach goes to people who don't know us — the From line needs
# to look like a person reached out, not a system. Reply-to still routes
# to the founder's Gmail. Override per-deploy via env var if needed.
OUTREACH_FROM_EMAIL = os.environ.get(
    'OUTREACH_FROM_EMAIL', 'Francis Anthony <francis@getofferwise.ai>'
)
SUPPORT_EMAIL = os.environ.get('SUPPORT_EMAIL', 'support@getofferwise.ai')
# v5.88.35: support replies come from a friendly support sender. Reply-To
# stays on this same address so the customer's reply loops back to support
# (and lands in the inbound webhook when v5.88.36 ships).
SUPPORT_FROM_EMAIL = os.environ.get(
    'SUPPORT_FROM_EMAIL', f'OfferWise Support <{SUPPORT_EMAIL}>'
)
        ...
        <table role="presentation" cellspacing="0" cellpadding="0" border="0" width="100%"
          ...
          <tr><td style="padding:16px 20px;">
            ...
            <p style="margin:0 0 10px;color:#94a3b8;font-size:12px;line-height:1.5;">
              OfferWise will draft your formal Request for Repair from these findings — state-aware, clause by clause.
            </p>
...
def send_email(to_email: str, subject: str, html_content: str, reply_to: str = None,
               headers: dict = None, email_type: str = 'other', user_id: int = None,
    ...
    Args:
        ...
        from_email: v5.87.80. Override the default From address. Defaults to
            FROM_EMAIL (transactional, 'OfferWise <noreply@...>'). Cold outreach
            should pass OUTREACH_FROM_EMAIL ('Francis Anthony <francis@...>')
            for a personal sender identity. Same domain, same SPF/DKIM, just
            a different display name + local part.
    ...
    try:
        params = {
            ...
            "subject": subject,
            "html": html_content,
            # v5.88.67: Resend tracking is OPT-IN. Without this, Resend
            # delivers the email but never injects the open-tracking pixel
            # and never rewrites links for click-tracking. Every send from
            # this function (transactional + outreach) had this turned off
            # by default, which is why the outreach engagement dashboard
            # showed 0% opens / 0% clicks across 95 emails. Opens and
            # clicks are now both tracked; engagement events arrive via
            # the Resend webhook at /webhook/resend (handler in app.py),
            # which writes to the EmailEvent table for the dashboard to
            # read. (v5.88.68: corrected the URL in this comment — an
            # earlier note pointed at a dead duplicate handler in
            # waitlist_routes.py that has since been removed.)
            "tracking": {
                "opens": True,
                "clicks": True,
            },
