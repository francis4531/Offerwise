...
class IntegrityTestEngine:
    ...
    def _test_auth_boundaries(self):
        ...
        # CRITICAL: Create a FRESH test client with no cookies/session
        # to avoid inheriting admin context when run from admin dashboard
        with self.app.test_client(use_cookies=False) as client:
            ...
            for method, path, label in [
                ...
                try:
                    ...
                    ok = resp.status_code in (400, 401, 403, 302)
            ...
            for method, path, label in [
                ...
                try:
                    resp = client.get(path) if method == 'GET' else client.post(path, json={})
                    # 401/403 = rejected, 302 = redirect to login, 404 = route not found (also safe)
                    ok = resp.status_code in (401, 403, 302, 404)
