...
class HybridCrossReferenceEngine:
    ...
    def _enhance_matches_with_ai(
        ...
        try:
            ...
            # Enhance each match with AI insights
            for match, analysis in zip(matches, ai_analysis):
    ...
    def _build_ai_analysis_prompt(self, matches: List[Dict], match_type: str) -> str:
        ...
        if match_type == "contradiction":
            context = """You are analyzing CONTRADICTIONS where the seller's disclosure conflicts with the inspector's findings.

Your job: Assess how serious each contradiction is and explain what it means for the buyer."""
        else:  # undisclosed
            context = """You are analyzing UNDISCLOSED ISSUES where the inspector found problems the seller didn't mention.

Your job: Assess how serious each omission is and explain what it means for the buyer."""
...
1. **severity**: "critical" | "major" | "moderate" | "minor"
   - Critical: Safety hazard, structural, or major system failure
   - Major: Significant cost ($5K+) or functionality issue
   - Moderate: Notable concern requiring attention ($1K-5K)
   - Minor: Small issue with low impact (<$1K)

2. **explanation**: 2-3 sentences in plain English explaining:
   - What the discrepancy is
   - Why it matters to the buyer
   - What the financial/safety implications are

3. **ai_confidence**: 0.0-1.0 (your confidence this is a real issue)

4. **deception_indicators**: List any red flags you notice:
   - "minimization" - Seller downplayed severity
   - "vague_language" - Seller was intentionally unclear
   - "omission" - Seller completely failed to mention
   - "timing_suspicious" - Disclosure timing suggests hiding
   - "contradicts_prior" - Conflicts with earlier statements

5. **buyer_impact**: One sentence: "This could cost you $X and affect Y"

6. **recommended_action**: What should the buyer do?
   - "request_credit" | "renegotiate_price" | "specialist_inspection" | 
   - "repair_before_close" | "walk_away" | "monitor_closely"
...
[
  {{
    ...
    "explanation": "...",
    "ai_confidence": 0.85,
    "deception_indicators": ["minimization", "vague_language"],
    "buyer_impact": "...",
    "recommended_action": "request_credit"
  }},
  ...
]"""
