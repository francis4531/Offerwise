...
class TestResultsDataExtraction(unittest.TestCase):
    ...
    def test_deal_breakers_check_explanation(self):
        """Contradiction/deal-breaker rendering should read the 'explanation' field
        (with fallbacks). The frontend accesses it as `c.explanation` on the
        contradiction object, not a `db.` prefix."""
        self.assertIn('c.explanation', self.html,
                       "Contradiction rendering should read the 'explanation' field")
    ...
    def test_transparency_checks_all_data_sources(self):
        """Transparency report should display its sub-scores. The frontend reads
        them off the `transparency` object (omission/minimization/consistency).
        Note: `undisclosed_issues` is computed server-side (cross_reference_engine)
        and is not surfaced as a frontend field, so it is not asserted here."""
        self.assertIn('transparency.omission_score', self.html)
        self.assertIn('transparency.minimization_score', self.html)
        self.assertIn('transparency.consistency_score', self.html)

    def test_boilerplate_filter_present(self):
        """Inspection boilerplate (disclaimers, mold/legal language) must be
        filtered out of findings. This filter moved from the frontend into the
        server-side ML training pipeline, so we assert it there."""
        import os
        path = os.path.join(os.path.dirname(__file__), 'ml_training_pipeline.py')
        with open(path, 'r') as f:
            pipeline = f.read()
        lower = pipeline.lower()
        self.assertIn('boilerplate', lower,
                      "ML pipeline should have a boilerplate filter")
        self.assertIn('not a qualified', lower,
                      "Boilerplate filter should catch the 'not a qualified' disclaimer")
        self.assertIn('mold disclaimer', lower,
                      "Boilerplate filter should catch mold-disclaimer language")

    def test_risk_dna_descriptions_present(self):
        """Risk DNA pentagram should show axis labels for each dimension."""
        # Labels updated in v5.84 — now use short names rendered at runtime
        self.assertIn('Structural', self.html)
        self.assertIn('Systems', self.html)
        self.assertIn('Transparency', self.html)
        self.assertIn('Temporal', self.html)
        self.assertIn('Financial', self.html)
