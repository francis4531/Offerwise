// Fire-and-forget feature event tracker (mirrors owTrack in app.html)
function _trackFeature(feature, action, meta) {
    try {
        var body = { feature: feature, action: action || 'click' };
...
function updateReferralUI(data) {
    ...
    if (data.next_tier) {
        if (data.next_tier.remaining === 0) {
            // Just hit this tier
            if (progressText) progressText.textContent = `🎉 Tier ${data.next_tier.tier} reached!`;
            if (progressBar) progressBar.style.width = '100%';
        } else {
            if (progressText) progressText.textContent = `${data.next_tier.remaining} more referral${data.next_tier.remaining === 1 ? '' : 's'} to reach Tier ${data.next_tier.tier}`;
...
function copyReferralCode(event) {
    ...
    navigator.clipboard.writeText(input.value).then(() => {
        ...
        _trackFeature('referral_code', 'copy', { source: 'settings' });
    }).catch(err => {
...
function copyReferralLink(event) {
    ...
    navigator.clipboard.writeText(input.value).then(() => {
        ...
        _trackFeature('referral_link', 'copy', { source: 'settings' });
    }).catch(err => {
        console.error('Failed to copy:', err);
        alert('Failed to copy. Please select and copy manually.');
    });
...
function shareViaEmailDirect() {
    const emailEl = document.getElementById('referral-email-input');
