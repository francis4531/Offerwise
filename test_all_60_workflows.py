...
class TestContractorW03_Subscribe(unittest.TestCase):
    ...
    def test_starter_sets_10_lead_limit(self):
        from auth_config import PRICING_TIERS
        tier = PRICING_TIERS.get('contractor_starter', {})
        self.assertEqual(tier.get('monthly_lead_limit'), 10)
        self.assertEqual(tier.get('zip_limit'), 3)

    def test_pro_sets_50_lead_limit(self):
        # Pro is a capped tier (50), not unlimited — Enterprise is the unlimited tier.
        from auth_config import PRICING_TIERS
        tier = PRICING_TIERS.get('contractor_pro', {})
        self.assertEqual(tier.get('monthly_lead_limit'), 50)
        self.assertEqual(tier.get('zip_limit'), 10)
