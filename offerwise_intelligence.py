...
def analyze_buyer_concerns_with_ai(buyer_input: str, anthropic_api_key: str) -> Optional[Dict[str, Any]]:
    ...
    try:
        ...
        prompt = f"""Analyze this home buyer's concern and extract detailed, structured information.

BUYER'S CONCERN:
"{buyer_input}"

Please analyze and return ONLY valid JSON (no markdown, no explanation) with this exact structure:

{{
  "primary_concerns": ["system1", "system2"],
  ...
  "has_trust_issue": true|false,
  "has_time_pressure": true|false,
  "emotional_weight": "low|medium|high",
  "specific_details": {{
    "why_they_care": "brief explanation",
    "past_experience": "any mentioned past issues",
    "specific_amounts": "any dollar amounts mentioned",
    "family_context": "kids, health issues, etc",
    "urgency_level": "how urgent is their timeline"
  }},
  "key_priorities": ["priority1", "priority2", "priority3"],
  "recommended_focus": "what should the analysis emphasize"
}}

SYSTEM OPTIONS for primary_concerns: foundation, roof, hvac, mold, electrical, plumbing, pest, safety, health, budget, seller_honesty
...
class OfferWiseIntelligence:
    ...
    def __init__(self):
        ...
        if anthropic_api_key:
            logger.info("🚀 Initializing OPTIMIZED hybrid cross-reference engine (6x faster)")
    ...
    def _generate_offer_strategy(
        ...
        if abs(expected_offer - recommended_offer) > 1.0:  # Allow $1 rounding error
            logging.error("=" * 80)
            logging.error("🚨 OFFER CALCULATION ERROR DETECTED!")
            logging.error(f"property_price: ${property_price:,.0f}")
            logging.error(f"discount_from_ask: ${discount_from_ask:,.0f}")
            logging.error(f"recommended_offer: ${recommended_offer:,.0f}")
            logging.error(f"expected_offer (price - discount): ${expected_offer:,.0f}")
            logging.error(f"MISMATCH: ${abs(expected_offer - recommended_offer):,.0f}")
            logging.error("This should never happen - discount_from_ask must equal (price - offer)")
            logging.error("=" * 80)
        
        # 🔍 VALIDATION: Ensure percentage is reasonable
        if discount_percentage > 90 or discount_percentage < 0:
            logging.error("=" * 80)
            logging.error("🚨 DISCOUNT PERCENTAGE OUT OF RANGE!")
            logging.error(f"discount_percentage: {discount_percentage}%")
            logging.error(f"property_price: ${property_price:,.0f}")
            logging.error(f"recommended_offer: ${recommended_offer:,.0f}")
            logging.error(f"discount_from_ask: ${discount_from_ask:,.0f}")
            logging.error("=" * 80)
        ...
        return {
            ...
            "discount_breakdown": {
                ...
                "buyer_sentiment": sentiment_adjustment,  # NEW: Can be positive (aggressive) or negative (conservative)
                "safety_buffer": safety_adjustment,  # NEW: Always 0 or positive
                "trauma_buffer": buffer_adjustment  # NEW: Always 0 or positive
            },
