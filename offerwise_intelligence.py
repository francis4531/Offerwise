        ...
        if property_address and len(property_address) >= 15:
            try:
                ...
                if research_data and research_data.get('tool_results'):
                    ...
                    # Fallback: direct API calls (only if research agent didn't run)
                    rp = research_data.get('profile', {}) if research_data else {}
                    market_intel = mi_engine.analyze(
                        address=property_address,
                        asking_price=int(property_price),
                        bedrooms=rp.get('bedrooms', 0) or 0,
                        bathrooms=rp.get('bathrooms', 0) or 0,
                        sqft=rp.get('sqft', 0) or 0,
                        property_type=rp.get('property_type', 'Single Family') or 'Single Family',
                    )
