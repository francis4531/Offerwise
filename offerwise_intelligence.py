...
Valid concerns: foundation, roof, hvac, mold, electrical, plumbing, pest, safety, health, budget, seller_honesty"""
        ...
        try:
            ...
            _track(message, "offer-strategy", (time.time() - _t0) * 1000, db=_ow_db, app=_ow_app)
        except Exception:
            pass
...
class PropertyAnalysis:
    ...
    external_verifications: Optional[List[Dict[str, Any]]] = None  # 🤖 Government records vs seller claims
    market_intelligence: Optional[Dict[str, Any]] = None  # 📊 Phase 2: Market context (comps, DOM, trends)
    hybrid_ai: Optional[Dict[str, Any]] = None  # 🧠 AI Intelligence Layer: LLM-enhanced findings, contradictions, cost context
...
class OfferWiseIntelligence:
    ...
    def analyze_property(
        ...
        Args:
            ...
            inspection_report_text: Raw text from inspection report PDF (includes [PHOTO: ...] descriptions from vision extraction)
            ...
            research_data: External research from Property Research Agent (optional)
        ...
        logger.info("🎯 Analyzing buyer concerns...")
        t0 = time.time()
        ...
        logger.info(f"  Disclosure OCR quality: {disclosure_quality:.1%}")
        logger.info(f"  Inspection OCR quality: {inspection_quality:.1%}")
        ...
        if 0.50 <= disclosure_quality < 0.95 and self.ai_helper.enabled:
            logger.info(f"  🔧 Fixing disclosure OCR errors (quality: {disclosure_quality:.1%})...")
            t0 = time.time()
            ...
            if disclosure_quality >= 0.95:
                logger.info(f"  ✅ Disclosure quality excellent ({disclosure_quality:.1%}), skipping OCR fix")
            elif disclosure_quality < 0.50:
                logger.info(f"  ⚠️  Disclosure quality very low ({disclosure_quality:.1%}), likely not OCR errors, skipping")
        
        if 0.50 <= inspection_quality < 0.95 and self.ai_helper.enabled:
            logger.info(f"  🔧 Fixing inspection OCR errors (quality: {inspection_quality:.1%})...")
            t0 = time.time()
            ...
            if inspection_quality >= 0.95:
                logger.info(f"  ✅ Inspection quality excellent ({inspection_quality:.1%}), skipping OCR fix")
            elif inspection_quality < 0.50:
                logger.info(f"  ⚠️  Inspection quality very low ({inspection_quality:.1%}), likely not OCR errors, skipping")
        
        # Step 1: Parse documents into structured data
        logger.info("Parsing seller disclosure...")
        t0 = time.time()
        ...
        # 🧠 ML HYBRID ROUTING: Use trained model for high-confidence category overrides.
        #
        # v5.86.100: Two big changes here based on honest accuracy assessment:
        #
        #   1. Per-dimension thresholds. The classifier returns separate
        #      category_confidence and severity_confidence. Previous code
        #      gated on min(cat, sev) ≥ 0.85, which conflated the two.
        #
        #   2. Severity override is DISABLED. Current model: 65% severity
        #      accuracy, even at high confidence. Net-negative for customers
        #      because Claude does ~85%+ on severity. Until severity model
        #      meaningfully outperforms Claude, severity stays with Claude.
        #
        #      Category override stays enabled — model is 83% accurate at
        #      0.85+ confidence, comparable to Claude, and category mistakes
        #      have lower customer blast radius (related categories still
        #      surface real findings).
        #
        # When the severity model crosses ~80% accuracy at 0.90+ confidence
        # in a future retrain, flip CATEGORY_OVERRIDE_ENABLED -> True.
        CATEGORY_OVERRIDE_ENABLED = True
        SEVERITY_OVERRIDE_ENABLED = False  # see comment above
        CATEGORY_THRESHOLD = 0.85
        SEVERITY_THRESHOLD = 0.95  # would-be threshold if re-enabled

        ml_routed_cat = 0
        ml_routed_sev = 0
        ml_total = 0
        if inspection_doc.inspection_findings:
            try:
                from ml_inference import get_classifier
                clf = get_classifier()
                if clf.is_ready():
                    t0 = time.time()
                    texts = [f.description for f in inspection_doc.inspection_findings if f.description]
                    if texts:
                        results = clf.classify_batch(texts)
                        ml_total = len(results)
                        text_idx = 0
                        for finding in inspection_doc.inspection_findings:
                            ...
                            cat_map = {v.value: v for v in IssueCategory}
                            sev_map = {v.value: v for v in Severity}

                            # Category override (enabled, threshold 0.85)
                            ...
                            # Severity override (disabled — see comment above)
                            if SEVERITY_OVERRIDE_ENABLED:
                                sev_conf = r.get('severity_confidence', 0)
                                if sev_conf >= SEVERITY_THRESHOLD:
                                    ml_sev = sev_map.get(r.get('severity'))
                                    if ml_sev:
                                        finding.severity = ml_sev
                                        ml_routed_sev += 1
                    timing['ml_classify'] = time.time() - t0
                    if ml_routed_cat > 0 or ml_routed_sev > 0:
                        logger.info(f"🧠 ML hybrid: {ml_routed_cat}/{ml_total} category overrides, {ml_routed_sev}/{ml_total} severity overrides")
            except Exception as ml_err:
                logger.debug(f"ML inference skipped: {ml_err}")

        # 💰 ML COST PREDICTION (v5.86.100, gated v5.87.0)
        #
        # Wire the trained Cost Predictor into the analysis flow with a
        # confidence gate at 0.85, matching the architectural principle
        # that ML overrides baseline only when ML is confident.
        #
        # v5.87.0: cost confidence now comes from a real quantile regression
        # interval (p90 - p10)/p50 → exp decay → confidence ∈ [0,1].
        # Legacy point-estimate models get confidence=0.70 (neutral) so they
        # never meet the 0.85 bar; they effectively defer to baseline until
        # retrained with quantile objective.
        #
        # Order of precedence for finding cost:
        #   1. cost_from_document=True      → respect (document says exact $X)
        #   2. ML confidence ≥ 0.85         → use ML prediction
        #   3. ML confidence < 0.85 or err  → leave None, baseline handles it
        COST_CONFIDENCE_THRESHOLD = 0.85

        ml_cost_predicted = 0
        ml_cost_low_conf = 0
        ml_cost_skipped_doc = 0
        if inspection_doc.inspection_findings:
            try:
                ...
                if cp.is_ready():
                    ...
                    for finding in inspection_doc.inspection_findings:
                        ...
                        sev_str = finding.severity.value if hasattr(finding.severity, 'value') else str(finding.severity or '')
                        ...
                        if ml_conf >= COST_CONFIDENCE_THRESHOLD:
        ...
        if self.ai_helper.enabled and inspection_doc.inspection_findings:
            logger.info(f"🎯 Calculating confidence scores for {len(inspection_doc.inspection_findings)} findings...")
            ...
            for idx, finding in enumerate(sorted_findings, 1):
                logger.info(f"  Verifying finding {idx}/1: {finding.description[:60]}...")
                ...
                logger.info(f"    {confidence_emoji} Confidence: {confidence:.0%} - {finding.confidence_explanation[:50]}...")
        ...
        logger.info("Cross-referencing documents...")
        t0 = time.time()
        ...
        research_profile = research_data.get('profile', {}) if research_data else {}
        
        if research_profile and self.ai_helper.enabled:
            logger.info("🤖 AI cross-referencing seller claims against government records...")
            t0 = time.time()
            
            external_verifications = self.ai_helper.cross_reference_against_public_records(
            ...
            if external_verifications:
                logger.info(f"   🔍 AI found {len(external_verifications)} external verification findings")
                for ev in external_verifications:
                    icon = '🔴' if ev['severity'] == 'high' else '🟡' if ev['severity'] == 'medium' else 'ℹ️'
                    logger.info(f"      {icon} [{ev['type']}] {ev['title'][:70]}")
            else:
                logger.info("   ✅ AI found no contradictions between seller claims and public records")
            
            timing['external_verification'] = time.time() - t0
        elif research_profile:
            logger.info("🤖 AI unavailable — skipping external verification")
            timing['external_verification'] = 0
        ...
        # Phase 2: Build market intelligence from research agent data (zero extra API calls)
        market_intel = None
        if property_address and len(property_address) >= 15:
            try:
                ...
                if research_data and research_data.get('tool_results'):
                    # Preferred: consume data already fetched by research agent
                    market_intel = mi_engine.from_research_data(
                        research_data=research_data,
                        asking_price=int(property_price),
                        address=property_address,
                    )
                else:
                    # Fallback: call RentCast directly (research agent didn't run or timed out)
                    try:
                        import os as _os2, requests as _req2, re as _re2
                        _rc_key = _os2.environ.get('RENTCAST_API_KEY', '')
                        if _rc_key and property_address:
                            _avm2 = _req2.get(
                                'https://api.rentcast.io/v1/avm/value',
                                params={'address': property_address, 'compCount': 10, 'maxRadius': 5, 'daysOld': 180},
                                headers={'X-Api-Key': _rc_key, 'Accept': 'application/json'},
                                timeout=8
                            )
                            if _avm2.status_code == 200:
                                _avmd = _avm2.json()
                                _comps2 = []
                                for _c2 in (_avmd.get('comparables') or []):
                                    _p2 = int(_c2.get('price', 0) or 0)
                                    if not _p2: continue
                                    _sq2 = int(_c2.get('squareFootage', 0) or 0)
                                    _comps2.append({'price': _p2, 'sqft': _sq2,
                                        'price_per_sqft': round(_p2/_sq2,2) if _sq2 else 0,
                                        'bedrooms': int(_c2.get('bedrooms',0) or 0),
                                ...
                                if _zm2:
                                    try:
                                        _msr2 = _req2.get('https://api.rentcast.io/v1/markets',
                                            params={'zipCode': _zm2.group(1), 'historyRange': 1},
                                            headers={'X-Api-Key': _rc_key}, timeout=6)
                                        if _msr2.status_code == 200:
                                            _sd2 = _msr2.json().get('saleData', _msr2.json())
                                            _ms2 = {'zip_code': _zm2.group(1),
                                                'average_days_on_market': int(_sd2.get('averageDaysOnMarket',0) or 0),
                                                'total_listings': int(_sd2.get('totalListings',0) or 0),
                                                'median_price_per_sqft': float(_sd2.get('medianPricePerSquareFoot',0) or 0)}
                                    except Exception: pass
                                _fallback_rd = {'tool_results': [
                                    {'tool_name':'rentcast','status':'success','data':{
                                        'found':True,
                                        'avm_price': int(_avmd.get('price',0) or 0),
                                        'avm_price_low': int(_avmd.get('priceRangeLow',0) or 0),
                                        'avm_price_high': int(_avmd.get('priceRangeHigh',0) or 0),
                                        'comparables': _comps2}},
                                    {'tool_name':'market_stats','status':'success' if _ms2 else 'skipped','data':_ms2}
                                ]}
                                market_intel = mi_engine.from_research_data(_fallback_rd, int(property_price), property_address)
                                logger.warning(f"[MARKET] 🔄 Fallback direct fetch: AVM={getattr(market_intel,'avm_price',0)} quality={getattr(market_intel,'data_quality','?')}")
                    except Exception as _fe:
                        logger.warning(f"[MARKET] Fallback direct fetch failed: {_fe}")
                logger.info(f"📊 Market intelligence: quality={market_intel.data_quality}, "
                             f"temp={market_intel.market_temperature}")
            except Exception as e:
        ...
        strategic_options_objs = self.options_generator.generate_options(
            ...
            findings_count=sum(
                1 for f in inspection_doc.inspection_findings
                if getattr(f, 'severity', None) and str(getattr(f.severity, 'value', f.severity)).lower()
                in ('critical', 'major', 'moderate')
            ),
            ...
            market_context=offer_strategy.get('market_context')
        )
        ...
        # 🚀 BILLION-DOLLAR INNOVATIONS
        logger.info("=" * 60)
        logger.info("🚀 GENERATING PATENTABLE INNOVATIONS")
        logger.info("=" * 60)
        
        # Innovation #1: Generate Property Risk DNA™
        logger.info("🧬 Encoding Property Risk DNA...")
        t0 = time.time()
        risk_dna = None
        ...
        try:
            ...
            logger.info(f"   ✅ Risk DNA: {risk_dna.composite_score:.1f}/100 ({risk_dna.risk_category})")
            ...
            if score_gap > 20:
                ...
                logger.info(f"   ⚠️  WARNING: Large score gap of {score_gap:.1f} points detected")
                logger.info(f"      overall_risk: {overall_risk:.1f}, risk_dna: {risk_dna.composite_score:.1f}")
            elif score_gap > 10:
                # Moderate gap - acceptable but log it
                logger.info(f"   ℹ️  Score gap: {score_gap:.1f} points (acceptable range)")
            else:
                # Good alignment
                logger.info(f"   ✅ Excellent score alignment (gap: {score_gap:.1f} points)")
            ...
            if len(self.dna_encoder.dna_database) >= 2:
                ...
                logger.info(f"   📊 Benchmarked against {market_benchmarks.get('similar_count', 0)} similar properties")
        except Exception as e:
            import traceback
            error_details = traceback.format_exc()
            logger.info(f"   ⚠️  Risk DNA generation failed: {e}")
            logger.info(f"   📋 Full error details:\n{error_details}")
            logger.error(f"Risk DNA error: {e}\n{error_details}")
        timing['risk_dna'] = time.time() - t0
        
        # Innovation #2: Generate Transparency Report™
        logger.info("🔍 Scoring Seller Transparency...")
        t0 = time.time()
        transparency_report = None
        _transparency_applicable = getattr(cross_ref, 'transparency_applicable', True)
        try:
            # Build property metadata from research data for transparency verification
            property_metadata = None
            if research_profile:
                property_metadata = {
                    'research_profile': research_profile,
                    'external_verifications': external_verifications,
                    'flood_zone': research_profile.get('flood_zone'),
                    'earthquake_zone': research_profile.get('earthquake_zone'),
                    'fire_hazard_zone': research_profile.get('fire_hazard_zone'),
                    'disaster_count': len(research_profile.get('disaster_declarations', [])),
                    'zip_median_home_value': research_profile.get('zip_median_home_value'),
                }

            if not _transparency_applicable:
                # v5.89.121: No seller disclosure was provided. We have no basis
                # to judge transparency, so we do NOT produce a transparency report
                # (a fabricated grade here would be misleading). Downstream UI hides
                # the Transparency Report and disclosure-gap sections in this case.
                logger.info("   ℹ️  Transparency not applicable (no disclosure provided) — skipping transparency report")
            else:
                ...
                logger.info(f"   ✅ Transparency: {transparency_report.transparency_score}/100 (Grade {transparency_report.grade.value})")
                logger.info(f"   🚨 Red flags: {len(transparency_report.red_flags)}")
                logger.info(f"   ⚠️  Undisclosed issues: {len(transparency_report.undisclosed_issues)}")
        except Exception as e:
            logger.info(f"   ⚠️  Transparency scoring failed: {e}")
        ...
        # Runs LLM intelligence on top of rule-based analysis.
        # Does NOT replace anything — enhances, verifies, and catches missed items.
        logger.info("🤖 Running AI Intelligence Layer...")
        t0 = time.time()
        hybrid_result = None
        try:
            from hybrid_ai import enhance_analysis
            # Convert rule findings to dicts for the hybrid layer
            rule_finding_dicts = []
            for f in (inspection_doc.inspection_findings or []):
                rule_finding_dicts.append({
                    'category': f.category.value if hasattr(f.category, 'value') else str(f.category),
                    'severity': f.severity.value if hasattr(f.severity, 'value') else str(f.severity),
                    'description': f.description,
                    'location': getattr(f, 'location', ''),
                    'raw_text': getattr(f, 'raw_text', ''),
                    'safety_concern': getattr(f, 'safety_concern', False),
                    'requires_specialist': getattr(f, 'requires_specialist', False),
                })

            # Get cost breakdown if available
            rule_cost_breakdown = None
            if hasattr(risk_score, 'repair_estimate') and risk_score.repair_estimate:
                rule_cost_breakdown = getattr(risk_score.repair_estimate, 'breakdown', None)

            hybrid_result = enhance_analysis(
                inspection_text=inspection_report_text,
                disclosure_text=seller_disclosure_text,
                rule_findings=rule_finding_dicts,
                rule_cost_breakdown=rule_cost_breakdown,
                zip_code=getattr(risk_score, 'zip_code', '') or '',
                property_year_built=None,
            )
            stats = hybrid_result.get('stats', {})
            logger.info(f"   ✅ AI Layer: {stats.get('llm_new_findings', 0)} new findings, "
                        f"{stats.get('llm_verified', 0)} verified, "
                        f"{stats.get('contradictions_found', 0)} contradictions, "
                        f"{stats.get('costs_adjusted', 0)} cost adjustments")
        except Exception as e:
            logger.warning(f"   ⚠️ AI layer failed (non-fatal): {e}")
        timing['ai_layer'] = time.time() - t0
        # ═══ END AI INTELLIGENCE LAYER ══════════════════════════════════════════

        logger.info("=" * 60)
        logger.info("✅ ALL INNOVATIONS GENERATED SUCCESSFULLY")
        logger.info("=" * 60)
        ...
        logger.info("\n" + "=" * 80)
        logger.info("⏱️  INTELLIGENCE PERFORMANCE BREAKDOWN")
        logger.info("=" * 80)
        for key, duration in sorted(timing.items(), key=lambda x: -x[1]):
            if key != 'total':
                pct = (duration / timing['total'] * 100) if timing['total'] > 0 else 0
                logger.info(f"{key:.<50} {duration:>6.2f}s ({pct:>5.1f}%)")
        logger.info("-" * 80)
        logger.info(f"{'TOTAL':.<50} {timing['total']:>6.2f}s (100.0%)")
        logger.info("=" * 80)
        ...
        return PropertyAnalysis(
            ...
            external_verifications=external_verifications if external_verifications else None,
            market_intelligence=market_intel.to_dict() if market_intel and hasattr(market_intel, 'to_dict') else None,
            hybrid_ai=hybrid_result,
        )
    ...
    def _generate_offer_strategy(
        ...
        # ── Phase 2: Market Intelligence Adjustment ───────────────────────
        market_adjustment = 0.0
        market_context = {}
        
        logger.warning(f"[MARKET] 📊 Market intel: {'yes' if market_intel else 'None'} quality={getattr(market_intel,'data_quality','n/a')} avm={getattr(market_intel,'avm_price',0)} temp={getattr(market_intel,'market_temperature','n/a')}")
        if market_intel and hasattr(market_intel, 'data_quality') and market_intel.data_quality != "none":
            from market_intelligence import apply_market_adjustment
            market_result = apply_market_adjustment(recommended_offer, property_price, market_intel)
            logger.warning(f"[MARKET] 📊 Market adj: applied={market_result.get('market_applied')} temp={market_result.get('market_temperature')} avm={market_result.get('estimated_value',0)}")
            
            if market_result.get('market_applied'):
                market_adjustment = market_result.get('market_adjustment_amount', 0)
                recommended_offer = market_result.get('adjusted_offer', recommended_offer)
                market_context = {
                    'market_applied': True,
                    ...
                    # Distressed sales
                    'foreclosure_count': getattr(market_intel, 'foreclosure_count', 0),
                    'short_sale_count': getattr(market_intel, 'short_sale_count', 0),
                    'new_construction_count': getattr(market_intel, 'new_construction_count', 0),
                    'distressed_pct': getattr(market_intel, 'distressed_pct', 0),
                    # Price positioning
                    'price_percentile': getattr(market_intel, 'price_percentile', 0),
                    'price_vs_type_median_pct': getattr(market_intel, 'price_vs_type_median_pct', 0),
                    'price_vs_bed_median_pct': getattr(market_intel, 'price_vs_bed_median_pct', 0),
                    # Market stats
                    'zip_code': getattr(market_intel.market, 'zip_code', '') if hasattr(market_intel, 'market') else '',
                    'median_price_per_sqft': getattr(market_intel.market, 'median_price_per_sqft', 0) if hasattr(market_intel, 'market') else 0,
                    'total_listings': getattr(market_intel.market, 'total_listings', 0) if hasattr(market_intel, 'market') else 0,
                    'new_listings': getattr(market_intel.market, 'new_listings_this_month', 0) if hasattr(market_intel, 'market') else 0,
                    'avg_dom': getattr(market_intel.market, 'average_days_on_market', 0) if hasattr(market_intel, 'market') else 0,
                    'price_trend_pct': getattr(market_intel.market, 'price_trend_pct', 0) if hasattr(market_intel, 'market') else 0,
                    'inventory_trend_pct': getattr(market_intel.market, 'inventory_trend_pct', 0) if hasattr(market_intel, 'market') else 0,
                    # Type + bedroom specific
                    'type_match_median': getattr(market_intel.market, 'type_match_median', 0) if hasattr(market_intel, 'market') else 0,
                    'bed_match_median': getattr(market_intel.market, 'bed_match_median', 0) if hasattr(market_intel, 'market') else 0,
                    # Monthly history for charts
                    'history': getattr(market_intel.market, 'history', []) if hasattr(market_intel, 'market') else [],
                }
                logging.info(f"📊 Market adjustment applied: ${market_adjustment:+,.0f} "
                             f"({market_result.get('market_adjustment_pct', 0):+.1%})")
        # ──────────────────────────────────────────────────────────────────
    ...
    def _generate_decision_framework(
        ...
        if len(risk_score.deal_breakers) > 2:
            ...
            confidence = 0.90
        elif getattr(cross_ref, 'transparency_applicable', True) and cross_ref.transparency_score < 30:
...
if __name__ == "__main__":
    ...
    logger.info("\n" + "=" * 100)
    logger.info("RUNNING OFFERWISE INTELLIGENCE SYSTEM")
    logger.info("=" * 100 + "\n")
    ...
    logger.info("\n\n" + "=" * 100)
    logger.info("DETAILED OFFER STRATEGY")
    logger.info("=" * 100)
    logger.info(json.dumps(analysis.offer_strategy, indent=2))
