...
def main():
    ...
    # v5.89.41: validate child_env.DATABASE_URL is real Postgres before
    # spawning any subprocess. Catches env pollution from test_*.py imports
    # leaking into the live worker process (Layer 1 fix is in
    # integrity_tests.py; this is defense-in-depth in case some other code
    # path imports a test file in the future). Loud, clear failure beats
    # silently training against an empty SQLite file.
    db_url = (child_env.get('DATABASE_URL', '') or '').strip()
    if not db_url:
        msg = (
            '❌ Training aborted: DATABASE_URL is not set. '
            'Production training requires a real Postgres connection. '
            'Suspect env pollution — restart the web worker (Render: '
    ...
    if db_url.startswith('sqlite:'):
        msg = (
            f'❌ Training aborted: DATABASE_URL points at SQLite '
            f'({db_url!r}). Production training requires Postgres. '
            f'This is almost certainly env pollution from a test_*.py '
            f'import — restart the web worker (Render: Manual Deploy → '
            f'Clear build cache & deploy) to clear the polluted env.'
        )
        _log(job_id, msg, 'error', app_dir, t_start)
        _mark_failed(job_id, msg, app_dir, t_start)
        sys.exit(1)
