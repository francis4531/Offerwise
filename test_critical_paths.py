...
class TestVisionExtraction(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        # Skip in production/server environment to avoid spamming Sentry with API errors
        # These tests need real PDFs and are meant for local development only
        if os.environ.get('RENDER') or os.environ.get('FLASK_ENV') not in ('development', 'testing', None):
            raise unittest.SkipTest("Vision extraction tests skipped in server environment")
