...
def _emit_confusion_diagnostics(
    _log, y_true, y_pred, target_names, texts,
    label='Classifier', max_pairs=6, examples_per_pair=2, indent='  ',
):
    """v5.89.36: emit confusion matrix + top confusion pairs + sample
    misclassifications. Used after both Category and Severity classifier
    eval to surface WHERE the model fails, not just aggregate accuracy.

    Args:
      y_true, y_pred: 1-D arrays of class indices (same length)
      target_names: list of class names, indexed by class id
      texts: list of original text strings, same length as y_true. Pass
        an empty list to skip the misclassification samples section.
      label: short prefix for log lines (e.g. "Category", "Severity")
      max_pairs: how many top confusion pairs to surface
      examples_per_pair: how many text examples per pair (capped at 3)
    """
    try:
        import numpy as _np
        ...
        # Each cell padded so values and abbreviated headers are visually
        # separated. cell_w = max(5, longest_count_digits + 1).
        # 4-char abbreviations + 1 space margin = width 5 minimum.
        max_count = int(cm.max()) if cm.size else 0
        cell_w = max(5, len(str(max_count)) + 1)
        # 4-char abbreviations for column headers (e.g. "elec", "envi")
        col_abbrevs = [n[:4] for n in target_names]
        # Row labels right-padded to the longest class-name length
        row_w = max(len(n) for n in target_names) + 2

        _log(f'{indent}Confusion matrix (rows=true, cols=predicted, diagonal=correct):')
        header = ' ' * row_w + ''.join(f'{a:>{cell_w}}' for a in col_abbrevs)
        _log(f'{indent}{header}')
        for i, name in enumerate(target_names):
            row = ''.join(f'{int(cm[i, j]):>{cell_w}}' for j in range(n_classes))
            _log(f'{indent}{name:>{row_w}}{row}')

        # --- 2. Top confusion pairs (off-diagonal errors only) ---
        pairs = []
        ...
        pairs.sort(reverse=True)  # most errors first

        if pairs:
            _log('')
            _log(f'{indent}Top {label} confusion pairs (errors only, descending):')
            for count, i, j in pairs[:max_pairs]:
                _log(f'{indent}  {target_names[i]:>{row_w-2}} → '
                     f'{target_names[j]:<{row_w-2}}  {count} misses')

        # --- 3. Sample misclassifications ---
        # Skip if texts unavailable (e.g. caller didn't preserve them).
        if not texts or not pairs:
            return

        examples_per_pair = min(3, max(1, examples_per_pair))
        _log('')
        _log(f'{indent}Sample misclassifications (up to {examples_per_pair} per top pair):')

        # Build (true_idx, pred_idx) → [text_idx, ...] index for fast lookup
        y_true_arr = _np.asarray(y_true)
        y_pred_arr = _np.asarray(y_pred)
        # For each top pair, pull a few example text indices and emit
        for count, i, j in pairs[:max_pairs]:
            mask = (y_true_arr == i) & (y_pred_arr == j)
            example_idxs = _np.where(mask)[0][:examples_per_pair]
            if len(example_idxs) == 0:
                continue
            _log(f'{indent}  [{target_names[i]} → {target_names[j]}]')
            for idx in example_idxs:
                # Truncate to keep one log line; collapse whitespace
                snippet = ' '.join(texts[int(idx)].split())[:120]
                _log(f'{indent}    "{snippet}"')
    except Exception as e:
        _log(f'{indent}(could not generate confusion diagnostics: {e})', 'warn')


def _persist_audit_queue(
    _log,
    training_job_id,
    y_true,
    y_pred,
    ...
    """v5.89.42: stratified sample of misclassifications, persisted to
    ml_label_audit_queue for operator review at /admin/labels/audit.

    The sampling distributes target_count rows across the top confusion
    pairs proportionally to error count. Then optionally adds
    high_conf_count rows where the model was ≥0.85 confident AND wrong
    (regardless of which pair). These are diagnostic of either label
    noise or systematic model failures.

    Args:
      training_job_id : per-run UUID (links rows of one training batch)
      y_true          : int array, true class indices
      y_pred          : int array, predicted class indices
      confidences     : float array, max predicted probability per row
      target_names    : list of class names indexed by class id
                        (e.g. ['electrical', 'environmental', ...])
      severity_names_for_each_row : per-row severity STRINGS aligned to
                        y_true/y_pred. For the category classifier this
                        is the row's actual severity label (we don't have
                        a severity prediction yet at that point — we
                        pass the original severity for both 'original'
                        and 'predicted'). For the severity classifier
                        this is irrelevant — pass the same list.
      texts           : per-row finding text snippet
      finding_ids     : per-row MLFindingLabel.id
      queue_type      : 'category' | 'severity' | 'high_conf'
      target_count    : how many primary-bucket rows to persist
      high_conf_count : how many extra rows from the high-conf-wrong
                        bucket to add (regardless of pair)

    Errors are logged but non-fatal — training continues even if the
    audit queue fails to populate. Returns the number of rows inserted.
    """
    try:
        import numpy as _np
        import random as _random
        from models import db as _db, MLLabelAuditQueue
        from datetime import datetime as _dt

        y_true_arr = _np.asarray(y_true)
        y_pred_arr = _np.asarray(y_pred)
        confs_arr = _np.asarray(confidences) if confidences is not None else None
        n = len(y_true_arr)

        # Build (true, pred) → [row indices] for off-diagonal cells
        wrong_mask = y_true_arr != y_pred_arr
        wrong_idxs = _np.where(wrong_mask)[0]
        if len(wrong_idxs) == 0:
            _log(f'  (no misclassifications to queue for audit — {queue_type})')
            return 0

        # Group by (true, pred) pair
        pair_buckets = {}
        for idx in wrong_idxs:
            pair = (int(y_true_arr[idx]), int(y_pred_arr[idx]))
            pair_buckets.setdefault(pair, []).append(int(idx))

        # Sort pairs by size (largest first) — primary error sources
        sorted_pairs = sorted(pair_buckets.items(), key=lambda kv: -len(kv[1]))

        # Distribute target_count proportionally across the top pairs.
        # Use simple proportional allocation with rounding.
        total_wrong = sum(len(v) for _, v in sorted_pairs)
        selected_indices = []
        for pair, bucket in sorted_pairs:
            share = max(1, round(target_count * len(bucket) / total_wrong))
            sampled = bucket if len(bucket) <= share else _random.sample(bucket, share)
        ...
        # Optionally add high-confidence-wrong rows. Pick top N by confidence
        # from the wrong set, excluding rows already selected above.
        if high_conf_count > 0 and confs_arr is not None:
            already = set(selected_indices)
            high_conf_candidates = [
                (int(idx), float(confs_arr[idx])) for idx in wrong_idxs
                if int(idx) not in already and float(confs_arr[idx]) >= 0.85
            ]
            high_conf_candidates.sort(key=lambda x: -x[1])
            for idx, _conf in high_conf_candidates[:high_conf_count]:
                selected_indices.append(idx)

        # Now build and insert the queue rows
        inserted = 0
        for idx in selected_indices:
            true_i = int(y_true_arr[idx])
            pred_i = int(y_pred_arr[idx])
            true_name = target_names[true_i]
            pred_name = target_names[pred_i]

            # For category queues: original_severity == predicted_severity
            # (same row, no severity prediction yet at category step).
            # For severity queues: target_names ARE severities, so
            # true_name/pred_name ARE the severity strings.
            if queue_type == 'category':
                orig_cat = true_name
                pred_cat = pred_name
                orig_sev = severity_names_for_each_row[idx] if severity_names_for_each_row else ''
                pred_sev = orig_sev
                conf_cat = float(confs_arr[idx]) if confs_arr is not None else None
                conf_sev = None
            elif queue_type == 'severity':
                orig_cat = ''  # unknown at this point
                pred_cat = ''
                orig_sev = true_name
                pred_sev = pred_name
                conf_cat = None
                conf_sev = float(confs_arr[idx]) if confs_arr is not None else None
            else:  # high_conf, treated like its underlying type
                orig_cat = true_name
                pred_cat = pred_name
                orig_sev = severity_names_for_each_row[idx] if severity_names_for_each_row else ''
                pred_sev = orig_sev
            ...
            confusion_pair = f'{true_name}→{pred_name}'

            try:
                row = MLLabelAuditQueue(
                    training_job_id=training_job_id,
                    finding_id=int(finding_ids[idx]),
                    queued_at=_dt.utcnow(),
                    queue_type=queue_type,
                    confusion_pair=confusion_pair[:100],
                    original_category=str(orig_cat)[:100],
                    original_severity=str(orig_sev)[:50],
                    predicted_category=str(pred_cat)[:100],
                    predicted_severity=str(pred_sev)[:50],
                    confidence_category=conf_cat,
                    confidence_severity=conf_sev,
                    text_snippet=(texts[idx] or '')[:5000],
                )
                _db.session.add(row)
                inserted += 1
            except Exception as row_err:
                _log(f'  (audit queue row failed: {row_err})', 'warn')

        try:
            _db.session.commit()
            _log(f'  Audit queue: persisted {inserted} {queue_type} misclassifications for review')
        except Exception as commit_err:
            _log(f'  Audit queue commit FAILED: {commit_err}', 'warn')
            try:
        ...
        _log(f'  Audit queue: persistence failed ({type(e).__name__}: {e})', 'warn')
        return 0
...
def train_finding_classifier(job_id):
    ...
    base_dir = os.path.dirname(os.path.abspath(__file__))
    # v5.89.55: write to persistent disk so artifacts survive deploys
    ...
    try:
        ...
        row_query = (MLFindingLabel.query
                     # v5.89.42: respect audit decisions. Rows marked
                     # excluded_from_training (verdict 5 'junk') are skipped.
                     .filter(MLFindingLabel.excluded_from_training == False)
                     .with_entities(
                         MLFindingLabel.id,            # v5.89.42: for audit queue
                     ...
                     .yield_per(2000))
        # We still need a total count for logging — also respecting the
        # exclusion filter so the displayed count matches what we'll train on
        total_count = MLFindingLabel.query.filter(
            MLFindingLabel.excluded_from_training == False
        ).count()
        ...
        # Each row is a 8-tuple matching the with_entities() column order above.
        # v5.89.42: first element is now finding.id (added for audit queue).
        for row_tuple in row_query:
            (finding_id, finding_text, category_v1, severity_v1,
            ...
            if cat and sev and sev in ('critical', 'major', 'moderate', 'minor'):
                # v5.89.42: preserve finding_id so the audit queue can
                # reference the source row
                data.append({'id': finding_id, 'text': text, 'category': cat, 'severity': sev})
            else:
...
[{{"text": "finding", "category": "cat", "severity": "sev"}}]"""
        ...
        # v5.89.35: taxonomy + singleton-class filter.
        #
        # Root cause: the corpus contained 1 row labeled with the category
        # 'legal_title'. That category is in document_parser.py's 8-category
        # taxonomy (which the live disclosure analyzer uses) but NOT in the
        # ml_ingestion/relabel_v1.py 7-category taxonomy (which the ML
        # classifier targets). The single misaligned row made it into the
        # train/test split and XGBoost rejected the discontinuous class set
        # with a cryptic "Invalid classes inferred" error.
        #
        # Two defensive filters, both with clear logging:
        #
        # Filter 1: drop rows whose category or severity is not in the ML
        # taxonomy. Catches taxonomy drift between document_parser and
        # relabel_v1, label corruption, and stale rows from older crawler
        # versions.
        #
        # Filter 2: drop rows belonging to classes with fewer than
        # MIN_ROWS_PER_CLASS=20 rows. 20 gives a clear margin for the
        # downstream stratified 80/20 split (≥16 train + ≥4 test per class).
        # Below that, XGBoost may technically fit but the resulting class
        # estimates are noise. Catches future singleton/sparse classes.
        try:
            from ml_ingestion.relabel_v1 import VALID_CATEGORIES, VALID_SEVERITIES
        except ImportError:
            # Hardcoded fallback in case relabel_v1 import fails for any reason.
            # Matches the taxonomy at v5.89.35. If the taxonomy changes in
            # relabel_v1.py, update here too OR import will succeed and this
            # block won't be used.
            VALID_CATEGORIES = ['electrical', 'environmental', 'foundation_structure',
                                'general', 'hvac_systems', 'plumbing', 'roof_exterior']
            VALID_SEVERITIES = ['critical', 'major', 'moderate', 'minor']

        VALID_CAT_SET = set(VALID_CATEGORIES)
        VALID_SEV_SET = set(VALID_SEVERITIES)
        MIN_ROWS_PER_CLASS = 20

        # Filter 1: out-of-taxonomy values
        pre_filter_count = len(data)
        oob_cat_counts = {}
        oob_sev_counts = {}
        for d in data:
            if d.get('category') not in VALID_CAT_SET:
                oob_cat_counts[d.get('category')] = oob_cat_counts.get(d.get('category'), 0) + 1
            if d.get('severity') not in VALID_SEV_SET:
                oob_sev_counts[d.get('severity')] = oob_sev_counts.get(d.get('severity'), 0) + 1

        data = [
            d for d in data
            if d.get('category') in VALID_CAT_SET and d.get('severity') in VALID_SEV_SET
        ]
        oob_dropped = pre_filter_count - len(data)
        if oob_dropped > 0:
            if oob_cat_counts:
                _log(f'  Filter 1: dropped {oob_dropped} rows with out-of-taxonomy '
                     f'categories: {dict(oob_cat_counts)}', 'warn')
            if oob_sev_counts:
                _log(f'  Filter 1: dropped rows with out-of-taxonomy severities: '
                     f'{dict(oob_sev_counts)}', 'warn')

        # Filter 2: singleton/sparse classes. Applied AFTER Filter 1 so we
        # don't double-count rows already dropped for being out-of-taxonomy.
        cat_counts = {}
        for d in data:
            cat_counts[d['category']] = cat_counts.get(d['category'], 0) + 1

        rare_cats = {c: n for c, n in cat_counts.items() if n < MIN_ROWS_PER_CLASS}
        if rare_cats:
            _log(f'  Filter 2: dropping {sum(rare_cats.values())} rows from rare '
                 f'categories (< {MIN_ROWS_PER_CLASS} rows): {rare_cats}', 'warn')
            data = [d for d in data if d['category'] not in rare_cats]

        # Final safety check — if filters left us with too little, abort
        # cleanly rather than crash downstream.
        if len(data) < 20:
            _log(f'After taxonomy + singleton filters: only {len(data)} rows left '
                 f'(was {pre_filter_count}). Need 20+ to train.', 'error')
            _update_results(job_id, {'Finding Classifier': {
                'status': 'INSUFFICIENT_AFTER_FILTERS',
                'data_points': len(data),
                'pre_filter_count': pre_filter_count,
            }})
            return

        if len(data) < pre_filter_count:
            _log(f'  After filters: {len(data):,} rows ready for encoding '
                 f'(was {pre_filter_count:,})')
        ...
        # v5.89.42: preserve finding_ids alongside texts so the audit
        # queue can reference the source ml_finding_labels rows
        finding_ids = [d['id'] for d in data]
        ...
        # v5.89.36: also split indices so we can grab the original texts for
        # test-set rows. Used later for misclassification diagnostics.
        # texts_te kept; texts_tr is discarded along with `texts` below.
        idx_all = np.arange(len(cats))
        try:
            X_tr, X_te, yc_tr, yc_te, ys_tr, ys_te, idx_tr, idx_te = train_test_split(
                emb, y_cat, y_sev, idx_all, test_size=0.2, random_state=42, stratify=y_cat)
        except ValueError:
            X_tr, X_te, yc_tr, yc_te, ys_tr, ys_te, idx_tr, idx_te = train_test_split(
                emb, y_cat, y_sev, idx_all, test_size=0.2, random_state=42)
        _log(f'Train/test split: {len(X_tr)} train, {len(X_te)} test')
        # v5.89.36: keep test-set texts for misclassification logging.
        # ~10MB for 1748 strings × ~500 chars — negligible vs ~1.4GB peak.
        texts_te = [texts[i] for i in idx_te]
        # v5.89.42: also keep test-set finding_ids for audit queue population.
        # ~16KB for 1748 integers — negligible.
        finding_ids_te = [finding_ids[i] for i in idx_te]
        del emb, texts, cats, sevs, y_cat, y_sev, idx_all, idx_tr, finding_ids
        gc.collect()
        _log_mem('post-split')
        ...
        # v5.89.36: confusion matrix + top confusion pairs + sample
        # misclassifications. Surfaces WHERE the model fails, not just
        # aggregate accuracy — gives operator the evidence base for
        # deciding which categories need targeted improvement.
        _emit_confusion_diagnostics(
            _log, yc_te, cp, list(cat_enc.classes_), texts_te,
            label='Category', max_pairs=6, examples_per_pair=2,
        )

        # v5.89.42: persist a stratified sample of category misclassifications
        # to ml_label_audit_queue. Operator reviews these via
        # /admin/labels/audit. ~120 rows for category errors distributed
        # across top confusion pairs, plus 20 high-confidence-wrong for
        # diagnostic value. The severity-names-for-each-row array is
        # the original sevs for these test-set rows (needed because the
        # severity classifier hasn't run yet at this point).
        #
        # We need the per-row original severity string. Pull from sev_enc
        # since we still have ys_te around.
        try:
            sevs_for_te = list(sev_enc.inverse_transform(ys_te))
        except Exception:
            sevs_for_te = []
        try:
            cat_conf_proba = cm.predict_proba(X_te)
            cat_conf_max = cat_conf_proba.max(axis=1)
        except Exception:
            cat_conf_max = None
        _persist_audit_queue(
            _log=_log,
            training_job_id=job_id,
            y_true=yc_te,
            y_pred=cp,
            confidences=cat_conf_max,
            target_names=list(cat_enc.classes_),
            severity_names_for_each_row=sevs_for_te,
            texts=texts_te,
            finding_ids=finding_ids_te,
            queue_type='category',
            target_count=120,
            high_conf_count=20,
        )
        ...
        # v5.89.83 CRITICAL GUARD: CalibratedClassifierCV(cv='prefit') fit on
        # the test split silently drops any class that has zero examples in
        # that split. A calibrator with fewer classes than the encoder breaks
        # inference: the dropped class can never be predicted, and argmax
        # indices decode against a mismatched table → confident-WRONG output.
        # (This is exactly the bug that made roof_exterior unpredictable and
        # sent roof findings to 'plumbing'.) Only trust the calibrator if it
        # covers every class; otherwise skip it and let inference use the
        # complete raw XGBoost model.
        cal_path = os.path.join(models_dir, 'category_calibrator.pkl')
        n_cal = len(getattr(cm_calibrated, 'classes_', []))
        n_enc = len(cat_enc.classes_)
        if n_cal == n_enc:
            pickle.dump(cm_calibrated, open(cal_path, 'wb'))
            _log(f'  >> Saved finding_category.xgb + category_calibrator.pkl ({n_cal} classes)')
        else:
            # Remove any stale calibrator so inference doesn't load an old broken one
            try:
                if os.path.exists(cal_path):
                    os.remove(cal_path)
            except Exception:
                pass
            _log(f'  >> WARNING: calibrator covers only {n_cal}/{n_enc} classes '
                 f'(a category was absent from the test split). NOT saving calibrator — '
                 f'inference will use raw XGBoost (all {n_enc} classes). '
                 f'To restore calibration, ensure every category appears in the test split '
                 f'(stratified split or more data for rare categories).')
            _log('  >> Saved finding_category.xgb + category_encoder.pkl (no calibrator)')
        cm = None
        cp = None
        cm_calibrated = None
        yc_tr = None
        ...
        # v5.89.36: same diagnostics as Category — confusion matrix + top
        # pairs + samples. 4 severity classes means at most 12 off-diagonal
        # cells, so max_pairs=4 is enough to cover the meaningful ones.
        _emit_confusion_diagnostics(
            _log, ys_te, sp, list(sev_enc.classes_), texts_te,
            label='Severity', max_pairs=4, examples_per_pair=2,
        )

        # v5.89.42: persist severity misclassifications to audit queue.
        # ~60 rows. No need for high-conf bucket — severity has only 4
        # classes so confusions are mostly between adjacent levels, all
        # of which are diagnostically valuable to audit.
        try:
            sev_conf_proba = sm.predict_proba(X_te)
            sev_conf_max = sev_conf_proba.max(axis=1)
        except Exception:
            sev_conf_max = None
        _persist_audit_queue(
            _log=_log,
            ...
            y_true=ys_te,
            y_pred=sp,
            confidences=sev_conf_max,
            target_names=list(sev_enc.classes_),
            severity_names_for_each_row=None,  # not used for severity queue
            texts=texts_te,
            finding_ids=finding_ids_te,
            queue_type='severity',
            target_count=60,
        ...
        # v5.89.83: same class-coverage guard as the category calibrator.
        sev_cal_path = os.path.join(models_dir, 'severity_calibrator.pkl')
        n_sev_cal = len(getattr(sm_calibrated, 'classes_', []))
        n_sev_enc = len(sev_enc.classes_)
        if n_sev_cal == n_sev_enc:
            pickle.dump(sm_calibrated, open(sev_cal_path, 'wb'))
            _log(f'  >> Saved finding_severity.xgb + severity_calibrator.pkl ({n_sev_cal} classes)')
        else:
            try:
                if os.path.exists(sev_cal_path):
                    os.remove(sev_cal_path)
            except Exception:
                pass
            _log(f'  >> WARNING: severity calibrator covers only {n_sev_cal}/{n_sev_enc} classes. '
                 f'NOT saving — inference will use raw XGBoost.')
            _log('  >> Saved finding_severity.xgb + severity_encoder.pkl (no calibrator)')

        # v5.89.36: release the test-set texts that were preserved through
        # the train/test split for misclassification diagnostics. ~10MB,
        # consistent with the existing memory hygiene pattern.
        try:
            del texts_te
        except Exception:
            pass
        try:
            del finding_ids_te  # v5.89.42: also release the audit-queue IDs
        except Exception:
            pass
        gc.collect()
...
def _validate_db_url_or_die():
    """v5.89.41: abort if DATABASE_URL doesn't look like real Postgres.

    Defense-in-depth against env-pollution from test_*.py module-load
    mutations leaking into a subprocess. Three failure modes:
      - DATABASE_URL not set → likely env-copy lost it
      - DATABASE_URL starts with sqlite: → test pollution
      - DATABASE_URL doesn't start with postgres:/postgresql: → unknown
        backend, refuse rather than guess

    Exits with code 2 (distinguishable from regular training failures
    which use code 1) and writes to stderr so the orchestrator's log
    pipeline captures it.
    """
    url = (os.environ.get('DATABASE_URL', '') or '').strip()
    if not url:
        print(
            '[pipeline] FATAL: DATABASE_URL is not set. '
            'Pipeline refuses to run without a real Postgres connection.',
            file=sys.stderr,
        )
        sys.exit(2)
    if url.startswith('sqlite:'):
        print(
            f'[pipeline] FATAL: DATABASE_URL points at SQLite ({url!r}). '
            f'Production training requires Postgres. This is almost certainly '
            f'env pollution from a test_*.py import — restart the web worker.',
            file=sys.stderr,
        )
        sys.exit(2)
    if not (url.startswith('postgres://') or url.startswith('postgresql://')
        ...
        print(
            f'[pipeline] FATAL: DATABASE_URL has unrecognized scheme '
            f'({url[:40]!r}...). Expected postgres:// or postgresql://.',
            file=sys.stderr,
        )
        sys.exit(2)
...
def main():
    ...
    # Must run BEFORE `from app import app` because that import triggers
    # db.create_all() against whatever DATABASE_URL says — and against
    # SQLite, that creates empty tables which would then look like
    # "production lost all data" to anyone glancing at the schema.
