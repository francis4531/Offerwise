...
class FastCrossReferenceEngine(CrossReferenceEngine):
                ...
                if related_findings:
                    ...
                    for finding in related_findings:
                        ...
                        contradictions.append(CrossReferenceMatch(
                            ...
                            # v5.88.96: include the actual finding description.
                            # Previously rendered "Seller said no X issues, but
                            # inspector found <severity> problem" — generic
                            # template with no specifics. Users saw 5 bullets
                            # of the same pattern with no information value.
                            # Now includes what the inspector actually found.
                            explanation=(
                                f"Seller disclosed no {disclosure.category} issues, but inspector found: "
                                f"{(finding.description or '').strip()}"
                            ).strip().rstrip(':').rstrip(',') or
                                f"Seller said no {disclosure.category} issues, but inspector found a {finding.severity.value} concern",
        ...
        for finding in inspection_doc.inspection_findings:
            ...
            if not self._was_category_disclosed(finding, disclosed_categories, disclosure_index):
                undisclosed.append(CrossReferenceMatch(
                    ...
                    confidence=0.8,
                    # v5.88.96: include the actual finding description so the
                    # user sees what the inspector found, not just a category.
                    explanation=(
                        f"Inspector found {finding.severity.value} {finding.category.value} issue not disclosed by seller: "
                        f"{(finding.description or '').strip()}"
                    ).strip().rstrip(':').rstrip(',') or
                        f"Inspector found {finding.severity.value} {finding.category.value} issue not disclosed by seller",
                    risk_impact="increases_risk"
    ...
    def _tokenize_cached(self, text: str) -> FrozenSet[str]:
        """Cached tokenization - strip punctuation for reliable keyword matching"""
        import re
        # Split on whitespace, strip punctuation, lowercase, remove empties
        # This ensures "foundation:" matches "foundation" and "foundation." matches "foundation"
        tokens = set()
        for word in text.split():
            clean = re.sub(r'[^a-z0-9_-]', '', word.lower())
            if clean and len(clean) > 1:
                tokens.add(clean)
            # Also add sub-tokens for compound words like "foundation_structure"
            if '_' in clean:
    ...
    def _fast_find_related_findings(
        ...
        # OPTIMIZATION: Early exit after finding 5 matches per disclosure
        for finding in candidate_findings:
            ...
            # Threshold 2: requires meaningful overlap (not just one common word)
            # Tokenization now strips punctuation so "foundation:" matches "foundation"
            shared_keywords = disclosure_tokens & finding_tokens
            
            # Filter out very common words that cause false matches
            noise_words = frozenset({'yes', 'no', 'the', 'is', 'and', 'or', 'of', 'in', 'to', 'for',
                                     'any', 'has', 'not', 'this', 'but', 'are', 'was', 'been', 'with',
                                     'its', 'may', 'can', 'all', 'from', 'that', 'have', 'had', 'an'})
            meaningful_shared = shared_keywords - noise_words
            
            if len(meaningful_shared) >= 2:
                related.append(finding)
                continue
            
            # Fallback: check if both share keywords from the SAME category
            # This catches cases like disclosure="Foundation" + finding="structural cracking"
            # where they share a category but not exact tokens
            for category, keywords in self.category_keyword_sets.items():
                disc_match = keywords & disclosure_tokens
                find_match = keywords & finding_tokens
                if disc_match and find_match:
                    related.append(finding)
                    break
