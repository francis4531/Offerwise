...
def _build():
    ...
    with mod.app.app_context():
        ...
        from models import (User, Property, Analysis, Bug, APIKey,
                            ...
                            PropertyWatch, ShareLink, Waitlist,
        ...
        # v5.88.38: was SupportShare(); replaced by Ticket + first TicketMessage
        # (SupportShare model was removed in v5.88.38).
        ss = Ticket(user_id=u.id, property_id=p.id,
                    subject='Coverage test ticket',
                    source='in_product_share', status='open')
        mod.db.session.add(ss); mod.db.session.flush()
        ssm = TicketMessage(ticket_id=ss.id, author_kind='user',
                            author_user_id=u.id, body='TCF coverage',
                            snapshot_json=snap, full_result_json=snap)
        mod.db.session.add(ssm); mod.db.session.flush()
...
class TestAdminParam(_B):
    ...
    def test_support_share_get(self):
        # v5.88.38: hits the new ticket endpoint (the old support-shares
        # endpoint returns 410 Gone now). _S["ss_id"] is now a ticket id.
        r = self.c.get(f'/api/admin/tickets/{_S["ss_id"]}' + _aq())
        self.assertIn(r.status_code, [200, 401, 403, 404, 500])

    def test_support_share_delete(self):
        # v5.88.38: legacy DELETE was on /api/admin/support-shares/<id>
        # which no longer exists. Sanity-check the 410 stub.
        r = self.c.get('/api/admin/support-shares' + _aq())
