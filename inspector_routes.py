...
def inspector_analyze():
    ...
    # v5.88.64: Optional inspector-hosted PDF URL pass-through.
    # The inspector pastes a public URL to their own inspection PDF
    # (typically from Spectora/HomeGauge/etc.). We don't fetch or
    # store the file — just persist the URL and render it as a button
    # on the buyer-facing report. Strict validation: must be http(s)
    # only (no javascript:, data:, file:, etc.), capped at the
    # column length (2048), and reject characters that could break
    # out of the href attribute when rendered (`"`, `'`, `<`, `>`,
    # whitespace, control chars).
    inspection_pdf_url = (data.get('inspection_pdf_url') or '').strip()
    if inspection_pdf_url:
        if not (inspection_pdf_url.startswith('http://') or inspection_pdf_url.startswith('https://')):
            return jsonify({'error': 'Inspection PDF link must start with http:// or https://.'}), 400
        if len(inspection_pdf_url) > 2048:
            return jsonify({'error': 'Inspection PDF link is too long (max 2048 characters).'}), 400
        # Reject any character that has no place in a real URL and could
        # be an attribute-escape vector. Real URLs only contain a fixed
        # ASCII vocabulary; anything else means the input is hostile or
        # mangled, in which case rejecting is better than guessing.
        _BAD_URL_CHARS = set('"\'<>`\\ \t\n\r\x00')
        if any(c in _BAD_URL_CHARS for c in inspection_pdf_url):
            return jsonify({'error': 'Inspection PDF link contains invalid characters. Please paste the URL only — no quotes or spaces.'}), 400
        # Confirm urlparse can see scheme + netloc — catches malformed
        # things like "https://" with no host.
        try:
            from urllib.parse import urlparse
            parsed = urlparse(inspection_pdf_url)
            if not parsed.scheme or not parsed.netloc:
                return jsonify({'error': 'Inspection PDF link looks malformed. Please paste the full URL including https://.'}), 400
        except Exception:
            return jsonify({'error': 'Could not parse the inspection PDF link. Please paste the full URL.'}), 400
    ...
    # v5.88.16: tightened from "@ in email" to is_valid_email
    ...
    report = InspectorReport(
        ...
        inspection_pdf_url       = inspection_pdf_url or None,  # v5.88.64
...
def inspector_reports_list():
    ...
    for r in reports:
        ...
        out.append({
            ...
            'buyer_registered_at': r.buyer_registered_at.isoformat() if r.buyer_registered_at else None,
            'buyer_converted': r.buyer_converted,
            'buyer_converted_at': r.buyer_converted_at.isoformat() if r.buyer_converted_at else None,
        })
...
def inspector_report_data(token):
    ...
    return jsonify({
        ...
        # v5.88.64: Optional pass-through URL to the inspector's own
        # hosted PDF. None when the inspector didn't paste a link.
        'inspection_pdf_url': report.inspection_pdf_url or None,
        'result': result,
    })
...
def inspector_forward_to_realtor():
    ...
    # v5.88.16: tightened from "@ in email" to is_valid_email
    from blueprint_helpers import is_valid_email
    if not is_valid_email(realtor_email):
