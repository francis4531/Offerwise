...
class TestStateDisclosures(unittest.TestCase):
    ...
    def test_mississippi_not_caveat_emptor(self):
        # v5.89.120: MS reclassified caveat_emptor -> moderate. Mississippi has a
        # mandatory Property Condition Disclosure Statement (Miss. Code
        # §§ 89-1-501 to 89-1-527) when an agent is involved; it is NOT caveat emptor.
        ctx = get_state_context('MS')
        self.assertEqual(ctx.disclosure_level, 'moderate')
        self.assertNotEqual(ctx.disclosure_level, 'caveat_emptor')
        self.assertTrue(any('PCDS' in n for n in ctx.disclosure_notes))
