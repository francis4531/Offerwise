...
class AnalysisAIHelper:
    ...
    def fix_ocr_errors(self, text: str, max_length: int = 2000) -> str:
        ...
        if not self.enabled or not text:
            return text
        # Circuit breaker — skip optional OCR fix if over daily budget
        try:
            from ai_cost_tracker import is_ai_over_budget
            if is_ai_over_budget():
                logger.warning("⚡ Circuit breaker: skipping OCR fix to protect budget")
                return text
        except Exception:
            pass
...
{{
    ...
    "explanation": "brief reason"
            ...
            # 529 = Anthropic overloaded (transient) — log as warning not error
            status = getattr(e, 'status_code', None)
            log_fn = logger.warning if status in (429, 500, 503, 529) else logger.error
            log_fn(f"❌ Fact-check failed: {e}")
            return {
    ...
    def cross_reference_against_public_records(
        ...
        # Circuit breaker — this is the most expensive AI call; skip if over budget
        try:
            from ai_cost_tracker import is_ai_over_budget
            if is_ai_over_budget():
                logger.warning("⚡ Circuit breaker: skipping cross-reference to protect budget")
                return []
        except Exception:
            pass
