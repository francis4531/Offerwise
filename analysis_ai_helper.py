...
class AnalysisAIHelper:
    ...
    def __init__(self):
        ...
        if api_key:
            logger.info(f"ANTHROPIC_API_KEY length: {len(api_key)}")
            logger.info(f"ANTHROPIC_API_KEY starts with: {api_key[:10]}..." if len(api_key) > 10 else "Key too short!")
        else:
    ...
    def fix_ocr_errors(self, text: str, max_length: int = 5000) -> str:
        """
        Fix common OCR errors in text
        
        Args:
            text: Raw OCR text
            max_length: Maximum characters to process (cost control)
        ...
        try:
            ...
            if len(text) > max_length:
                logger.info(f"📄 Text too long ({len(text)} chars), processing first {max_length}")
                text = text[:max_length]
            
            logger.info(f"🔧 Fixing OCR errors (quality: {quality:.1%})...")
            
            prompt = f"""Fix OCR errors in this real estate document text.

RULES:
- Fix obvious character substitutions (0→O, 5→S, 1→l, etc.)
- Fix spacing issues
...
OUTPUT: Fixed text only"""
    ...
    def verify_finding_against_source(
        ...
        Args:
            ...
            max_source_length: Max chars to send to API
        ...
        try:
            ...
            prompt = f"""You are fact-checking a property analysis finding.
...
TASK:
Determine if the finding is ACTUALLY supported by the source document.

Be STRICT:
- "supported" = Clear evidence in source
- "unsupported" = Contradicts source OR not mentioned
- "uncertain" = Unclear or ambiguous

Find EXACT QUOTES from source as evidence (2-3 quotes maximum).
