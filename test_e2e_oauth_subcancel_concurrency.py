...
class TestAppleOAuthCallback(unittest.TestCase):
    ...
    def test_apple_callback_new_user_creates_row(self):
        """First-time Apple sign-in creates User with auth_provider='apple'."""
        ...
        # Apple is None in test env (no APPLE_CLIENT_ID). The
        # _OAuthProxy('apple') in auth_routes resolves to None and
        # would raise. Replace the proxy with a MagicMock entirely.
        mock_apple = MagicMock()
        mock_apple.authorize_access_token.return_value = token
        mock_resp = MagicMock()
        mock_resp.json.return_value = token['userinfo']
        mock_apple.get.return_value = mock_resp

        with patch('auth_routes.apple', mock_apple):
            r = self.client.get('/auth/apple/callback?code=fake&state=fake')

        self.assertIn(r.status_code, [200, 302],
            f'Apple callback should succeed, got {r.status_code}')

        with self.app.app_context():
            user = self.User.query.filter_by(email=email).first()
            self.assertIsNotNone(user)
            self.assertEqual(user.auth_provider, 'apple')
            self.assertEqual(user.apple_id, 'apple_sub_001')

    def test_apple_callback_handles_missing_name(self):
        """Apple often doesn't include name on subsequent logins —
        the handler must use email prefix as fallback."""
        email = _unique_email('apple_noname')
        token = {
            'access_token': 'x',
            'userinfo': {
                'email': email,
                'sub': 'apple_sub_noname',
                # NO 'name' key — simulates subsequent login
            },
        }
        ...
        with patch('auth_routes.apple', mock_apple):
            r = self.client.get('/auth/apple/callback?code=fake')

        self.assertNotEqual(r.status_code, 500,
            'Missing name from Apple must NOT crash with 500')

        with self.app.app_context():
            user = self.User.query.filter_by(email=email).first()
            if user:
                self.assertTrue(user.name,
                    'User must have a name (from email prefix fallback)')

    def test_apple_callback_no_email_redirects(self):
        token = {'access_token': 'x', 'userinfo': {'sub': 'no_email_user'}}
        mock_apple = MagicMock()
        mock_apple.authorize_access_token.return_value = token
        mock_resp = MagicMock()
        mock_resp.json.return_value = token['userinfo']
