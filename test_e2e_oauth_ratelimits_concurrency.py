...
class TestAppleOAuthCallback(unittest.TestCase):
    ...
    def test_apple_callback_creates_new_user(self):
        """First Apple sign-in creates User with auth_provider='apple'
        and apple_id set."""
        email = _unique_email('newapple')

        fake_token = {'userinfo': {
            'email': email,
            'sub': 'apple_id_abc_789',
            'name': {'firstName': 'Apple', 'lastName': 'User'},
        }}

        # Apple proxy resolves at runtime — patch at app level
        # Skip if Apple is not configured in the test app
        try:
            from app import apple
            apple_configured = apple is not None
        except Exception:
            apple_configured = False

        if not apple_configured:
            self.skipTest('Apple OAuth not configured in test env (no APPLE_CLIENT_ID)')

        with patch('app.apple.authorize_access_token', return_value=fake_token), \
             patch('email_service.send_welcome_email', return_value=True):
            r = self.client.get('/auth/apple/callback?state=fake&code=fake')

        self.assertIn(r.status_code, [200, 302])

        with self.app.app_context():
            user = self.User.query.filter_by(email=email).first()
            self.assertIsNotNone(user, 'User must be created on first Apple sign-in')
            self.assertEqual(user.auth_provider, 'apple')
            self.assertEqual(user.apple_id, 'apple_id_abc_789')

    def test_apple_callback_handles_missing_name(self):
        """Apple often omits the name on subsequent logins (only sent
        on first authorization). Handler must use email prefix as
        fallback name."""
        email = _unique_email('appleNoName')
        ...
        try:
            from app import apple
            if apple is None:
                self.skipTest('Apple OAuth not configured')
        except Exception:
            self.skipTest('Apple OAuth not configured')
        ...
        with self.app.app_context():
            user = self.User.query.filter_by(email=email).first()
            self.assertIsNotNone(user)
            # Name fallback should be the email prefix
            self.assertTrue(user.name,
                'name must be set even when Apple omits it (fallback to email prefix)')
