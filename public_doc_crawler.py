...
class SourceAdapter:
    ...
    def download(self, url: str, doc_info: dict) -> Optional[str]:
        ...
        try:
            ...
            # Network/HTTP errors from external sites are expected — warn, don't error
            import requests as _req
            if isinstance(e, (_req.RequestException, _req.HTTPError,
                              _req.ConnectionError, _req.Timeout,
                              _req.TooManyRedirects)):
                logger.warning(f"Download failed for {url}: {e}")
            else:
                # Unexpected error (programming bug, disk full, etc.)
                http_errs = ('403', '404', '429', '503', '500', 'timeout',
                             'connection', 'ssl', 'timed out', 'refused')
                err_str = str(e).lower()
                if any(k in err_str for k in http_errs):
                    logger.warning(f"Download failed for {url}: {e}")
                else:
    ...
    def scan(self) -> list:
        ...
        for query in self.SEARCH_QUERIES:
            try:
                ...
                logger.warning(f"CourtListener scan error for '{query}': {e}")
                continue
        ...
        for state_code, source in self.STATE_SOURCES.items():
            ...
            if source['active']:
                ...
                try:
                    ...
                    logger.warning(f"State scrape failed for {state_code}: {e}")
    ...
    def scan(self) -> list:
        ...
        for source in self.SOURCES:
            try:
                ...
                logger.warning(f"Open data scan error for {source['name']}: {e}")
                continue
    ...
    def scan_all(self) -> dict:
        ...
        for source in self.sources:
            try:
                ...
                logger.warning(f"Scan failed for {source.name}: {e}")
