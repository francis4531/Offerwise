...
class RentCastTool(ResearchTool):
    ...
    def execute(self, address: str, profile: PropertyProfile) -> ToolResult:
        ...
        try:
            ...
            logger.warning(f"[MARKET] RentCast /avm/value HTTP {resp.status_code} in {int((time.time()-start)*1000)}ms for {address[:40]}")
            if resp.status_code == 404:
                logger.warning(f"[MARKET] RentCast 404 — address not found in database")
                return ToolResult(
                    tool_name=self.name, status=ToolStatus.SUCCESS,
                    data={'found': False},
                    duration_ms=int((time.time() - start) * 1000),
            ...
            if resp.status_code != 200:
                logger.warning(f"[MARKET] RentCast error body: {resp.text[:200]}")
                return ToolResult(
                    tool_name=self.name, status=ToolStatus.FAILED,
...
class MarketStatsTool(ResearchTool):
    ...
    def execute(self, address: str, profile: PropertyProfile) -> ToolResult:
        ...
        try:
            ...
            getattr(logger, level)(f"[MARKET] MarketStats /markets HTTP {resp.status_code} in {int((time.time()-start)*1000)}ms for ZIP {zip_code}")
            if resp.status_code != 200:
                body = resp.text[:500]
                logger.warning(f"[MARKET] MarketStats error body: {body}")
                # 402 = quota exceeded, 403 = invalid key, 404 = no data for ZIP, 429 = rate limit
                if resp.status_code == 402:
                    err = f"RentCast quota exceeded (HTTP 402). Check usage at app.rentcast.io/app/api"
                elif resp.status_code == 403:
                    err = f"RentCast API key invalid or expired (HTTP 403)"
                elif resp.status_code == 404:
                    err = f"No market data available for ZIP {zip_code} (HTTP 404)"
                elif resp.status_code == 429:
                    err = f"RentCast rate limit hit (HTTP 429) — retry later"
                else:
                    err = f"Market stats API returned HTTP {resp.status_code}: {body[:100]}"
                return ToolResult(
                    tool_name=self.name, status=ToolStatus.FAILED,
                    error=err,
                    duration_ms=int((time.time() - start) * 1000)
            ...
            # RentCast returns history as either a list of dicts OR a dict keyed by month string
            # Normalize to a sorted list regardless of format
            raw_history = sale.get('history') or data.get('history') or []
            if isinstance(raw_history, dict):
                # Dict format: {"2025-01": {...}, "2025-02": {...}}
                history_list = []
                for month_key, month_data in raw_history.items():
                    if isinstance(month_data, dict):
                        entry = dict(month_data)
                        entry.setdefault('month', month_key)
                        history_list.append(entry)
                history_list.sort(key=lambda x: x.get('month', ''))
            elif isinstance(raw_history, list):
                history_list = [h for h in raw_history if isinstance(h, dict)]
            else:
                history_list = []

            monthly_history = []
            if len(history_list) >= 2:
                recent = history_list[-1]
                prior = history_list[-2]
                ...
                pd_val = float(prior.get('averageDaysOnMarket', 0) or 0)
                if pd_val > 0:
                    market_data['dom_trend_pct'] = round((rd - pd_val) / pd_val * 100, 1)
            ...
            err = f"RentCast market stats timed out after 10s for ZIP {zip_code}"
            logger.warning(f"[MARKET] {err}")
            return ToolResult(
                tool_name=self.name, status=ToolStatus.FAILED,
                error=err, duration_ms=int((time.time() - start) * 1000)
            )
        except requests.exceptions.SSLError as e:
            err = f"RentCast SSL error (Render may be blocking api.rentcast.io): {type(e).__name__}: {str(e)[:150]}"
            logger.warning(f"[MARKET] {err}")
            return ToolResult(
                tool_name=self.name, status=ToolStatus.FAILED,
                error=err, duration_ms=int((time.time() - start) * 1000)
            ...
            err = f"RentCast connection error (network block?): {type(e).__name__}: {str(e)[:150]}"
            logger.warning(f"[MARKET] {err}")
            return ToolResult(
                tool_name=self.name, status=ToolStatus.FAILED,
                error=err, duration_ms=int((time.time() - start) * 1000)
            ...
            logger.warning(f"[MARKET] Unexpected error for ZIP {zip_code}: {err}")
            return ToolResult(
                tool_name=self.name, status=ToolStatus.FAILED,
                error=err, duration_ms=int((time.time() - start) * 1000)
            )
...
Be specific and actionable. Reference actual data points. No generic advice."""
            ...
            _t0 = time.time()
            # Use resilient ai_client with OpenAI fallback
            try:
                from ai_client import get_ai_response as _get_ai
                _synth_text = _get_ai(prompt, max_tokens=1000, temperature=0)
                # Wrap in compatible response object
                response = type("_R", (), {
                    "content": [type("_C", (), {"text": _synth_text})()],
                    "usage": type("_U", (), {"input_tokens": 0, "output_tokens": 0})()
                })()
            except RuntimeError:
                # Both providers failed — skip synthesis, return raw data
                raise
            try:
                try:
            ...
            # OverloadedError (529) is expected during high traffic — not a bug
            err_name = type(e).__name__
            if err_name in ('OverloadedError', 'RateLimitError', 'APIStatusError'):
                logger.warning(f"AI synthesis skipped (expected): {err_name} — returning raw data only")
            else:
