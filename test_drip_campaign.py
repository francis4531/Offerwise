...
class TestEmailTemplates:
    ...
    def test_email_2_education(self):
        ...
        assert 'TDS' in html or 'Transfer Disclosure' in html

    def test_email_3_case_study(self):
        _, html = drip_email_3(FakeEntry())
        assert '$23' in html

    def test_email_4_urgency_with_address(self):
    ...
    def test_email_5_last_chance(self):
        subj, _ = drip_email_5(FakeEntry())
        assert 'last chance' in subj.lower()
...
class TestSendDripEmail:
    ...
    @patch('email_service.EMAIL_ENABLED', True)
    @patch('email_service.send_email', return_value=True)
    def test_step_5_marks_completed(self, mock_send):
        entry = FakeEntry(drip_step=4)
        send_drip_email(entry, 5)
