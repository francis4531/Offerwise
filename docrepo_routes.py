...
# Lazy-loaded from app to avoid circular imports
def _get_app_attr(name):
    import app as _app
    return getattr(_app, name)

class _LazyStr(str):
    """String proxy that resolves DOCREPO_DISK_PATH from app on first use."""
    _resolved = None
    ...
    def _resolve(self):
        if _LazyStr._resolved is None:
            _LazyStr._resolved = _get_app_attr('DOCREPO_DISK_PATH')
        return _LazyStr._resolved
    ...
    def startswith(self, *a, **k): return self._resolve().startswith(*a, **k)

DOCREPO_DISK_PATH = _LazyStr()

def _resolve_doc_path(doc):
    try:
        return _get_app_attr('_resolve_doc_path')(doc)
    except AttributeError:
        # app._resolve_doc_path not yet defined during startup
        return None

def _extract_text_from_html(filepath):
    return _get_app_attr('_extract_text_from_html')(filepath)
...
def docrepo_catalog():
    ...
    if not os.path.exists(catalog_path):
        catalog = {"documents": [], "collection_stats": {"total_documents": 0}, "sources_tracked": []}
    else:
        try:
    ...
    try:
        # Auto-discover crawled documents from persistent disk
        crawled_base = os.path.join(DOCREPO_DISK_PATH, 'crawled')
        if os.path.isdir(crawled_base):
            existing_ids = {d.get('id') for d in catalog.get('documents', [])}
            for source in os.listdir(crawled_base):  # homeadvisor, permits, fema
                source_dir = os.path.join(crawled_base, source)
                if not os.path.isdir(source_dir):
                    continue
                for date_dir_name in os.listdir(source_dir):
                    date_dir = os.path.join(source_dir, date_dir_name)
                    ...
                    for fname in os.listdir(date_dir):
                        ...
                        if fname.endswith('.meta.json'):
                            continue
                        doc_id = f'crawled_{source}_{date_dir_name}_{fname}'
                        if doc_id in existing_ids:
                            continue
                        fpath = os.path.join(date_dir, fname)
                        # Load metadata if exists
                        meta_path = fpath.replace('.html', '.meta.json').replace('.json', '.meta.json')
                        meta = {}
                        ...
                        catalog.setdefault('documents', []).append({
                            'id': doc_id,
                            'title': f'{source}: {fname}',
                            'source': 'crawled',
                            'source_url': meta.get('source_url', ''),
                            'filename': fname,
                            'file_path': fpath,
                            'document_type': f'crawled_{source}',
                            'category': meta.get('category', ''),
                            'severity': meta.get('severity', ''),
                            'crawled_at': meta.get('fetched_at', ''),
                            'extracted_ranges': {k: v for k, v in meta.items() if k.startswith('cost_')},
                        })

        # Enrich with actual file sizes (checks persistent disk + local)
        total_bytes = 0
        for doc in catalog.get('documents', []):
            # Crawled docs have file_path directly
            if doc.get('source') == 'crawled' and doc.get('file_path'):
                fpath = doc['file_path']
        ...
        if 'collection_stats' in catalog:
            ...
            crawled_count = sum(1 for d in catalog.get('documents', []) if d.get('source') == 'crawled')
            ...
            catalog['collection_stats']['crawled_count'] = crawled_count
            catalog['collection_stats']['total_documents'] = len(catalog.get('documents', []))

        return jsonify(catalog)
    except Exception as e:
        logging.error(f"Error enriching docrepo catalog: {e}")
        return jsonify({"error": "Failed to enrich catalog", "documents": [], "collection_stats": {"total_documents": 0}, "sources_tracked": []}), 500
...
def docrepo_download(doc_id):
    ...
    try:
        ...
        logging.error(f"Error downloading docrepo file {doc_id!r}: {e}")
    ...
    try:
        ...
        with tarfile.open(fileobj=io.BytesIO(archive_bytes), mode='r:gz') as tar:
            for member in tar.getmembers():
                ...
                dest = os.path.realpath(os.path.join(target_dir, rel_path))
                real_target = os.path.realpath(target_dir)
                if not dest.startswith(real_target + os.sep):
                    skipped.append(f"BLOCKED:{rel_path}")
                    continue
