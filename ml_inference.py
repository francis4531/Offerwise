...
class FindingClassifier:
    ...
    def load(self, base_dir=None):
        ...
        # v5.89.55: models live on persistent disk in production so they
        # survive deploys. base_dir still passed for sentence-transformer
        # cache discovery below.
        ...
        try:
            ...
            #
            # v5.89.83: VALIDATE the calibrator's class coverage before
            # trusting it. CalibratedClassifierCV(cv='prefit') fit on the
            # test split silently drops any class absent from that split.
            # A calibrator with fewer classes than the encoder produces
            # confident-WRONG predictions (the dropped class can never be
            # emitted, and argmax indices decode against the wrong table).
            # If counts mismatch, reject the calibrator and use raw XGBoost,
            # which always carries the full class set.
            cat_cal_path = os.path.join(models_dir, 'category_calibrator.pkl')
            sev_cal_path = os.path.join(models_dir, 'severity_calibrator.pkl')
            n_cat_classes = len(self._cat_encoder.classes_)
            n_sev_classes = len(self._sev_encoder.classes_)
            if os.path.exists(cat_cal_path):
                try:
                    with open(cat_cal_path, 'rb') as f:
                        _cal = pickle.load(f)
                    _n_cal = len(getattr(_cal, 'classes_', []))
                    if _n_cal == n_cat_classes:
                        self._cat_calibrator = _cal
                        logger.info("ML inference: category calibrator loaded (isotonic, %d classes)", _n_cal)
                    else:
                        self._cat_calibrator = None
                        logger.warning(
                            "ML inference: REJECTED category calibrator — it has %d classes but encoder has %d. "
                            "Calibrator dropped a category (likely absent from the prefit test split). "
                            "Falling back to raw XGBoost for category prediction.",
                            _n_cal, n_cat_classes)
            ...
            if os.path.exists(sev_cal_path):
                try:
                    with open(sev_cal_path, 'rb') as f:
                        _cal = pickle.load(f)
                    _n_cal = len(getattr(_cal, 'classes_', []))
                    if _n_cal == n_sev_classes:
                        self._sev_calibrator = _cal
                        logger.info("ML inference: severity calibrator loaded (isotonic, %d classes)", _n_cal)
                    else:
                        self._sev_calibrator = None
                        logger.warning(
                            "ML inference: REJECTED severity calibrator — it has %d classes but encoder has %d. "
                            "Falling back to raw XGBoost for severity prediction.",
                            _n_cal, n_sev_classes)
                except Exception as e:
    ...
    def classify(self, finding_text):
        ...
        try:
            ...
            # v5.89.83: decode argmax through the SAME class list that produced
            # the probabilities. For a calibrator, that's calibrator.classes_
            # (encoded integer labels in the calibrator's own order); we map
            # that integer back to a string via the encoder. For raw XGB, the
            # model was fit on encoder-encoded labels so the index aligns with
            # the encoder directly.
            if self._cat_calibrator is not None:
                cat_proba = self._cat_calibrator.predict_proba(embedding)[0]
                cat_argmax = int(np.argmax(cat_proba))
                cat_encoded = int(self._cat_calibrator.classes_[cat_argmax])
                category = self._cat_encoder.inverse_transform([cat_encoded])[0]
                cat_conf = float(cat_proba[cat_argmax])
            else:
                cat_proba = self._cat_model.predict_proba(embedding)[0]
                cat_argmax = int(np.argmax(cat_proba))
                cat_conf = float(cat_proba[cat_argmax])
                category = self._cat_encoder.inverse_transform([cat_argmax])[0]

            if self._sev_calibrator is not None:
                sev_proba = self._sev_calibrator.predict_proba(embedding)[0]
                sev_argmax = int(np.argmax(sev_proba))
                sev_encoded = int(self._sev_calibrator.classes_[sev_argmax])
                severity = self._sev_encoder.inverse_transform([sev_encoded])[0]
                sev_conf = float(sev_proba[sev_argmax])
            else:
                sev_proba = self._sev_model.predict_proba(embedding)[0]
                sev_argmax = int(np.argmax(sev_proba))
                sev_conf = float(sev_proba[sev_argmax])
                severity = self._sev_encoder.inverse_transform([sev_argmax])[0]
    ...
    def classify_batch(self, texts):
        ...
        try:
            ...
            for i in range(len(texts)):
                # v5.89.83: decode through the calibrator's own class list when
                # a calibrator produced the probabilities (see classify()).
                if self._cat_calibrator is not None:
                    cat_argmax = int(np.argmax(cat_probas[i]))
                    cat_conf = float(cat_probas[i][cat_argmax])
                    cat_label = self._cat_encoder.inverse_transform(
                        [int(self._cat_calibrator.classes_[cat_argmax])])[0]
                else:
                    cat_argmax = int(np.argmax(cat_probas[i]))
                    cat_conf = float(cat_probas[i][cat_argmax])
                    cat_label = self._cat_encoder.inverse_transform([cat_argmax])[0]

                if self._sev_calibrator is not None:
                    sev_argmax = int(np.argmax(sev_probas[i]))
                    sev_conf = float(sev_probas[i][sev_argmax])
                    sev_label = self._sev_encoder.inverse_transform(
                        [int(self._sev_calibrator.classes_[sev_argmax])])[0]
                else:
                    sev_argmax = int(np.argmax(sev_probas[i]))
                    sev_conf = float(sev_probas[i][sev_argmax])
                    sev_label = self._sev_encoder.inverse_transform([sev_argmax])[0]
...
class ContradictionDetector:
    ...
    def load(self, base_dir=None, embedder=None):
        ...
        # v5.89.55: persistent-disk-aware path.
        models_dir = get_models_dir()
