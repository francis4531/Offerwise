...
def analyze_property():
    ...
    try:
        # v5.88.32: initialize research_data at function entry to avoid
        # UnboundLocalError on the cache-hit code path. Python sees
        # research_data assigned at lines 502 and 1420 and treats it as
        # function-local. If the cache HIT branch is taken (line 477),
        # neither assignment runs before line 1347 reads research_data,
        # raising "cannot access local variable 'research_data'".
        # Symptom: production users hitting cached analyses got 500 errors;
        # all 5 cassette replay tests failed identically because the
        # cassettes use deterministic inputs that always hit the cache
        # after the first run.
        research_data = None
        ...
        if job_id and (not seller_disclosure_text or not inspection_report_text):
            ...
            if job.status == 'complete' and job.result:
                ...
                # Phase 1b/1d: carry TDS field state if the worker extracted it.
                if job.result.get('tds_field_state') and not _reasoning_tds_field_state:
        ...
        # State 3: address + inspection     → repair costs + risk + offer strategy (NO transparency/disclosure-gap; nothing to cross-reference)
        # State 4: address + both documents → full offer strategy (existing behaviour)
        ...
        analysis_depth = (
            'full' if (has_disclosure and has_inspection)
            else 'disclosure_only' if has_disclosure
            else 'inspection_only' if has_inspection
            else 'address_only'
        )
        ...
        if not cached_result:
                ...
                def _run_research(addr):
                    try:
                        ...
                        for tr in (result.get('tool_results') or []):
                            # v5.88.23: (tr.get('error') or '')[:60] handles the case
                            # where the tool result has 'error': None explicitly.
                            # The previous tr.get('error', '')[:60] crashed because
                            # dict.get returns None (not the default '') when the
                            # key exists with value None. This crash was masking
                            # research results across all analyses since v5.59.x.
                            _err = (tr.get('error') or '')
                            logging.warning(f"[MARKET]   tool={tr.get('tool_name')} status={tr.get('status')} ms={tr.get('duration_ms',0)} err={_err[:60]}")
                        return result
                    except Exception as e:
            ...
            if analysis_depth == 'address_only':
                ...
                # disclosure_only, inspection_only, or full — run intelligence engine
                ...
                elif analysis_depth == 'inspection_only':
                    _sp(sse_job_id, 'parsing',      'Parsing inspection report...', 15)
                    _sp(sse_job_id, 'research',     'Querying public records...', 30)
                    _sp(sse_job_id, 'repair_costs', 'Calculating repair costs...', 55)
                    _sp(sse_job_id, 'risk_dna',     'Building Property Risk DNA™...', 75)
        ...
        if analysis_depth != 'address_only' and result_dict.get('risk_score'):
            try:
                ...
                # v5.87.82: Permit lookup for each repair category. Single
                # batched Claude call (90-day cached). Adds permit_findings
                # block to the analysis result with per-category permit
                # status, cost, and consequences.
                try:
                    from permit_lookup import lookup_permits
                    # Resolve jurisdiction robustly: (1) parse the address, which
                    # may or may not be comma-delimited, then (2) fall back to the
                    # ZIP to recover the state when the parse is incomplete. This
                    # avoids the "Unknown jurisdiction" degrade when the address
                    # isn't in the exact "St, City, ST 12345" shape.
                    ...
                    # and no-comma forms), else recover from the ZIP.
                    _state = ''
                    _state_tok = _re.search(r'\b([A-Z]{2})\b(?=\s*\d{5}|\s*$|,)', _addr_str)
                    if _state_tok:
                        _state = _state_tok.group(1)
                    if not _state and _zip:
                        try:
                            from state_disclosures import detect_state_from_zip
                            _state = detect_state_from_zip(_zip) or ''
                        except Exception:
                            pass

                    # City: best-effort from a comma-delimited address; the
                    # segment just before the state/zip tail. Blank is acceptable
                    # — the lookup can key on state + ZIP without it.
                    _city = ''
                    _parts = [p.strip() for p in _addr_str.split(',') if p.strip()]
                    if len(_parts) >= 3:
                        _city = _parts[-2]
                    elif len(_parts) == 2:
                        # "City ST 12345" tail in the 2nd part, or "Street, City"
                        _tail = _parts[-1]
                        _city = _re.sub(r'\b[A-Z]{2}\b.*$', '', _tail).strip() or _parts[0]

                    juris = {
                        'state': result_dict.get('state') or _state,
                        'county': result_dict.get('county') or '',
                        'city': result_dict.get('city') or _city,
                        'zip': _zip,
                    }
                    if not juris['state'] and not juris['zip']:
                        logging.warning(
                    ...
                    logging.info(
                        f"📋 Permit findings attached: ${permit_findings['total_low']:,}–"
                        f"${permit_findings['total_high']:,} permit fees, "
                        f"{permit_findings['cache_hits']} hits / "
                        f"{permit_findings['cache_misses']} misses"
                    )
                except Exception as permit_err:
                    logging.warning(f"📋 Could not generate permit findings: {permit_err}")
        ...
        # Phase 1d (flagged OFF by default): attach checklist-driven reasoning.
