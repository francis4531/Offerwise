...
def contractor_claim_lead(lead_id):
    ...
    # ── FLYWHEEL: structured scope card (v5.87.40) ────────────────────────
    # Complements the inline contact-info email above with a designed
    # lead-card showing the repair scope, severity, cost range, and quality
    # score. Contact email = "here's how to call them"; flywheel email =
    # "here's what to bid on."
    try:
        from flywheel_notifications import send_contractor_lead_email
        send_contractor_lead_email(lead.id)
    except Exception as e:
        logging.warning(f"Flywheel lead email failed (non-fatal): {e}")
...
# ═══════════════════════════════════════════════════════════════════════════════
# PUBLIC LEAD SUBMISSION — v5.88.87 D
# ═══════════════════════════════════════════════════════════════════════════════
# Briefing and inspector-report share pages are seen by buyers who haven't
# signed up for OfferWise — they got the link from their agent or inspector.
# This endpoint lets those buyers submit a contractor-lead request without
# logging in, identified by (share_token + email + phone). The lead is
# attributed to the source (briefing / inspector report) so OfferWatch can
# link the contractor's claim back to the property via PropertyWatch.
# contractor_lead_id, completing the four-persona connection on a single
# property record.
#
# Auth model: anyone who has the share_token can submit a lead on that
# share. This is acceptable because (a) the share token is unguessable
# (32 chars), (b) the lead exposes only what the share page already
# showed, and (c) the contractor marketplace already has admin review
# before leads route to contractors.

@contractor_bp.route('/api/contractor-lead/public', methods=['POST'])
def submit_public_contractor_lead():
    """Buyer (un-authed) submits a contractor lead from a share page.

    Required body fields:
      share_token      — the briefing or inspector-report token (identifies the source)
      source_type      — 'briefing' | 'inspector_report'
      user_email       — buyer's email
      repair_system    — e.g. 'roof', 'hvac' (matches the analysis category)

    Optional:
      user_name, user_phone, trade_needed, cost_estimate, issue_description,
      contact_timing ('asap' | 'this_week' | 'just_exploring')

    The endpoint resolves share_token to the source row, pulls
    property_address from it, dedups against recent leads with the same
    email + address + repair_system, creates a ContractorLead, and links
    it back to any matching PropertyWatch.
    """
    data = request.get_json() or {}
    ...
    repair_system = (data.get('repair_system') or '').strip()

    # Basic validation
    if not share_token or not source_type or not user_email or not repair_system:
        return jsonify({'error': 'Missing required fields (share_token, source_type, user_email, repair_system)'}), 400
    if source_type not in ('briefing', 'inspector_report'):
        return jsonify({'error': 'source_type must be "briefing" or "inspector_report"'}), 400
    if '@' not in user_email or len(user_email) > 255:
        return jsonify({'error': 'Invalid email'}), 400

    # Rate limit by email — at most 10 lead submissions per email per hour.
    # Prevents abuse but allows a legitimate buyer to request multiple
    # quotes across multiple findings on one property.
    from datetime import timedelta as _td_pub
    one_hour_ago = datetime.utcnow() - _td_pub(hours=1)
    recent_by_email = ContractorLead.query.filter(
        ContractorLead.user_email == user_email,
        ContractorLead.created_at >= one_hour_ago,
    ).count()
    if recent_by_email >= 10:
        return jsonify({'error': 'Too many requests. Please try again later.'}), 429

    # Resolve share_token → property_address + linkage IDs
    property_address = ''
    property_price = None
    inspector_report_id = None
    agent_briefing_id = None
    try:
        if source_type == 'briefing':
            from models import AgentBriefing
            br = AgentBriefing.query.filter_by(share_token=share_token).first()
            if not br:
                return jsonify({'error': 'Briefing not found'}), 404
            property_address = br.property_address or ''
            property_price = br.property_price
            agent_briefing_id = br.id
        else:  # inspector_report
            rep = InspectorReport.query.filter_by(share_token=share_token).first()
            if not rep:
                return jsonify({'error': 'Report not found'}), 404
            property_address = rep.property_address or ''
            property_price = rep.property_price
            inspector_report_id = rep.id
    except Exception as e:
        logging.error(f"Public lead share_token resolve failed: {e}")
    ...
    # Dedup: same email + address + repair within 24h
    recent_cutoff = datetime.utcnow() - _td_pub(hours=24)
    dupe = ContractorLead.query.filter(
        ContractorLead.user_email == user_email,
        ContractorLead.property_address == property_address,
        ContractorLead.repair_system == repair_system,
        ContractorLead.created_at >= recent_cutoff,
    ).first()
    if dupe:
        logging.info(f"Duplicate public lead suppressed for {user_email} ({repair_system} at {property_address})")
        return jsonify({'success': True, 'lead_id': dupe.id, 'duplicate': True})

    # Resolve user_id if the buyer has an account (so the lead links to
    # their User row when they do sign up later). Match by email.
    user_id = None
    try:
        from models import User as _U
        u = _U.query.filter_by(email=user_email).first()
        if u:
            user_id = u.id
    except Exception:
        pass
    ...
    lead = ContractorLead(
        ...
        user_name         = (data.get('user_name') or '').strip()[:255],
        user_phone        = (data.get('user_phone') or '').strip()[:50],
        property_address  = property_address[:500],
        property_zip      = zip_code,
        repair_system     = repair_system[:100],
        trade_needed      = (data.get('trade_needed') or '').strip()[:100],
        cost_estimate     = (data.get('cost_estimate') or '').strip()[:100],
        issue_description = (data.get('issue_description') or '')[:2000],
        contact_timing    = (data.get('contact_timing') or 'this_week')[:50],
        status            = 'available',
        available_at      = _now,
        expires_at        = _now + _td_pub(hours=48),
        claim_count       = 0,
    )
    db.session.add(lead)
    db.session.commit()

    logging.info(f"🔧 NEW PUBLIC LEAD #{lead.id}: {lead.repair_system} from {lead.user_email} via {source_type}/{share_token[:8]}")

    # Link the lead back to a PropertyWatch for this source if one exists.
    # That way OfferWatch shows the contractor link and the four-persona
    # connection on a single property record is complete.
    try:
        watch_q = PropertyWatch.query.filter_by(is_active=True)
        if agent_briefing_id:
            watch = watch_q.filter_by(agent_briefing_id=agent_briefing_id).first()
        elif inspector_report_id:
            watch = watch_q.filter_by(inspector_report_id=inspector_report_id).first()
        else:
            watch = None
        if watch and not watch.contractor_lead_id:
            watch.contractor_lead_id = lead.id
            db.session.commit()
            logging.info(f"🔗 Linked ContractorLead #{lead.id} → PropertyWatch #{watch.id}")
    except Exception as we:
        logging.warning(f"Watch link for public lead {lead.id} failed (non-fatal): {we}")

    # Email admin (same shape as authenticated path so the admin workflow
    # is unchanged)
    try:
        timing_labels = {'asap': '🔴 ASAP', 'this_week': 'This week', 'just_exploring': 'Just exploring'}
        timing = timing_labels.get(lead.contact_timing, lead.contact_timing)
        html = f"""
        <div style="font-family:sans-serif;max-width:600px;margin:0 auto;background:#0f172a;color:#f1f5f9;padding:24px;border-radius:12px;">
          <div style="font-size:22px;font-weight:800;margin-bottom:16px;color:#f97316;">🔧 New Contractor Lead #{lead.id} (public)</div>
          <table style="width:100%;border-collapse:collapse;">
            <tr><td style="padding:8px 0;color:#94a3b8;font-size:13px;width:140px;">Source</td><td style="padding:8px 0;font-weight:700;text-transform:capitalize;">{source_type.replace('_', ' ')}</td></tr>
            <tr><td style="padding:8px 0;color:#94a3b8;font-size:13px;">Repair needed</td><td style="padding:8px 0;font-weight:700;text-transform:capitalize;">{lead.repair_system}</td></tr>
            <tr><td style="padding:8px 0;color:#94a3b8;font-size:13px;">Trade</td><td style="padding:8px 0;">{lead.trade_needed or '—'}</td></tr>
            <tr><td style="padding:8px 0;color:#94a3b8;font-size:13px;">Est. cost</td><td style="padding:8px 0;color:#f59e0b;font-weight:700;">{lead.cost_estimate or '—'}</td></tr>
            <tr><td style="padding:8px 0;color:#94a3b8;font-size:13px;">Property</td><td style="padding:8px 0;">{lead.property_address}</td></tr>
            <tr><td style="padding:8px 0;color:#94a3b8;font-size:13px;">ZIP</td><td style="padding:8px 0;">{lead.property_zip or '—'}</td></tr>
            <tr><td style="padding:8px 0;color:#94a3b8;font-size:13px;">User</td><td style="padding:8px 0;">{lead.user_name or '—'} · {lead.user_email}</td></tr>
            <tr><td style="padding:8px 0;color:#94a3b8;font-size:13px;">Phone</td><td style="padding:8px 0;font-weight:700;color:#22c55e;">{lead.user_phone or '(not provided)'}</td></tr>
            <tr><td style="padding:8px 0;color:#94a3b8;font-size:13px;">Timing</td><td style="padding:8px 0;font-weight:700;">{timing}</td></tr>
          </table>
          {f'<div style="margin-top:12px;padding:12px;background:rgba(255,255,255,0.05);border-radius:8px;font-size:13px;color:#94a3b8;">{lead.issue_description}</div>' if lead.issue_description else ''}
          <div style="margin-top:16px;font-size:11px;color:#475569;">Lead ID #{lead.id} · {lead.created_at.strftime('%Y-%m-%d %H:%M')} UTC · share_token {share_token[:8]}…</div>
        </div>
        """
        send_email(
            to_email=ADMIN_EMAIL,
            subject=f"🔧 Public Lead #{lead.id}: {lead.repair_system.title()} — {lead.property_address[:40]}",
            html_content=html,
        )
    except Exception as e:
        logging.error(f"Failed to send admin public-lead email: {e}")

    return jsonify({
        'success': True,
        'lead_id': lead.id,
        'message': 'Quote request submitted. A licensed contractor in your area will reach out within 48 hours.',
    })
