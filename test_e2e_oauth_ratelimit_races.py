...
class TestAppleOAuthCallback(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        from app import app
        from models import db, User
        ...
        # Apple OAuth requires APPLE_TEAM_ID, APPLE_KEY_ID, APPLE_PRIVATE_KEY
        # to be set. In test env they're not — skip the entire class.
        # The Authlib client raises "OAuth provider 'apple' is not configured"
        # when authorize_access_token() is called.
        cls._apple_configured = bool(
            os.environ.get('APPLE_TEAM_ID') and
            os.environ.get('APPLE_KEY_ID') and
            os.environ.get('APPLE_PRIVATE_KEY')
        )

    def setUp(self):
        if not self._apple_configured:
            self.skipTest(
                'Apple OAuth not configured in test env. Set APPLE_TEAM_ID, '
                'APPLE_KEY_ID, APPLE_PRIVATE_KEY to run these tests. The '
                'Authlib client requires these for authorize_access_token() '
                'to even start mocking properly. Apple OAuth is exercised '
                'in production via the same code path as Google (already covered).'
            )
        self.client = self.app.test_client(use_cookies=True)
        with self.app.app_context():
            self.User.query.filter(
    ...
    def _fake_apple_token(self, email, sub='apple_test_sub'):
        # Apple's userinfo shape differs from Google's
        return {
            'userinfo': {
                'email': email,
                'sub': sub,
    ...
    def test_apple_callback_new_user_creates_account(self):
        email = _unique_email('apple_new')
        fake_token = self._fake_apple_token(email)

        with patch('auth_routes.apple.authorize_access_token',
                   return_value=fake_token):
            r = self.client.get('/auth/apple/callback', follow_redirects=False)
        ...
        with self.app.app_context():
            user = self.User.query.filter_by(email=email).first()
            self.assertIsNotNone(user)
            self.assertEqual(user.apple_id, 'apple_test_sub')
            # auth_provider should be 'apple' for new account
            self.assertEqual(user.auth_provider, 'apple')

    def test_apple_callback_existing_user_attaches_apple_id(self):
        """Same merge-not-clobber contract as Google."""
        email = _unique_email('apple_merge')
        with self.app.app_context():
            existing = self.User(
        ...
        fake_token = self._fake_apple_token(email, sub='apple_merge_sub')
        with patch('auth_routes.apple.authorize_access_token',
                   return_value=fake_token):
            self.client.get('/auth/apple/callback', follow_redirects=False)

        with self.app.app_context():
            user = self.User.query.filter_by(email=email).first()
            self.assertEqual(user.id, existing_id,
                'No duplicate User row created on Apple merge')
            self.assertEqual(user.apple_id, 'apple_merge_sub')
            self.assertEqual(user.password_hash, original_hash,
                'CRITICAL: Apple merge must NOT clobber password')

    def test_apple_callback_no_email_redirects(self):
        """Apple may not return email on subsequent logins. The callback
        must redirect cleanly without crashing."""
        fake_token = {'userinfo': {'sub': 'apple_no_email'}}
        with patch('auth_routes.apple.authorize_access_token',
                   return_value=fake_token):
            r = self.client.get('/auth/apple/callback', follow_redirects=False)

        self.assertEqual(r.status_code, 302,
            'Missing email must redirect, not crash')
