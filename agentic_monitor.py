...
def _send_with_id(to_email, subject, html, email_type='agent_alert', user_id=None):
    """v5.88.86: alert-specific send that returns the Resend message id so the
    caller can store it on the AgentAlert row. This is the link that lets the
    /webhook/resend handler attribute opens and clicks back to a specific
    alert (via EmailEvent.resend_id == AgentAlert.resend_id).

    Returns the resend_id string on success, or None on failure / when email is
    disabled. We look it up from EmailSendLog after the send rather than
    changing send_email's return contract (which is widely used and checked
    for truthiness in many places). The lookup window is tight (the most
    recent matching row in the last 60s) so cross-contamination between
    parallel sends is not possible at our scale.
    """
    try:
        from email_service import send_email
        ok = send_email(to_email=to_email, subject=subject, html_content=html,
                        email_type=email_type, user_id=user_id)
        if not ok:
            return None
        # Look up the resend_id from the EmailSendLog row we just wrote.
        from models import EmailSendLog
        from datetime import timedelta
        log = EmailSendLog.query.filter(
            EmailSendLog.to_email == to_email,
            EmailSendLog.email_type == email_type,
            EmailSendLog.ts >= _now() - timedelta(seconds=60),
        ).order_by(EmailSendLog.ts.desc()).first()
        return log.resend_id if log else None
    except Exception as e:
        logger.warning(f"Alert email (with id) failed to {to_email}: {e}")
        return None

def _save_alert(watch_id, user_id, alert_type, severity, title, body, detail, email_sent=False, resend_id=None):
    """Persist an alert and optionally mark email sent.

    v5.88.86: when email_sent=True and resend_id is not explicitly passed,
    auto-discover it by looking up the most recent EmailSendLog row for this
    user_id with email_type='agent_alert' in the last 60 seconds. This lets
    every call site flip from un-instrumented to fully-tracked without
    touching the call site itself — the alert email was just sent, the
    EmailSendLog row was just written by email_service.send_email, so a
    bounded lookup is reliable. The /webhook/resend handler later writes
    EmailEvent rows against the same resend_id; opens and clicks are then
    attributable to specific alerts via that join.
    """
    try:
        from models import AgentAlert
        db = _get_db()

        # Auto-discover resend_id from EmailSendLog if email was sent.
        if email_sent and not resend_id:
            try:
                from models import EmailSendLog
                from datetime import timedelta
                log = EmailSendLog.query.filter(
                    EmailSendLog.user_id == user_id,
                    EmailSendLog.email_type == 'agent_alert',
                    EmailSendLog.ts >= _now() - timedelta(seconds=60),
                ).order_by(EmailSendLog.ts.desc()).first()
                if log:
                    resend_id = log.resend_id
            except Exception:
                pass  # non-fatal, alert still saves without telemetry link
...
# v5.88.87 B: permit type classification. Routine permits (driveways,
# fences, pool resurfacing, solar) shouldn't trigger any alert — they're
# normal property maintenance and the "🚩 emergency repairs" framing the
# old code used was wrong 90% of the time, producing alert blindness.
# Repair-related permits (electrical, plumbing, roofing, foundation,
# structural, HVAC) DO warrant an alert because they may correspond to
# inspection findings. Unknown types get an informational alert with
# softer framing.

_PERMIT_REPAIR_KEYWORDS = (
    'electrical', 'plumbing', 'roof', 'roofing', 'foundation', 'structural',
    'hvac', 'heating', 'cooling', 'water heater', 'sewer', 'gas line',
    'mechanical', 'furnace', 'air conditioning', 'mold', 'remediation',
    'leak', 'rewire', 'repipe', 'retrofit',
)

_PERMIT_ROUTINE_KEYWORDS = (
    'driveway', 'fence', 'pool resurface', 'pool deck', 'solar', 'photovoltaic',
    'landscape', 'shed', 'patio cover', 'awning', 'mailbox', 'paint',
    'window replacement only', 'irrigation', 'sprinkler',
)


def _classify_permit(permit_type: str, description: str = '') -> str:
    """Return one of: 'repair', 'routine', 'unknown'.

    'repair'  → fire alert at 'warning' severity with full framing
    'routine' → do not alert at all; these are normal property maintenance
    'unknown' → fire alert at 'info' severity with soft framing
    """
    combined = f"{permit_type or ''} {description or ''}".lower()
    if not combined.strip():
        return 'unknown'
    for kw in _PERMIT_REPAIR_KEYWORDS:
        if kw in combined:
            return 'repair'
    for kw in _PERMIT_ROUTINE_KEYWORDS:
        if kw in combined:
            return 'routine'
    return 'unknown'


def _within_alert_cooldown(watch_id: int, alert_type: str, hours: int = 24) -> bool:
    """v5.88.87 B: frequency cap — return True if the same alert_type was
    already fired for this watch within the cooldown window. Prevents
    alert-spamming the same recipient with daily repeated signals
    (e.g., a price still being down, comps still being below) and is
    independent of the threshold logic itself. Default 24h matches the
    daily cadence of the monitors.
    """
    try:
        from models import AgentAlert
        cutoff = _now() - timedelta(hours=hours)
        recent = AgentAlert.query.filter(
            AgentAlert.watch_id == watch_id,
            AgentAlert.alert_type == alert_type,
            AgentAlert.created_at >= cutoff,
        ).first()
        return recent is not None
    except Exception:
        return False  # fail-open: if check fails, allow the alert through
...
def _notify_linked_professionals(watch, title, body_html, address, emoji="🔔"):
    ...
    """
    try:
        from models import InspectorReport, AgentShare, AgentBriefing, ContractorLead, Contractor, User, AgentAlert
        db = _get_db()
        ...
        # Agent linked to this watch via legacy AgentShare
        ...
        # v5.88.85: agent linked to this watch via AgentBriefing (v0 product).
        # Briefings produce the same OfferWatch surface as legacy shares now.
        if getattr(watch, 'agent_briefing_id', None):
            briefing = AgentBriefing.query.get(watch.agent_briefing_id)
            if briefing:
                agent_user = User.query.get(briefing.agent_user_id)
                if agent_user and agent_user.email:
                    recipients.append((agent_user.email, agent_user.id, 'agent'))
...
def _check_comps_for_watch(watch, db):
    ...
    # v5.88.87 B: frequency cap — at most one new_comp alert per watch per
    # 24h, regardless of whether the threshold logic would fire again.
    if _within_alert_cooldown(watch.id, 'new_comp', hours=24):
        # Still update the check timestamp so the next-day cadence is preserved
    ...
    # v5.88.87 B: tightened from "any one comp 3% below" to "2+ comps below, OR
    # a single comp 5%+ below." Rationale: a single outlier sale (distressed
    # transaction, family transfer, anomaly) was triggering alerts that didn't
    # represent real market movement. Multiple comps below asking is a true
    # market signal; a deep single discount is meaningful enough on its own.
    asking = watch.asking_price
    below_3pct = [c for c in new_comps if c['sale_price'] and c['sale_price'] < asking * 0.97]
    below_5pct = [c for c in new_comps if c['sale_price'] and c['sale_price'] < asking * 0.95]

    if len(below_3pct) >= 2 or len(below_5pct) >= 1:
        below_comps = below_3pct
    else:
        return
...
def _check_earthquake_for_watch(watch, db):
    ...
    # v5.88.87 B: frequency cap on earthquake alerts. Aftershock clusters
    # can produce many M4.5+ events in 24h around a major quake — without
    # this, the user would get one alert per shock. One alert per watch
    # per 24h is the right cadence; if the shock pattern continues
    # tomorrow, they'll get an updated alert then.
    if _within_alert_cooldown(watch.id, 'earthquake', hours=24):
    ...
    try:
        resp = requests.get(
            ...
            params={
                ...
                'maxradiuskm': 50,
                # v5.88.87 B: minmagnitude raised from 4.0 to 4.5. M4.0
                # within 50km is technically "felt" but rarely causes
                # structural damage; alerts at this level produced interest
                # but were "interesting trivia" rather than actionable. M4.5
                # is the threshold at which damage to sensitive structures
                # becomes possible — and below it, the existing seismic
                # reanalysis path already gates ("magnitude >= 5.0 or
                # struct_score >= 40" in _run_seismic_reanalysis), so we
                # were producing alerts without backing deep analysis.
                'minmagnitude': 4.5,
                'starttime': yesterday,
                'limit': 5,
                'orderby': 'magnitude',
...
def _check_price_for_watch(watch, db):
    ...
    # v5.88.87 B: threshold raised from 2.0% to 3.0%. RentCast AVMs jitter
    # ±1-2% from day-to-day variance even with no material change. Firing at
    # 2% pulled a lot of "the AVM noise floor moved" alerts into the system.
    # 3% is just above that noise floor; below it, the signal isn't reliable
    # enough to be worth an alert.
    if drop_pct < 3.0:
        return

    # v5.88.87 B: frequency cap — once-per-watch-per-24h ceiling. A price
    # that stays down doesn't warrant a daily reminder; the user already
    # got the signal.
    if _within_alert_cooldown(watch.id, 'price_drop', hours=24):
        return
...
def _check_permits_for_watch(watch, db):
    ...
    for p in permits:
        ...
        if pid and pid not in baseline_ids:
            ptype = p.get('type', p.get('permit_type', 'Unknown'))
            pdesc = p.get('description', p.get('work_description', ''))[:120]
            classification = _classify_permit(ptype, pdesc)
            # v5.88.87 B: drop routine permits entirely — driveway, fence,
            # landscape, solar, paint, etc. are normal property maintenance
            # and don't warrant alerting the buyer.
    ...
    # v5.88.87 B: frequency cap — at most one permit alert per watch per 24h.
    # Permit databases occasionally backfill multiple historical entries in
    # one batch; without the cap, the buyer gets blasted.
    if _within_alert_cooldown(watch.id, 'new_permit', hours=24):
        return
    ...
    # v5.88.87 B: severity tiering. If any new permit is repair-related,
    # severity is 'warning' (this may correspond to an inspection finding
    # the seller is now addressing). If all are unknown type, severity is
    # 'info' (informational, no urgency framing).
    has_repair = any(p['classification'] == 'repair' for p in new_permits)
    severity = 'warning' if has_repair else 'info'

    if has_repair:
        title = f"{len(new_permits)} new repair permit{'s' if len(new_permits)>1 else ''} filed on this property"
    else:
    ...
    # v5.88.87 B: rewritten framing. The old copy was "🚩 Red flag:
    # emergency repairs before closing — possibly for issues not fully
    # disclosed" which assumed bad seller intent. Most permits aren't
    # red flags. New framing is informational with a clear suggestion to
    # review against inspection findings — useful without crying wolf.
    if has_repair:
        body_html = f"""
    <div style='padding:10px 14px;background:rgba(249,115,22,.08);border:1px solid rgba(249,115,22,.2);border-radius:8px;margin-bottom:14px;'>
      A new repair permit was filed on this property after your analysis. Repairs filed during escrow may relate to inspection findings — worth reviewing what's being addressed.
    </div>
    {permit_rows}
    <p style='font-size:12px;color:#64748b;margin-top:12px;'>Suggested: ask the seller (or their agent) what the permit covers. If it relates to an inspection finding, this may strengthen your position on credits or re-inspection.</p>"""
    else:
        body_html = f"""
    <div style='padding:10px 14px;background:rgba(96,165,250,.07);border:1px solid rgba(96,165,250,.18);border-radius:8px;margin-bottom:14px;'>
      A new permit was filed on this property. Most permits during escrow are routine, but worth a quick check.
    </div>
    {permit_rows}
    <p style='font-size:12px;color:#64748b;margin-top:12px;'>Suggested: ask the seller (or their agent) what the permit covers if the work isn't clear from the description.</p>"""
    ...
    # v5.88.87 B: subject line no longer leads with 🚩 Alert. For repair
    # permits the subject says "Repair permit filed" (informative). For
    # other permits it says "Permit filed" (low-key). The emoji in the
    # body is preserved for visual identification.
    _subject = (
        f"🔨 Repair permit filed on {watch.address[:40]}"
        if has_repair else
        f"🔨 Permit filed on {watch.address[:40]}"
    )
    _send(user.email,
          _subject,
          _email_html(title, body_html, watch.address, "🔨", cta_label="Open OfferWise →", cta_url=_analysis_url),
          user_id=user.id)

    # Notify linked professionals — inspector especially when permit is
    # repair-related, since it may relate to their findings.
    if has_repair:
        pro_title = f"Repair permit filed on your client's property: {watch.address[:40]}"
        pro_body  = f"""
    <p style='margin-bottom:12px;'><strong>{len(new_permits)} new repair permit{'s' if len(new_permits)>1 else ''}</strong> filed on this property after your client's inspection. This may relate to findings in your report.</p>
    {permit_rows}
    <div style='padding:12px 14px;background:rgba(249,115,22,.08);border:1px solid rgba(249,115,22,.2);border-radius:8px;margin-bottom:10px;'>
      Review these permit types against your inspection findings. If the work matches issues you flagged, this validates your report and may strengthen your client's negotiation position.
    </div>"""
        _notify_linked_professionals(watch, pro_title, pro_body, watch.address, "🔨")
    # For non-repair permits, skip the professional notification entirely —
    # not signal worth their attention.

    # v5.88.87 B: severity tied to classification, not hardcoded 'critical'.
    _save_alert(watch.id, user.id, 'new_permit', severity, title,
