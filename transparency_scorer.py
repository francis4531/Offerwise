...
class SellerTransparencyScorer:
    ...
    def _analyze_omissions(
        ...
        non_issue_patterns = [
            ...
            # v5.59.22: Inspection report TEMPLATE/BOILERPLATE language
            re.compile(r'\b(items that (inevitably|may) lead to|adverse impact on the value|unreasonable risk.{0,20}(unsafe|to people|to property))\b', re.IGNORECASE),
            # Generic safety boilerplate without specifics
            re.compile(r'^this poses a safety hazard', re.IGNORECASE),
            # Meta-descriptions of report methodology
            re.compile(r'\b(will be called out in the report|will be (noted|documented|reported)|are (noted|documented|reported) (in|throughout|elsewhere))\b', re.IGNORECASE),
            # Inspection access/limitation disclaimers
            re.compile(r'\b(not readily accessible|could cause damage|in the inspector.s opinion|unable to (access|inspect|evaluate|observe)|was not (accessible|visible|available)|inaccessible (area|space|location)|enter the attic or any)\b', re.IGNORECASE),
            # UI/navigation artifacts from digital reports
            re.compile(r'\b(click here to view|view on web|click to (view|see|open)|video \(click)\b', re.IGNORECASE),
            # Report structure/section headers parsed as findings
            re.compile(r'^\d+\.\d+\.\d+\s+[A-Z]', re.IGNORECASE),
            # Generic "what this means" explanations
            re.compile(r'\b(may seem like a minor|more serious .{0,30} will be|large amounts of .{0,20} will be)\b', re.IGNORECASE),
            # v5.72.8: Inspector methodology boilerplate
            re.compile(r'\b(inspector will only|will only make elaborating|only make elaborating comments|if more nefarious items|nefarious items are noted)\b', re.IGNORECASE),
            # v5.72.8: Positive/neutral material descriptions (not defects)
            re.compile(r'\b(pipes are durable|is durable and resistant|resistant to corrosion|resistant to rot|in good condition|functioning (properly|correctly|normally|as intended)|no (repair|replacement|action) (needed|required|necessary|recommended))\b', re.IGNORECASE),
            # v5.72.8: Raw "Recommendation" prefix with section numbers
            re.compile(r'^Recommendation\s+(Contact|Recommended|Have|Consider|Consult)', re.IGNORECASE),
            # v5.72.8: Raw section number prefixes (e.g. "Item 2.3.2", "Maintenance Item 6.4.1")
            re.compile(r'\b(Maintenance Item|Item \d+\.\d+|Section \d+\.\d+)\b', re.IGNORECASE),
            # v5.72.8: ALL CAPS phrases that are clearly raw parser output (3+ consecutive uppercase words)
            re.compile(r'[A-Z]{3,}\s+[A-Z]{3,}\s+[A-Z]{3,}'),
            # v5.72.8: "Recommended DIY Project" boilerplate from inspection software
            re.compile(r'\b(Recommended DIY|DIY project|DIY Project|Contact a handyman)\b', re.IGNORECASE),
            # v5.72.9: Educational/normalizing language (not actual issues)
            re.compile(r'\b(is not uncommon|is (common|normal|typical) (in|for|especially)|commonly (found|seen|occurs)|is to be expected|nothing unusual)\b', re.IGNORECASE),
            # v5.72.9: Scope limitation disclaimers
            re.compile(r'\b(beyond the scope|outside the scope|not within the scope|lies beyond|exceeds the scope|not part of (this|the) inspection)\b', re.IGNORECASE),
            # v5.72.9: General age-related normalizing ("in homes over X years")
            re.compile(r'\b(in homes? (over|older than|built before)|for (a |this )?(home|house|property) (of this|this) age)\b', re.IGNORECASE),
            # v5.72.9: Leading asterisks/bullets from parser
    ...
    def _analyze_consistency(
        ...
        Now enhanced with Property Research Agent data:
        - FEMA flood zones vs seller claims
        - CGS earthquake zones vs seller claims  
        - CAL FIRE threat levels vs seller claims
        - FEMA disaster history for area
        """
        ...
        # 🤖 NEW: Check external verification findings from Property Research Agent
        external_verifications = metadata.get('external_verifications', [])
        for ev in external_verifications:
            if ev.get('severity') in ('high', 'medium'):
                consistency_issues.append({
                    'work_type': ev.get('category', 'external'),
                    'reason': ev.get('title', 'External data inconsistency'),
                    'detail': ev.get('detail', ''),
                    'source': ev.get('source', 'Public Records'),
                    'severity': ev.get('severity', 'medium'),
                    'external_verification': True  # Flag as research-agent-sourced
                })
        ...
        for issue in consistency_issues:
            if issue.get('external_verification'):
                # External verification issues: heavier deduction for high severity
                sev = issue.get('severity', 'medium')
                consistency_score -= 20 if sev == 'high' else 12
            else:
                consistency_score -= 15  # Permit-based deduction
    ...
    def _detect_red_flags(
        ...
        # RED FLAG 6: External public records contradict seller claims (Property Research Agent)
        # v5.59.29: Individual red flags per finding with specific sources cited
        external_issues = [i for i in consistency_issues if i.get('external_verification')]
        high_severity_external = [i for i in external_issues if i.get('severity') == 'high']
        
        for ext in high_severity_external:
            source = ext.get('source', 'Public Records')
            title = ext.get('reason', '')
            detail = ext.get('detail', '')
            
            # Build a specific, actionable description
            if detail:
                description = f'{source}: {detail}' if source != 'Public Records' else detail
            elif title:
                description = f'{source} data indicates: {title}' if source != 'Public Records' else title
            else:
                continue  # Skip if we have nothing specific to say
            
            # Build specific evidence from source data
            evidence_items = []
            if title and title != detail:
                evidence_items.append(title)
            if source and source != 'Public Records':
                evidence_items.append(f'Data source: {source}')
            
            # Build specific impact statement
            if 'flood' in (ext.get('work_type', '') + title + detail).lower():
                impact = f'{source} records indicate flood risk that may affect insurance costs and property value. This should have been disclosed by the seller.'
            elif 'earthquake' in (ext.get('work_type', '') + title + detail).lower() or 'seismic' in (ext.get('work_type', '') + title + detail).lower():
                impact = f'{source} records confirm seismic hazard exposure that affects insurance requirements and structural risk assessment.'
            elif 'fire' in (ext.get('work_type', '') + title + detail).lower():
                impact = f'{source} data shows fire hazard conditions that impact insurance availability and premiums.'
            elif 'value' in (ext.get('work_type', '') + title + detail).lower() or 'price' in (ext.get('work_type', '') + title + detail).lower():
                impact = f'{source} valuation data suggests the asking price may not align with comparable market data for this area.'
            else:
                impact = f'{source} data reveals conditions the seller did not address in their disclosure, which may affect your purchase decision.'
            
            red_flags.append(RedFlag(
                flag_type='public_records_contradiction',
                severity='critical',
                description=description,
                evidence=evidence_items if evidence_items else [f'Verified through {source}'],
                impact=impact,
                recommendation=f'Request the seller address this {source} finding in an amended disclosure. Use this documented evidence as leverage in negotiations.'
            ))
        
        # Medium-severity external issues: group if 2+
        medium_external = [i for i in external_issues if i.get('severity') != 'high' and i not in high_severity_external]
        if len(medium_external) >= 2:
            evidence_items = []
            for ext in medium_external:
                source = ext.get('source', 'Public Records')
                reason = ext.get('reason', '')
                detail = ext.get('detail', '')
                if detail:
                    evidence_items.append(f'{source}: {detail}' if source != 'Public Records' else detail)
                elif reason:
                    evidence_items.append(f'{source}: {reason}' if source != 'Public Records' else reason)
            
            if evidence_items:
                sources = list(set(ext.get('source', 'Public Records') for ext in medium_external if ext.get('source')))
                source_list = ', '.join(sources) if sources else 'public records'
                
                red_flags.append(RedFlag(
                    flag_type='public_records_gaps',
                    severity='major',
                    description=f'{len(medium_external)} findings from {source_list} not addressed in seller disclosure',
                    evidence=evidence_items,
                    impact=f'Data from {source_list} reveals conditions the seller did not disclose, which may indicate incomplete or selective disclosure.',
                    recommendation=f'Request the seller address these {source_list} findings in an amended disclosure.'
                ))
