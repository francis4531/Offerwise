...
def _attribute_inspector_referral(share_token, user):
    """v5.88.91: shared helper for inspector referral attribution at signup.

    Called from each of the 4 signup paths (Google OAuth new user, email
    new user, Apple OAuth, Facebook OAuth) so the attribution writes the
    same fields consistently — the prior pattern had 4 copy-pasted
    blocks that drifted (one tried to set _rpt.user_id which doesn't
    exist as a column, two bumped total_buyers_converted on signup
    which conflates registration with paid conversion, none of them
    persisted buyer_registered_at because the column didn't exist).

    Sets, on a successful match:
      InspectorReport.buyer_registered      = True
      InspectorReport.buyer_registered_at   = now
      InspectorReport.buyer_user_id         = user.id
      Inspector.total_buyers_registered    += 1   (signups, not paid)

    total_buyers_converted is now bumped only from the payment flow,
    not from signup. Caller is responsible for db.session.commit().
    Returns True if attribution wrote anything, False if no match or
    already attributed.
    """
    ...
    try:
        ...
        rpt = InspectorReport.query.filter_by(share_token=share_token).first()
        if not rpt or rpt.buyer_registered:
            return False
        now = datetime.utcnow()
        rpt.buyer_registered    = True
        rpt.buyer_registered_at = now
        rpt.buyer_user_id       = user.id
        insp = Inspector.query.get(rpt.inspector_id)
        if insp:
            insp.total_buyers_registered = (insp.total_buyers_registered or 0) + 1
        logging.info(
            f"📊 Inspector attribution: report→buyer {user.email} via "
            f"{share_token[:8]} (inspector_id={rpt.inspector_id})"
        )
        return True
    except Exception as e:
        logging.warning(f"Inspector attribution error: {e}")
        return False
...
def _apply_signup_attribution(user):
    """v5.89.66: Persist any captured UTM/gclid/referrer onto a newly
    created User row. Reads from session['signup_attribution'] which was
    populated by the OAuth entry route (/login/google or equivalent).

    Best-effort: any error is logged but never raised, since attribution
    must not block signup. Also handles the case where attribution columns
    don't exist yet (migration hasn't run) by using setattr with checks.
    """
    try:
        ...
        mapping = {
            ...
            'utm_term':     'signup_utm_term',
            'utm_content':  'signup_utm_content',
            'referrer':     'signup_referrer',
            'landing_page': 'signup_landing_page',
            'gclid':        'signup_gclid',
        }
        applied = []
        for src_key, dst_col in mapping.items():
            v = att.get(src_key)
            if v and hasattr(user, dst_col):
                setattr(user, dst_col, v)
                applied.append(dst_col)
        if applied:
            logging.info(f"📊 Attribution persisted on user {user.id}: {applied}")
    except Exception as e:
        logging.warning(f"⚠️ Attribution persist failed (non-critical): {e}")
...
def _capture_signup_attribution_to_session():
    """v5.89.66: Capture UTM/gclid/referrer from current request and stash
    in session for later persistence in the OAuth callback. Called at the
    OAuth entry routes (e.g. /login/google) BEFORE the redirect to the
    OAuth provider, since URL params get lost during the OAuth roundtrip.
    """
    _att = {}
    for k in ('utm_source', 'utm_medium', 'utm_campaign', 'utm_term', 'utm_content', 'gclid'):
        v = (request.args.get(k) or '').strip()[:255]
        if v:
            _att[k] = v
    try:
        _ref = (request.referrer or '')[:500]
        if _ref:
            _att['referrer'] = _ref
        _att['landing_page'] = (request.full_path or request.path or '')[:500]
    except Exception:
        pass
    if _att:
        session['signup_attribution'] = _att
        logging.info(f"📊 Captured signup attribution: {list(_att.keys())}")
...
def login_google():
    ...
    # v5.89.66: Stash UTM + gclid + referrer for ad attribution.
...
def google_callback():
    ...
    try:
        ...
        if user:
            ...
            _attribute_inspector_referral(_share_token, user)

            # v5.89.66: Persist UTM/gclid/referrer for first-party attribution
            _apply_signup_attribution(user)

            db.session.commit()
            # v5.89.66: signup_pixel_fired defaults False on new User rows,
            # so the conversion pixel will fire on the next page load that
            # checks /api/config/new-signup. No session flag needed.
...
def auth_register():
    ...
    # Validation
    # v5.88.09: tightened from "must contain @ and ." to a real check that
    # there's something on each side of the @ AND a tld after the dot.
    # Caught by test_register_invalid_email_returns_400 — the loose check
    # accepted '@nodomain.com' because it had both characters.
    _at_idx = email.find('@')
    if not email or _at_idx <= 0 or _at_idx == len(email) - 1:
        return jsonify({'error': 'Please enter a valid email address.'}), 400
    _local, _domain = email[:_at_idx], email[_at_idx+1:]
    ...
    # v5.89.66: Persist signup attribution + use DB flag for pixel fire
    _apply_signup_attribution(user)
...
def apple_callback():
    ...
    try:
        ...
        if user:
            ...
            # v5.89.66: Persist signup attribution (UTM/gclid/referrer)
            _apply_signup_attribution(user)
            try:
                db.session.commit()
            except Exception:
...
def send_magic_link():
    ...
    if not email or '@' not in email or '.' not in email:
        return jsonify({'error': 'Please enter a valid email address.'}), 400
    # v5.88.16: stricter validation. The simpler check above accepts
    # '@nodomain.com' (no local part). Use the shared helper which
    # also requires non-empty local + valid TLD.
    from blueprint_helpers import is_valid_email
    if not is_valid_email(email):
        return jsonify({'error': 'Please enter a valid email address.'}), 400
