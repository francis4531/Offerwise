...
@admin_bp.route('/api/admin/daily-tasks', methods=['GET'])
@_api_admin_req_dec
def api_daily_tasks():
    """Today's founder checklist + live metrics (powers the panel and email)."""
    import daily_tasks as dt
    return jsonify(dt.build_daily_tasks_data())


@admin_bp.route('/api/admin/daily-tasks/toggle', methods=['POST'])
@_api_admin_req_dec
def api_daily_tasks_toggle():
    """Mark a task done/undone for today. Body: {id, done}."""
    import daily_tasks as dt
    ...
    ids = dt.set_task_done(task_id, done, updated_by=_dt_actor())
    return jsonify({'success': True, 'done': ids})


@admin_bp.route('/api/admin/daily-tasks/extra', methods=['POST'])
@_api_admin_req_dec
def api_daily_tasks_extra():
    """Edit custom tasks. Body: {action:'add'|'remove', text:<str>, id:<str>}.
    Add needs text; remove accepts text or the task id."""
    import daily_tasks as dt
    data = request.get_json(silent=True) or {}
    action = (data.get('action') or '').strip()
    text = (data.get('text') or '').strip()
    rid = (data.get('id') or '').strip()
    current = dt.get_extra_tasks()
    if action == 'add':
        if not text:
            return jsonify({'error': 'text is required to add'}), 400
        if text not in current:
            current.append(text)
    elif action == 'remove':
        if rid:
            current = [t for t in current if dt._extra_id(t) != rid]
        elif text:
            current = [t for t in current if t != text]
        else:
            return jsonify({'error': 'id or text is required to remove'}), 400
    else:
        return jsonify({'error': "action must be 'add' or 'remove'"}), 400
    saved = dt.set_extra_tasks(current, updated_by=_dt_actor())
    return jsonify({'success': True, 'extra': saved})


@admin_bp.route('/api/admin/daily-tasks/settings', methods=['POST'])
@_api_admin_req_dec
def api_daily_tasks_settings():
    """Update morning-email settings. Body: {email_to?, enabled?}."""
    ...
    if 'email_to' in data:
        ...
        if to:
            SystemSetting.set('daily_tasks_email_to', to, updated_by=who)
    if 'enabled' in data:
        SystemSetting.set('daily_tasks_email_enabled',
                          '1' if data.get('enabled') else '0', updated_by=who)
    import daily_tasks as dt
    return jsonify({'success': True, 'email_to': dt.get_email_to(),
                    'email_enabled': dt.email_enabled()})


@admin_bp.route('/api/admin/daily-tasks/send', methods=['POST'])
@_api_admin_req_dec
def api_daily_tasks_send():
    """Send the daily-tasks email right now (test / on-demand)."""
    import daily_tasks as dt
    ok = dt.send_daily_tasks_email(force=True)
    if ok:
        return jsonify({'success': True, 'sent_to': dt.get_email_to()})
    return jsonify({'success': False,
                    'error': 'Send failed — check RESEND_API_KEY / email logs'}), 500
