...
from models import db, Property, Analysis, ShareLink
...
def submit_support_share():
    ...
    v5.88.34: creates a Ticket (was SupportShare). The first
    TicketMessage carries the analysis snapshot. SupportShare model is
    kept as legacy for one release (will be removed in v5.88.38).
    """
    try:
        from support_service import create_ticket_from_in_product_share
        data = request.get_json(silent=True) or {}
        property_id = data.get('property_id')
        ...
        # v5.88.34: create a Ticket (with the share as its first message)
        # instead of a SupportShare row. Admin notification sent inside.
        ticket = create_ticket_from_in_product_share(
            user=current_user,
            property_obj=prop,
        ...
        logging.info(f"🎫 Support ticket #{ticket.id} created from {current_user.email} for property {property_id}")
        return jsonify({'success': True, 'ticket_id': ticket.id,
                        # Backward-compat alias for any frontend code that
                        # still expects share_id:
                        'share_id': ticket.id})

    except Exception as e:
        logging.error(f"❌ Error creating support ticket: {e}", exc_info=True)
