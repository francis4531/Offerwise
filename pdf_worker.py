...
class PDFWorker:
    ...
    def _process_job(self, job_id: str):
        ...
        try:
            ...
            # Phase 1b/1d: for seller disclosures, structure the (now best-available)
            # TDS text into field state for the reasoning pipeline. Additive — uses
            # the existing extractor, never blocks the job on failure.
            if doc_type == 'seller_disclosure':
                try:
                    from pdf_handler import extract_tds_field_state
                    fs, fs_score, fs_notes = extract_tds_field_state(result['text'])
                    if fs is not None:
                        result['tds_field_state'] = fs
                        result['tds_field_state_confidence'] = fs_score
                        logger.info(f"🧩 Job {job_id}: TDS field state extracted (conf {fs_score:.2f})")
            ...
            if doc_type == 'inspection_report':
                try:
                    ...
                    if insp:
                        ...
                        result['inspection_extract_method'] = _ins.get('method')
                        _silent = sum(1 for r in insp if r.get('silent_hazard'))
                        logger.info(f"🔬 Job {job_id}: inspection findings extracted "
                                    f"via {_ins.get('method')} "
                                    f"({len(insp)} readings, {_silent} silent hazards)")
                except Exception as _ir_err:
                    logger.warning(f"🔬 Job {job_id}: inspection extraction skipped: {_ir_err}")
