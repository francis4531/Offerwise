...
class TestStripeInspectorProRouting(unittest.TestCase):
    ...
    def test_inspector_pro_price_id_from_env(self):
        """Price ID should come from env var, not be hardcoded in source code."""
        import os
        # Verify the price ID is set via environment variable (not hardcoded in source)
        price_id = os.environ.get('STRIPE_INSPECTOR_PRO_PRICE_ID', '')
        # Either the env var is set to a valid Stripe price ID, or it's empty (dev mode)
        # The important thing is it's NOT hardcoded — we just check it's a string
        self.assertIsInstance(price_id, str,
            "STRIPE_INSPECTOR_PRO_PRICE_ID must be a string env var")
