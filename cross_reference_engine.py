...
class CrossReferenceEngine:
    ...
    def cross_reference(
        ...
        **kwargs
        ...
        # v5.89.121: Distinguish two very different "no usable disclosure" cases.
        # Previously (Bug #27 fix) BOTH were collapsed into one extreme-caution
        # path that scored transparency 25, risk 75, marked every finding
        # "undisclosed", and told the buyer to "exercise extreme caution":
        #   (a) Bank-owned / as-is — the seller AFFIRMATIVELY provided no
        #       disclosure. Treating findings as undisclosed and flagging
        #       caution is appropriate here.
        #   (b) Disclosure simply not uploaded (the inspection-only flow) —
        #       we have NO basis to judge the seller's transparency at all.
        #       We must NOT brand the seller as "hiding everything" or discount
        #       the offer for a transparency problem we cannot observe. The
        #       previous behavior was defamatory and false in this case.
        content_lower = ''
        if disclosure_doc and getattr(disclosure_doc, 'content', None):
            content_lower = (disclosure_doc.content or '').lower()
        bank_owned_signal = any(kw in content_lower for kw in [
            'no disclosure', 'bank-owned', 'bank owned', 'foreclosure', 'sold as-is',
            'sold as is', 'seller declined', 'not available', 'reo property',
        ])
        disclosure_empty = (
            not disclosure_doc
            or not disclosure_doc.disclosure_items
            or len(disclosure_doc.disclosure_items) == 0
        )

        # Case (a): explicit bank-owned / as-is → keep extreme-caution framing.
        if bank_owned_signal:
            logging.warning("Disclosure indicates bank-owned / as-is — applying caution framing")
            undisclosed = []
            for finding in inspection_doc.inspection_findings:
                desc = finding.description or ''
                if self._is_non_issue(desc):
                    logging.info(f"🧹 Cross-ref filter (bank-owned): skipping non-issue: {desc[:80]}...")
                    continue
            ...
            return CrossReferenceReport(
                ...
                transparency_applicable=True,
            )

        # Case (b): disclosure simply not provided (inspection-only) → neutral.
        # Transparency is NOT applicable — we score risk from the inspection
        # findings themselves and suppress any transparency judgement downstream.
        if disclosure_empty:
            logging.info("No seller disclosure provided — transparency not applicable; scoring from inspection findings only")
            findings = inspection_doc.inspection_findings if inspection_doc else []
            ...
            return CrossReferenceReport(
                ...
                confirmed_disclosures=[],
                disclosed_not_found=[],
                transparency_score=50.0,   # neutral sentinel — trips no penalty threshold
                risk_score=risk_score,     # derived from the inspection findings
                summary=(
                    "No seller disclosure was provided, so this analysis is based on the "
                    "inspection report alone. Seller transparency cannot be assessed without a "
                    "disclosure. The repair costs, risk score, and offer strategy below reflect "
                    "the inspection findings."
                ),
                transparency_applicable=False,
            )
